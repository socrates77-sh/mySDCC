...
void updateCseDefAncestors(cseDef *cdp, set *cseSet)
{
  cseDef *loop;
  set *sl;
  iCode *ic = cdp->diCode;
...
DEFSETFUNC(findCheaperOp)
{
         ...
         (SPEC_LONG(operandType(cop)) == SPEC_LONG(operandType(*opp)))))))
  {
    ...
    if (IS_SPEC(operandType(cop)) && IS_SPEC(operandType(*opp)) && isOperandLiteral(*opp) && SPEC_NOUN(operandType(*opp)) == SPEC_NOUN(operandType(cop)) && SPEC_USIGN(operandType(*opp)) != SPEC_USIGN(operandType(cop)))
    {
...
algebraicOpts(iCode *ic, eBBlock *ebp)
{
  ...
  case '+':
    ...
    if (IC_LEFT(ic)->key == IC_RIGHT(ic)->key &&
        !(IS_FLOAT(operandType(IC_RESULT(ic))) || IS_FIXED(operandType(IC_RESULT(ic)))))
    {
      ic->op = LEFT_OP;
  ...
  case '!':
    if (IS_OP_LITERAL(IC_LEFT(ic)))
    {
      ic->op = '=';
      IC_RIGHT(ic) =
          (operandLitValue(IC_LEFT(ic)) == 0 ? operandFromLit(1) : operandFromLit(0));
      IC_LEFT(ic) = NULL;
      SET_ISADDR(IC_RESULT(ic), 0);
    }
    break;
...
/*-----------------------------------------------------------------*/
void updateSpillLocation(iCode *ic, int induction)
{
  sym_link *setype;
...
/*-----------------------------------------------------------------*/
int cseBBlock(eBBlock *ebb, int computeOnly, ebbIndex *ebbi)
{
  ...
  for (ic = ebb->sch; ic; ic = ic->next)
  {
        ...
        !computeOnly)
    {
      ...
      if (pdic && compareType(operandType(IC_RESULT(pdic)), operandType(IC_RESULT(ic))) != 1)
      {
        pdic = NULL;
      }
...
/*------------------------------------------------------------------*/
int cseAllBlocks(ebbIndex *ebbi, int computeOnly)
{
