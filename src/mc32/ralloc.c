...
set *mc32_dynAllocRegs = NULL;
set *mc32_dynStackRegs = NULL;
set *mc32_dynProcessorRegs = NULL;
set *mc32_dynDirectRegs = NULL;
set *mc32_dynDirectBitRegs = NULL;
set *mc32_dynInternalRegs = NULL;
...
static int mc32_ptrRegReq; /* one byte pointer register required */

static hTab *mc32_dynDirectRegNames = NULL;
// static hTab  *regHash = NULL;    /* a hash table containing ALL registers */

static int mc32_dynrIdx = 0x1000;

int mc32_Gstack_base_addr = 0; /* The starting address of registers that
                         * are used to pass and return parameters */
static int mc32_Gstack_size = 0;

static int mc32_debug = 0; // should be 0 when committed, creates .d files
static FILE *mc32_debugF = NULL;
...
// mc32_debugLog(char *fmt, ...)
{
        ...
        if (!mc32_debugF)
        {
                ...
                append = 1; // Next time mc32_debugLog is called, we'll append the mc32_debug info
        }
        ...
        fprintf(mc32_debugF, "%s", buffer);
        //if (options.verbose) fprintf (stderr, "%s: %s", __FUNCTION__, buffer);
}

static void
mc32_debugNewLine(void)
{
        if (mc32_debugF)
                fputc('\n', mc32_debugF);
}
/*-----------------------------------------------------------------*/
/* mc32_debugLogClose - closes the mc32_debug log file (if opened)           */
/*-----------------------------------------------------------------*/
void mc32_debugLogClose(void)
{
        if (mc32_debugF)
        {
                fclose(mc32_debugF);
...
/*-----------------------------------------------------------------*/
/*-----------------------------------------------------------------*/
static int mc32_regname2key(char const *name)
{
        int key = 0;
...
/*-----------------------------------------------------------------*/
/* mc32_regWithIdx - Search through a set of registers that matches idx */
/*-----------------------------------------------------------------*/
static reg_info *
mc32_regWithIdx(set *dRegs, int idx, int fixed)
{
        reg_info *dReg;
...
// static reg_info *mc32_newReg(short type, PIC_OPTYPE pc_type, int rIdx, char *name, int size, int alias)
{
        ...
        if (type != REG_STK)
                hTabAddItem(&mc32_dynDirectRegNames, mc32_regname2key(dReg->name), dReg);
        mc32_debugLog("%s: Created register %s.\n", __FUNCTION__, dReg->name);

        return dReg;
}

/*-----------------------------------------------------------------*/
/* mc32_regWithName - Search through a set of registers that matches name */
/*-----------------------------------------------------------------*/
static reg_info *
mc32_regWithName(set *dRegs, const char *name)
{
        reg_info *dReg;
...
/*-----------------------------------------------------------------*/
/* mc32_regWithName - Search for a registers that matches name          */
/*-----------------------------------------------------------------*/
reg_info *
mc32_regFindWithName(const char *name)
{
        reg_info *dReg;

        if ((dReg = mc32_regWithName(mc32_dynDirectRegs, name)) != NULL)
        {
                mc32_debugLog("Found a Direct Register!\n");
                return dReg;
        }
        if ((dReg = mc32_regWithName(mc32_dynDirectBitRegs, name)) != NULL)
        {
                mc32_debugLog("Found a Direct Bit Register!\n");
                return dReg;
        }
        ...
        if ((dReg = mc32_regWithName(mc32_dynAllocRegs, name)) != NULL)
        {
                mc32_debugLog("Found a Dynamic Register!\n");
                return dReg;
        }
        if ((dReg = mc32_regWithName(mc32_dynProcessorRegs, name)) != NULL)
        {
                mc32_debugLog("Found a Processor Register!\n");
                return dReg;
        }
        if ((dReg = mc32_regWithName(mc32_dynInternalRegs, name)) != NULL)
        {
                mc32_debugLog("Found an Internal Register!\n");
                return dReg;
        }
        if ((dReg = mc32_regWithName(mc32_dynStackRegs, name)) != NULL)
        {
                mc32_debugLog("Found an Stack Register!\n");
                return dReg;
        }
...
/*-----------------------------------------------------------------*/
/* mc32_regFindFree - Search for a free register in a set of registers  */
/*-----------------------------------------------------------------*/
static reg_info *
mc32_regFindFree(set *dRegs)
{
        reg_info *dReg;
...
/*-----------------------------------------------------------------*/
/* mc32_initStack - allocate registers for a pseudo stack               */
/*-----------------------------------------------------------------*/
void mc32_initStack(int base_address, int size, int shared)
{
        ...
        pic = mc32_getPIC();
        mc32_Gstack_base_addr = base_address;
        mc32_Gstack_size = size;
        //fprintf(stderr,"mc32_initStack [base:0x%02x, size:%d]\n", base_address, size);
...
// mc32_allocProcessorRegister(int rIdx, char *name, short po_type, int alias)
{
        ...
        return addSet(&mc32_dynProcessorRegs, mc32_newReg(REG_SFR, po_type, rIdx, name, 1, alias));
}
...
/*-----------------------------------------------------------------*/
/* mc32_allocReg - allocates register of given type                     */
/*-----------------------------------------------------------------*/
static reg_info *
mc32_allocReg(short type)
{
        reg_info *reg;

        mc32_debugLog("%s of type %s\n", __FUNCTION__, mc32_debugLogRegType(type));
        //fprintf(stderr,"mc32_allocReg\n");
        ...
        //return addSet(&mc32_dynAllocRegs,mc32_newReg(REG_GPR, PO_GPR_TEMP,mc32_dynrIdx++,NULL,1,0));
}
...
// mc32_dirregWithName(char *name)
{
        ...
        hkey = mc32_regname2key(name);

        reg = hTabFirstItemWK(mc32_dynDirectRegNames, hkey);
        ...
        while (reg)
        {
                ...
                reg = hTabNextItemWK(mc32_dynDirectRegNames);
        }
...
/*-----------------------------------------------------------------*/
/* mc32_allocNewDirReg - allocates a new register of given type         */
/*-----------------------------------------------------------------*/
reg_info *
mc32_allocNewDirReg(sym_link *symlnk, const char *name)
{
        reg_info *reg;
        int address = 0;
        ...
        * mc32_dynDirectRegNames set */
        if (MC32_IS_CONFIG_ADDRESS(address))
        {
                mc32_debugLog("  -- %s is declared at a config word address (0x%x)\n", name, address);
                reg = 0;
        }
        else
        {
                ...
                reg = mc32_newReg(REG_GPR, PO_DIR, idx, (char *)name, getSize(symlnk), 0);
                mc32_debugLog("  -- added %s to hash, size = %d\n", (char *)name, reg->size);
...
/*-----------------------------------------------------------------*/
/* mc32_allocDirReg - allocates register of given type                  */
/*-----------------------------------------------------------------*/
reg_info *
mc32_allocDirReg(operand *op)
{
        ...
        if (!IS_SYMOP(op))
        {
                mc32_debugLog("%s BAD, op is NULL\n", __FUNCTION__);
                return NULL;
        }
        ...
        * to match the .c declared register names */
        ...
        mc32_debugLog("%s symbol name %s\n", __FUNCTION__, name);
        {
                if (SPEC_CONST(OP_SYM_ETYPE(op)) && (IS_CHAR(OP_SYM_ETYPE(op))))
                {
                        mc32_debugLog(" %d  const char\n", __LINE__);
                        mc32_debugLog(" value = %s \n", SPEC_CVAL(OP_SYM_ETYPE(op)));
                }

                mc32_debugLog("  %d  storage class %d \n", __LINE__, SPEC_SCLS(OP_SYM_ETYPE(op)));
                if (IS_CODE(OP_SYM_ETYPE(op)))
                        mc32_debugLog(" %d  code space\n", __LINE__);

                if (IS_INTEGRAL(OP_SYM_ETYPE(op)))
                        mc32_debugLog(" %d  integral\n", __LINE__);
                if (IS_LITERAL(OP_SYM_ETYPE(op)))
                        mc32_debugLog(" %d  literal\n", __LINE__);
                if (IS_SPEC(OP_SYM_ETYPE(op)))
                        mc32_debugLog(" %d  specifier\n", __LINE__);
                mc32_debugAopGet(NULL, op);
        }
        ...
        if(!reg) {
                ...
                * mc32_dynDirectRegNames set */
                if(!MC32_IS_CONFIG_ADDRESS(address)) {
                        ...
                        reg = mc32_newReg(REG_GPR, PO_DIR, mc32_dynrIdx++, name,getSize (OP_SYMBOL (op)->type),0 );
                        mc32_debugLog ("  -- added %s to hash, size = %d\n", name,reg->size);

                        //hTabAddItem(&mc32_dynDirectRegNames, mc32_regname2key(name), reg);
        ...
        {
                mc32_allocNewDirReg(OP_SYM_TYPE(op), name);
        }
...
// mc32_allocRegByName(char *name, int size)
{
        ...
        if (!reg)
        {
                ...
                * mc32_dynDirectRegNames set */
                ...
                reg = mc32_newReg(REG_GPR, PO_DIR, mc32_dynrIdx++, name, size, 0);
...
/*-----------------------------------------------------------------*/
reg_info *
mc32_typeRegWithIdx(int idx, int type, int fixed)
{

        reg_info *dReg;

        mc32_debugLog("%s - requesting index = 0x%x\n", __FUNCTION__, idx);

        switch (type)
        {

        case REG_GPR:
                if ((dReg = mc32_regWithIdx(mc32_dynAllocRegs, idx, fixed)) != NULL)
                {

                        mc32_debugLog("Found a Dynamic Register!\n");
                        return dReg;
                }
                if ((dReg = mc32_regWithIdx(mc32_dynDirectRegs, idx, fixed)) != NULL)
                {
                        mc32_debugLog("Found a Direct Register!\n");
                        return dReg;
                }
        ...
        case REG_STK:
                ...
                else
                {
                        ...
                        return mc32_regWithIdx(mc32_dynStackRegs, 0x7f, 0);
                }
                break;
        case REG_SFR:
                if ((dReg = mc32_regWithIdx(mc32_dynProcessorRegs, idx, fixed)) != NULL)
                {
                        mc32_debugLog("Found a Processor Register!\n");
                        return dReg;
                }
...
/*-----------------------------------------------------------------*/
/* mc32_pic14_regWithIdx - returns pointer to register with index number*/
/*-----------------------------------------------------------------*/
reg_info *
mc32_pic14_regWithIdx(int idx)
{
        reg_info *dReg;

        if ((dReg = mc32_typeRegWithIdx(idx, REG_GPR, 0)) != NULL)
                return dReg;

        if ((dReg = mc32_typeRegWithIdx(idx, REG_SFR, 0)) != NULL)
                return dReg;
...
mc32_allocWithIdx(int idx)
{
        ...
        else
        {
                ...
                mc32_debugLog("Dynamic Register not found\n");

                //fprintf(stderr,"%s %d - requested register: 0x%x\n",__FUNCTION__,__LINE__,idx);
                werror(E_INTERNAL_ERROR, __FILE__, __LINE__,
                       "mc32_regWithIdx not found");
                exit(1);
        }
...
mc32_findFreeReg(short type)
{
        ...
        {
        case REG_GPR:
                if ((dReg = mc32_regFindFree(mc32_dynAllocRegs)) != NULL)
                        return dReg;
                return addSet(&mc32_dynAllocRegs, mc32_newReg(REG_GPR, PO_GPR_TEMP, mc32_dynrIdx++, NULL, 1, 0));

        case REG_STK:

                if ((dReg = mc32_regFindFree(mc32_dynStackRegs)) != NULL)
                        return dReg;
...
/*-----------------------------------------------------------------*/
/* mc32_nFreeRegs - returns number of free registers                    */
/*-----------------------------------------------------------------*/
static int
mc32_nFreeRegs(int type)
{
...
/*-----------------------------------------------------------------*/
/* mc32_nfreeRegsType - free registers with type                         */
/*-----------------------------------------------------------------*/
static int
mc32_nfreeRegsType(int type)
{
        int nfr;
        mc32_debugLog("%s\n", __FUNCTION__);
        if (type == REG_PTR)
        {
                if ((nfr = mc32_nFreeRegs(type)) == 0)
                        return mc32_nFreeRegs(REG_GPR);
        }

        return mc32_nFreeRegs(type);
}

static void mc32_packBits(set *bregs)
{
        set *regset;
        reg_info *breg;
             ...
             regset = regset->next)
        {
                ...
                if (breg->isFixed)
                {
                        ...
                        bitfield = mc32_typeRegWithIdx(breg->address >> 3, -1, 1);
                        ...
                        if (!bitfield)
                        {
                                ...
                                //hTabAddItem(&mc32_dynDirectRegNames, mc32_regname2key(buffer), bitfield);
                        }
                        else
                        {
                ...
                else
                {
                        ...
                        breg->address = mc32_dynrIdx; /* byte_no; */
                        breg->rIdx = bit_no++;
                }
        }
}

static void mc32_bitEQUs(FILE *of, set *bregs)
{
...
void mc32_writeUsedRegs(FILE *of)
{

        mc32_packBits(mc32_dynDirectBitRegs);

        mc32_bitEQUs(of, mc32_dynDirectBitRegs);
}

/*-----------------------------------------------------------------*/
/* mc32_computeSpillable - given a point find the spillable live ranges */
/*-----------------------------------------------------------------*/
static bitVect *
mc32_computeSpillable(iCode *ic)
{
        bitVect *spillable;
...
/*-----------------------------------------------------------------*/
/* mc32_noSpilLoc - return true if a variable has no spil location      */
/*-----------------------------------------------------------------*/
static int
...
/*-----------------------------------------------------------------*/
/* mc32_hasSpilLoc - will return 1 if the symbol has spil location      */
/*-----------------------------------------------------------------*/
static int
...
/*-----------------------------------------------------------------*/
/* mc32_directSpilLoc - will return 1 if the splilocation is in direct  */
/*-----------------------------------------------------------------*/
static int
mc32_directSpilLoc(symbol *sym, eBBlock *ebp, iCode *ic)
{
        mc32_debugLog("%s\n", __FUNCTION__);
        if (sym->usl.spillLoc &&
...
/*-----------------------------------------------------------------*/
/* mc32_hasSpilLocnoUptr - will return 1 if the symbol has spil location */
...
/*-----------------------------------------------------------------*/
/* mc32_rematable - will return 1 if the remat flag is set              */
/*-----------------------------------------------------------------*/
static int
mc32_rematable(symbol *sym, eBBlock *ebp, iCode *ic)
{
        mc32_debugLog("%s\n", __FUNCTION__);
        return sym->remat;
}
...
/*-----------------------------------------------------------------*/
/* mc32_notUsedInRemaining - not used or defined in remain of the block */
/*-----------------------------------------------------------------*/
static int
mc32_notUsedInRemaining(symbol *sym, eBBlock *ebp, iCode *ic)
{
        mc32_debugLog("%s\n", __FUNCTION__);
...
/*-----------------------------------------------------------------*/
/* mc32_allLRs - return true for all                                    */
/*-----------------------------------------------------------------*/
static int
mc32_allLRs(symbol *sym, eBBlock *ebp, iCode *ic)
{
        mc32_debugLog("%s\n", __FUNCTION__);
        return 1;
}
...
/*-----------------------------------------------------------------*/
/* mc32_liveRangesWith - applies function to a given set of live range  */
/*-----------------------------------------------------------------*/
static set *
                    ...
                    eBBlock *ebp, iCode *ic)
{
        ...
        for (i = 1; i < lrs->size; i++)
        {
                ...
                {
                        werror(E_INTERNAL_ERROR, __FILE__, __LINE__,
                               "mc32_liveRangesWith could not find liveRange");
                        exit(1);
                }
...
/*-----------------------------------------------------------------*/
/* mc32_leastUsedLR - given a set determines which is the least used    */
/*-----------------------------------------------------------------*/
static symbol *
mc32_leastUsedLR(set *sset)
{
...
/*-----------------------------------------------------------------*/
/* mc32_noOverLap - will iterate through the list looking for over lap  */
/*-----------------------------------------------------------------*/
static int
mc32_noOverLap(set *itmpStack, symbol *fsym)
{
        symbol *sym;
        mc32_debugLog("%s\n", __FUNCTION__);
...
static DEFSETFUNC(isFree)
{
        ...
        if (sym->isFree &&
            mc32_noOverLap(sym->usl.itmpStack, fsym) &&
...
/*-----------------------------------------------------------------*/
/* mc32_spillLRWithPtrReg :- will spil those live ranges which use PTR  */
/*-----------------------------------------------------------------*/
static void
mc32_spillLRWithPtrReg(symbol *forSym)
{
        symbol *lrsym;
        int k;
...
/*-----------------------------------------------------------------*/
/* mc32_createStackSpil - create a location on the stack to spil        */
/*-----------------------------------------------------------------*/
static symbol *
mc32_createStackSpil(symbol *sym)
{
        symbol *sloc = NULL;
...
/*-----------------------------------------------------------------*/
/* mc32_isSpiltOnStack - returns true if the spil location is on stack  */
/*-----------------------------------------------------------------*/
static bool
mc32_isSpiltOnStack(symbol *sym)
{
        sym_link *etype;
...
/*-----------------------------------------------------------------*/
/* mc32_spillThis - spils a specific operand                            */
/*-----------------------------------------------------------------*/
static void
mc32_spillThis(symbol *sym)
{
        int i;
        ...
        LIVE ranges */
        if (!mc32_ptrRegReq && mc32_isSpiltOnStack(sym))
        {
                mc32_ptrRegReq++;
                mc32_spillLRWithPtrReg(sym);
        }
...
/*-----------------------------------------------------------------*/
/* mc32_selectSpil - select a iTemp to spil : rather a simple procedure */
/*-----------------------------------------------------------------*/
static symbol *
mc32_selectSpil(iCode *ic, eBBlock *ebp, symbol *forSym)
{
        bitVect *lrcs = NULL;
        set *selectS;
        symbol *sym;
        ...
        lrcs = mc32_computeSpillable(ic);

        /* get all live ranges that are rematerizable */
        if ((selectS = mc32_liveRangesWith(lrcs, mc32_rematable, ebp, ic)))
        {
                /* return the least used of these */
                return mc32_leastUsedLR(selectS);
        }

        /* get live ranges with spillLocations in direct space */
        if ((selectS = mc32_liveRangesWith(lrcs, mc32_directSpilLoc, ebp, ic)))
        {
                sym = mc32_leastUsedLR(selectS);
        ...
        if (forSym->liveTo < ebp->lSeq)
        {
                ...
                if (!_G.blockSpil && (selectS = mc32_liveRangesWith(lrcs, notUsedInBlock, ebp, ic)))
                {
                        sym = mc32_leastUsedLR(selectS);
                ...
                if (!_G.blockSpil &&
                    ...
                    (selectS = mc32_liveRangesWith(lrcs, mc32_notUsedInRemaining, ebp, ic)))
                {
                        sym = mc32_leastUsedLR(selectS);
                        if (!sym->remat)
                        {
        ...
        if ((selectS = mc32_liveRangesWith(lrcs, mc32_hasSpilLocnoUptr, ebp, ic)))
        {
        ...
        if ((selectS = mc32_liveRangesWith(lrcs, mc32_hasSpilLoc, ebp, ic)))
        {
        ...
        used ofcourse */
        if ((selectS = mc32_liveRangesWith(lrcs, mc32_noSpilLoc, ebp, ic)))
        {

                /* return a created spil location */
                sym = mc32_createStackSpil(mc32_leastUsedLR(selectS));
                sym->usl.spillLoc->allocreq = 1;
                return sym;
        }
        ...
        mc32_spillThis(forSym);
        return forSym;
}

/*-----------------------------------------------------------------*/
/* mc32_spilSomething - spil some variable & mark registers as free     */
/*-----------------------------------------------------------------*/
static bool
mc32_spilSomething(iCode *ic, eBBlock *ebp, symbol *forSym)
{
        symbol *ssym;
        int i;
        ...
        ssym = mc32_selectSpil(ic, ebp, forSym);
        ...
        for (i = 0; i < ssym->nRegs; i++)
                if (ssym->regs[i])
                        mc32_freeReg(ssym->regs[i]);

        /* if spilt on stack then free up r0 & r1
                if they could have been assigned to as gprs */
        if (!mc32_ptrRegReq && mc32_isSpiltOnStack(ssym))
        {
                mc32_ptrRegReq++;
                mc32_spillLRWithPtrReg(ssym);
        }
...
/*-----------------------------------------------------------------*/
/* mc32_getRegPtr - will try for PTR if not a GPR type if not spil      */
/*-----------------------------------------------------------------*/
static reg_info *
mc32_getRegPtr(iCode *ic, eBBlock *ebp, symbol *sym)
{
        reg_info *reg;
        int j;
...
tryAgain:
        /* try for a ptr type */
        if ((reg = mc32_allocReg(REG_PTR)))
                return reg;

        /* try for gpr type */
        if ((reg = mc32_allocReg(REG_GPR)))
                return reg;

        /* we have to spil */
        if (!mc32_spilSomething(ic, ebp, sym))
                return NULL;
...
/*-----------------------------------------------------------------*/
/* mc32_getRegGpr - will try for GPR if not spil                        */
/*-----------------------------------------------------------------*/
static reg_info *
mc32_getRegGpr(iCode *ic, eBBlock *ebp, symbol *sym)
{
        reg_info *reg;
        int j;
...
/*-----------------------------------------------------------------*/
/* mc32_symHasReg - symbol has a given register                         */
/*-----------------------------------------------------------------*/
static bool
mc32_symHasReg(symbol *sym, reg_info *reg)
{
        int i;
...
/*-----------------------------------------------------------------*/
/* mc32_deassignLRs - check the live to and if they have registers & are */
/*               not spilt then free up the registers              */
/*-----------------------------------------------------------------*/
static void
mc32_deassignLRs(iCode *ic, eBBlock *ebp)
{
        symbol *sym;
        int k;
        symbol *result;
             ...
             sym = hTabNextItem(liveRanges, &k))
        {
                ...
                if (sym->nRegs)
                {
                        ...
                        if (IC_RESULT(ic) &&
                            ...
                            ((mc32_nfreeRegsType(result->regType) +
                              sym->nRegs) >= result->nRegs))
                        {
                                ...
                                for (i = 0; i < max(sym->nRegs, result->nRegs); i++)
                                        if (i < sym->nRegs)
                                                ...
                                                result->regs[i] = mc32_getRegGpr(ic, ebp, result);
                        ...
                        for (; i < sym->nRegs; i++)
                        {
                                if (psym)
                                {
                                        if (!mc32_symHasReg(psym, sym->regs[i]))
                                                mc32_freeReg(sym->regs[i]);
                                }
                                else
                                        mc32_freeReg(sym->regs[i]);
...
/*-----------------------------------------------------------------*/
/* mc32_reassignLR - reassign this to registers                         */
/*-----------------------------------------------------------------*/
static void
mc32_reassignLR(operand *op)
{
...
/*-----------------------------------------------------------------*/
/* mc32_willCauseSpill - determines if allocating will cause a spill    */
/*-----------------------------------------------------------------*/
static int
mc32_willCauseSpill(int nr, int rt)
{
        mc32_debugLog("%s\n", __FUNCTION__);
...
/*-----------------------------------------------------------------*/
/* mc32_positionRegs - the allocator can allocate same registers to res- */
...
/*------------------------------------------------------------------*/
/* mc32_verifyRegsAssigned - make sure an iTemp is properly initialized; */
/* it should either have registers or have beed spilled. Otherwise, */
/* there was an uninitialized variable, so just spill this to get   */
/* the operand in a valid state.                                    */
/*------------------------------------------------------------------*/
static void
mc32_verifyRegsAssigned(operand *op, iCode *ic)
{
        symbol *sym;
...
/*-----------------------------------------------------------------*/
/* mc32_serialRegAssign - serially allocate registers to the variables  */
/*-----------------------------------------------------------------*/
static void
mc32_serialRegAssign(eBBlock **ebbs, int count)
{
        int i;
        ...
        for (i = 0; i < count; i++)
        {
                ...
                for (ic = ebbs[i]->sch; ic; ic = ic->next)
                {
                        ...
                        if (IC_RESULT(ic) && IS_SYMOP(IC_RESULT(ic)))
                        {
                                ...
                                spill this one */
                                willCS = mc32_willCauseSpill(sym->nRegs, sym->regType);

                                // zwr 1.0.0
                                /* explicit turn off register spilling */
                                willCS = 0;

                                spillable = mc32_computeSpillable(ic);
                                if (sym->remat ||
                                ...
                                if (willCS)
                                {
                                        ...
                                        else
                                        {
                                                ...
                                                if (mc32_liveRangesWith(spillable, mc32_noSpilLoc, ebbs[i], ic))
                                                {
                                ...
                                if (ic->op == RECEIVE)
                                        mc32_debugLog("When I get clever, I'll optimize the receive logic\n");
                                ...
                                mc32_debugLog("  %d - \n", __LINE__);
                                if (mc32_debugF)
                                        bitVectDebugOn(_G.regAssigned, mc32_debugF);
                                for (j = 0; j < sym->nRegs; j++)
                                {
                                        if (sym->regType == REG_PTR)
                                                sym->regs[j] = mc32_getRegPtr(ic, ebbs[i], sym);
                                        else
                                                sym->regs[j] = mc32_getRegGpr(ic, ebbs[i], sym);
        ...
        for (i = 0; i < count; i++)
        {
                ...
                for (ic = ebbs[i]->sch; ic; ic = ic->next)
                {
                        ...
                        if (ic->op == IFX)
                        {
                                mc32_verifyRegsAssigned(IC_COND(ic), ic);
                                continue;
                        }

                        if (ic->op == JUMPTABLE)
                        {
                                mc32_verifyRegsAssigned(IC_JTCOND(ic), ic);
                                continue;
                        }

                        mc32_verifyRegsAssigned(IC_RESULT(ic), ic);
                        mc32_verifyRegsAssigned(IC_LEFT(ic), ic);
                        mc32_verifyRegsAssigned(IC_RIGHT(ic), ic);
                }
        }
}

/*-----------------------------------------------------------------*/
/* mc32_rUmaskForOp :- returns register mask for an operand             */
/*-----------------------------------------------------------------*/
static bitVect *
mc32_rUmaskForOp(operand *op)
{
        bitVect *rumask;
        symbol *sym;
        int j;
...
/*-----------------------------------------------------------------*/
/* mc32_regsUsedIniCode :- returns bit vector of registers used in iCode */
/*-----------------------------------------------------------------*/
static bitVect *
mc32_regsUsedIniCode(iCode *ic)
{
        ...
        if (IC_LEFT(ic))
                rmask = bitVectUnion(rmask,
                                     mc32_rUmaskForOp(IC_LEFT(ic)));

        if (IC_RIGHT(ic))
                rmask = bitVectUnion(rmask,
                                     mc32_rUmaskForOp(IC_RIGHT(ic)));

        if (IC_RESULT(ic))
                rmask = bitVectUnion(rmask,
                                     mc32_rUmaskForOp(IC_RESULT(ic)));
...
/*-----------------------------------------------------------------*/
/* mc32_createRegMask - for each instruction will determine the regsUsed */
/*-----------------------------------------------------------------*/
static void
mc32_createRegMask(eBBlock **ebbs, int count)
{
        int i;
        ...
        for (i = 0; i < count; i++)
        {
                ...
                for (ic = ebbs[i]->sch; ic; ic = ic->next)
                {
                        ...
                        instruction */
                        ic->rUsed = mc32_regsUsedIniCode(ic);
                        ...
                        for (j = 1; j < ic->rlive->size; j++)
                        {
                                ...
                                {
                                        werror(E_INTERNAL_ERROR, __FILE__, __LINE__,
                                               "mc32_createRegMask cannot find live range");
                                        exit(0);
                                }
...
mc32_regTypeNum(void)
{
             ...
             sym = hTabNextItem(liveRanges, &k))
        {
                ...
                if (sym->isitmp)
                {
                        ...
                        mc32_debugLog("  %d - itemp register\n", __LINE__);
                        ...
                        if (sym->accuse)
                        {
                                ...
                                mc32_debugLog("  %d - no reg needed - accumulator used\n", __LINE__);
                        ...
                        if (sym->ruonly)
                        {
                                //if (IS_AGGREGATE (sym->type) || sym->isptr)
                                //  sym->type = aggrToPtr (sym->type, FALSE);
                                mc32_debugLog("  %d - used as a return\n", __LINE__);
                        ...
                        {
                                mc32_debugLog("  %d const pointer type requires %d registers, changing to 2\n", __LINE__, sym->nRegs);
                                sym->nRegs = 2;
                        }
                        ...
                        mc32_debugLog("  reg name %s,  reg type %s\n", sym->rname, mc32_debugLogRegType(sym->regType));
                }
                else
...
/*-----------------------------------------------------------------*/
/* mc32_farSpacePackable - returns the packable icode for far variables */
/*-----------------------------------------------------------------*/
static iCode *
mc32_farSpacePackable(iCode *ic)
{
        iCode *dic;
...
/*-----------------------------------------------------------------*/
/* mc32_packRegsForAssign - register reduction for assignment           */
/*-----------------------------------------------------------------*/
static int
mc32_packRegsForAssign(iCode *ic, eBBlock *ebp)
{
        ...
        mc32_debugAopGet("  result:", IC_RESULT(ic));
        mc32_debugAopGet("  left:", IC_LEFT(ic));
        mc32_debugAopGet("  right:", IC_RIGHT(ic));

        /* if this is at an absolute address, then get the address. */
        if (SPEC_ABSA(OP_SYM_ETYPE(IC_RESULT(ic))))
        {
                if (MC32_IS_CONFIG_ADDRESS(SPEC_ADDR(OP_SYM_ETYPE(IC_RESULT(ic)))))
                {
                        mc32_debugLog("  %d - found config word declaration\n", __LINE__);
        ...
        {
                mc32_allocDirReg(IC_RESULT(ic));
                mc32_debugLog("  %d - result is not temp\n", __LINE__);
        }
        /*
        if (IC_LEFT (ic) && !IS_ITEMP (IC_LEFT (ic))) {
        mc32_debugLog ("  %d - left is not temp, allocating\n", __LINE__);
        ...
        */
        ...
        {
                mc32_debugLog("  %d - not packing - right is not temp\n", __LINE__);
        ...
        for (dic = ic->prev; dic; dic = dic->prev)
        {
                ...
                {
                        mc32_debugLog("  %d - dic is VOLATILE \n", __LINE__);
                        dic = NULL;
                        break;
                }
                ...
                {
                        mc32_debugLog("  %d - dic right key == ic rightor result key\n", __LINE__);
                        dic = NULL;
                        break;
                }
                ...
                {
                        mc32_debugLog("  %d - dic left key == ic rightor result key\n", __LINE__);
                        dic = NULL;
                        break;
                }
...
pack:
        ...
        mc32_debugLog("  replacing with %s\n", OP_SYMBOL(IC_RESULT(dic))->rname);
...
/*-----------------------------------------------------------------*/
/* mc32_findAssignToSym : scanning backwards looks for first assig found */
/*-----------------------------------------------------------------*/
static iCode *
mc32_findAssignToSym(operand *op, iCode *ic)
{
        iCode *dic;
...
/*-----------------------------------------------------------------*/
/* mc32_packRegsForSupport :- reduce some registers for support calls   */
/*-----------------------------------------------------------------*/
static int
mc32_packRegsForSupport(iCode *ic, eBBlock *ebp)
{
        int change = 0;
        ...
        {
                iCode *dic = mc32_findAssignToSym(IC_LEFT(ic), ic);
                iCode *sic;

                if (!dic)
                        goto right;

                mc32_debugAopGet("removing left:", IC_LEFT(ic));
...
right:
        ...
        {
                iCode *dic = mc32_findAssignToSym(IC_RIGHT(ic), ic);
                iCode *sic;
                ...
                mc32_debugAopGet("removing right:", IC_RIGHT(ic));
...
/*-----------------------------------------------------------------*/
/* mc32_packRegsForOneuse : - will reduce some registers for single Use */
/*-----------------------------------------------------------------*/
static iCode *
mc32_packRegsForOneuse(iCode *ic, operand *op, eBBlock *ebp)
{
        bitVect *uses;
        iCode *dic, *sic;
        ...
        the usage of b, & acc */
        if (getSize(operandType(op)) > (mc32_fReturnSizePic - 2) &&
            ic->op != RETURN &&
            ic->op != SEND)
                return NULL;
...
/*-----------------------------------------------------------------*/
/* mc32_isBitwiseOptimizable - requirements of JEAN LOUIS VERN          */
/*-----------------------------------------------------------------*/
static bool
mc32_isBitwiseOptimizable(iCode *ic)
{
...
/*-----------------------------------------------------------------*/
/* mc32_packRegsForAccUse - pack registers for acc use                  */
/*-----------------------------------------------------------------*/
static void
mc32_packRegsForAccUse(iCode *ic)
{
        //iCode *uic;
...
accuse:
        mc32_debugLog ("%s - Yes we are using the accumulator\n", __FUNCTION__);
        OP_SYMBOL (IC_RESULT (ic))->accuse = 1;
#endif
}

/*-----------------------------------------------------------------*/
/* mc32_packForPush - hueristics to reduce iCode for pushing            */
/*-----------------------------------------------------------------*/
static void
mc32_packForReceive(iCode *ic, eBBlock *ebp)
{
        iCode *dic;
        ...
        mc32_debugLog("  hey we can remove this unnecessary assign\n");
}
/*-----------------------------------------------------------------*/
/* mc32_packForPush - hueristics to reduce iCode for pushing            */
/*-----------------------------------------------------------------*/
static void
mc32_packForPush(iCode *ic, eBBlock *ebp)
{
        iCode *dic, *lic;
        bitVect *dbv;
        ...
        /* ebp (see the mcs51 version of mc32_packForPush), but we weren't passed    */
...
static void mc32_printSymType(char *str, sym_link *sl)
{
        if (mc32_debug)
        {
                mc32_debugLog("    %s Symbol type: ", str);
                printTypeChain(sl, mc32_debugF);
                mc32_debugLog("\n");
        }
}

/*-----------------------------------------------------------------*/
/* some mc32_debug code to print the symbol S_TYPE. Note that
* the function checkSClass in src/SDCCsymt.c dinks with
* the S_TYPE in ways the PIC port doesn't fully like...*/
/*-----------------------------------------------------------------*/
static void mc32_isData(sym_link *sl)
{
        FILE *of = stderr;
...
/*-----------------------------------------------------------------*/
/* mc32_packRegisters - does some transformations to reduce register    */
/*                   pressure                                      */
/*-----------------------------------------------------------------*/
static void
mc32_packRegisters(eBBlock *ebp)
{
        iCode *ic;
        int change = 0;
        ...
        while (1)
        {
                ...
                for (ic = ebp->sch; ic; ic = ic->next)
                {
                        ...
                        if (ic->op == '=' && !POINTER_SET(ic))
                                change += mc32_packRegsForAssign(ic, ebp);
                        /* mc32_debug stuff */
                        if (ic->op == '=')
                        {
                                if (POINTER_SET(ic))
                                        mc32_debugLog("pointer is set\n");
                                mc32_debugAopGet("  result:", IC_RESULT(ic));
                                mc32_debugAopGet("  left:", IC_LEFT(ic));
                                mc32_debugAopGet("  right:", IC_RIGHT(ic));
                        }
        ...
        for (ic = ebp->sch; ic; ic = ic->next)
        {
                ...
                if (IS_SYMOP(IC_LEFT(ic)))
                {
                        ...
                        if (IS_PTR_CONST(OP_SYMBOL(IC_LEFT(ic))->type))
                                mc32_debugLog("    is a pointer\n");

                        if (IS_OP_VOLATILE(IC_LEFT(ic)))
                                mc32_debugLog("    is volatile\n");

                        mc32_isData(etype);

                        mc32_printSymType("   ", OP_SYMBOL(IC_LEFT(ic))->type);
                }

                if (IS_SYMOP(IC_RIGHT(ic)))
                {
                        mc32_debugAopGet("  right:", IC_RIGHT(ic));
                        mc32_printSymType("    ", OP_SYMBOL(IC_RIGHT(ic))->type);
                }

                if (IS_SYMOP(IC_RESULT(ic)))
                {
                        mc32_debugAopGet("  result:", IC_RESULT(ic));
                        mc32_printSymType("     ", OP_SYMBOL(IC_RESULT(ic))->type);
                }

                if (POINTER_SET(ic))
                        mc32_debugLog("  %d - Pointer set\n", __LINE__);
                    ...
                    !OP_SYMBOL(IC_LEFT(ic))->onStack)
                {
                        ...
                        mc32_debugLog("  %d - %s. result is rematerializable\n", __LINE__, __FUNCTION__);
                ...
                {
                        mc32_debugLog("  %d - %s. straight rematerializable\n", __LINE__, __FUNCTION__);
                ...
                {
                        mc32_debugLog("  %d - %s. rematerializable because op is +/-\n", __LINE__, __FUNCTION__);
                        //int i =
                ...
                if (POINTER_SET(ic) && IS_SYMOP(IC_RESULT(ic)))
                {
                        ...
                        mc32_debugLog("  marking as a pointer (set) =>");
                        mc32_debugAopGet("  result:", IC_RESULT(ic));
                }
                if (POINTER_GET(ic) && IS_SYMOP(IC_LEFT(ic)))
                {
                        OP_SYMBOL(IC_LEFT(ic))->uptr = 1;
                        mc32_debugLog("  marking as a pointer (get) =>");
                        mc32_debugAopGet("  left:", IC_LEFT(ic));
                }
                ...
                if ((ic->op == RETURN || ic->op == SEND) &&
                    ...
                    !options.model)
                        mc32_packRegsForOneuse(ic, IC_LEFT(ic), ebp);
                ...
                if (POINTER_SET(ic) &&
                    ...
                    getSize(aggrToPtr(operandType(IC_RESULT(ic)), FALSE)) > 1)
                        ...
                        mc32_packRegsForOneuse(ic, IC_RESULT(ic), ebp);
                ...
                if (ic->op == CAST)
                {
                        ...
                        mc32_debugLog("  %d - casting\n", __LINE__);

                        if (IS_INTEGRAL(fromType) && IS_INTEGRAL(toType) &&
                            getSize(fromType) != getSize(toType))
                        {

                                iCode *dic = mc32_packRegsForOneuse(ic, IC_RIGHT(ic), ebp);
                                if (dic)
                                {
                ...
                */
                ...
                if ((IS_ARITHMETIC_OP(ic)
                    ...
                    getSize(operandType(IC_RESULT(ic))) <= 2)
                        ...
                        mc32_packRegsForAccUse(ic);
        }
}

static void
mc32_dumpEbbsToDebug(eBBlock **ebbs, int count)
{
        int i;

        if (!mc32_debug || !mc32_debugF)
                return;

        for (i = 0; i < count; i++)
        {
                fprintf(mc32_debugF, "\n----------------------------------------------------------------\n");
                fprintf(mc32_debugF, "Basic Block %s : loop Depth = %d noPath = %d , lastinLoop = %d\n",
                        ebbs[i]->entryLabel->name,
                        ebbs[i]->depth,
                        ebbs[i]->noPath,
                        ebbs[i]->isLastInLoop);
                fprintf(mc32_debugF, "depth 1st num %d : bbnum = %d 1st iCode = %d , last iCode = %d\n",
                        ebbs[i]->dfnum,
                        ebbs[i]->bbnum,
                        ebbs[i]->fSeq,
                        ebbs[i]->lSeq);
                fprintf(mc32_debugF, "visited %d : hasFcall = %d\n",
                        ebbs[i]->visited,
                        ebbs[i]->hasFcall);

                fprintf(mc32_debugF, "\ndefines bitVector :");
                bitVectDebugOn(ebbs[i]->defSet, mc32_debugF);
                fprintf(mc32_debugF, "\nlocal defines bitVector :");
                bitVectDebugOn(ebbs[i]->ldefs, mc32_debugF);
                fprintf(mc32_debugF, "\npointers Set bitvector :");
                bitVectDebugOn(ebbs[i]->ptrsSet, mc32_debugF);
                fprintf(mc32_debugF, "\nin pointers Set bitvector :");
                bitVectDebugOn(ebbs[i]->inPtrsSet, mc32_debugF);
                fprintf(mc32_debugF, "\ninDefs Set bitvector :");
                bitVectDebugOn(ebbs[i]->inDefs, mc32_debugF);
                fprintf(mc32_debugF, "\noutDefs Set bitvector :");
                bitVectDebugOn(ebbs[i]->outDefs, mc32_debugF);
                fprintf(mc32_debugF, "\nusesDefs Set bitvector :");
                bitVectDebugOn(ebbs[i]->usesDefs, mc32_debugF);
                fprintf(mc32_debugF, "\n----------------------------------------------------------------\n");
                printiCChain(ebbs[i]->sch, mc32_debugF);
        }
}
...
/*-----------------------------------------------------------------*/
void mc32_assignRegisters(ebbIndex *ebbi)
{
        int i;
        iCode *ic;
        ...
        mc32_ptrRegReq = _G.stackExtend = _G.dataExtend = 0;
   ...
   * disjoint sets of registers are allocated to functions */
        if (1)
        {
                ...
                for (r = setFirstItem(mc32_dynAllocRegs);
                     r;
     ...
     live ranges reducing some register pressure */
        ...
        if (1)
        {
                ...
                mc32_debugLog("dir registers allocated so far:\n");
                reg = hTabFirstItem(mc32_dynDirectRegNames, &hkey);
     ...
     registers & the type of registers required for each */
        ...
        mc32_serialRegAssign(ebbs, count);
        ...
        mc32_debugLog("ebbs after optimizing:\n");
        mc32_dumpEbbsToDebug(ebbs, count);

        mc32_genpic14Code(ic);
