...
static pCodePeepSnippets *mc32_peepSnippets = NULL;
...
static pCodeToken mc32_tokArr[50];
static unsigned mc32_tokIdx = 0;
...
static parsedPattern mc32_parsedPatArr[MAX_PARSEDPATARR];
...
static char mc32_pcpat_label[] = {PCT_PERCENT, PCT_NUMBER, PCT_COLON, 0};
static char mc32_pcpat_number[] = {PCT_NUMBER, 0};
static char mc32_pcpat_string[] = {PCT_STRING, 0};
static char mc32_pcpat_wildString[] = {PCT_PERCENT, PCT_STRING, 0};
static char mc32_pcpat_wildVar[] = {PCT_PERCENT, PCT_NUMBER, 0};
static char mc32_pcpat_comma[] = {PCT_COMMA, 0};
static char mc32_pcpat_comment[] = {PCT_COMMENT, 0};
...
static pcPattern mc32_pcpArr[] = {
    {PCP_LABEL, mc32_pcpat_label, NULL},
    {PCP_WILDSTR, mc32_pcpat_wildString, NULL},
    {PCP_STR, mc32_pcpat_string, NULL},
    {PCP_WILDVAR, mc32_pcpat_wildVar, NULL},
    {PCP_COMMA, mc32_pcpat_comma, NULL},
    {PCP_COMMENT, mc32_pcpat_comment, NULL},
    {PCP_NUMBER, mc32_pcpat_number, NULL}};

#define PCPATTERNS (sizeof(mc32_pcpArr) / sizeof(pcPattern))
...
static char mc32_alt_comment[] = {PCP_COMMENT, 0};
static char mc32_alt_label[] = {PCP_LABEL, 0};
static char mc32_alt_mnem0[] = {PCP_STR, 0};
static char mc32_alt_mnem0a[] = {PCP_WILDVAR, 0};
static char mc32_alt_mnem1[] = {PCP_STR, PCP_STR, 0};
static char mc32_alt_mnem1a[] = {PCP_STR, PCP_WILDVAR, 0};
static char mc32_alt_mnem1b[] = {PCP_STR, PCP_NUMBER, 0};
static char mc32_alt_mnem2[] = {PCP_STR, PCP_STR, PCP_COMMA, PCP_STR, 0};
static char mc32_alt_mnem2a[] = {PCP_STR, PCP_WILDVAR, PCP_COMMA, PCP_STR, 0};
static char mc32_alt_mnem3[] = {PCP_STR, PCP_STR, PCP_COMMA, PCP_NUMBER, 0};

static void *mc32_cvt_altpat_label(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_comment(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_mnem0(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_mnem0a(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_mnem1(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_mnem1a(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_mnem1b(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_mnem2(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_mnem2a(void *pp, pCodeWildBlock *pcwb);
static void *mc32_cvt_altpat_mnem3(void *pp, pCodeWildBlock *pcwb);

static pcPattern mc32_altArr[] = {
    {ALT_LABEL, mc32_alt_label, mc32_cvt_altpat_label},
    {ALT_COMMENT, mc32_alt_comment, mc32_cvt_altpat_comment},
    {ALT_MNEM3, mc32_alt_mnem3, mc32_cvt_altpat_mnem3},
    {ALT_MNEM2A, mc32_alt_mnem2a, mc32_cvt_altpat_mnem2a},
    {ALT_MNEM2, mc32_alt_mnem2, mc32_cvt_altpat_mnem2},
    {ALT_MNEM1B, mc32_alt_mnem1b, mc32_cvt_altpat_mnem1b},
    {ALT_MNEM1A, mc32_alt_mnem1a, mc32_cvt_altpat_mnem1a},
    {ALT_MNEM1, mc32_alt_mnem1, mc32_cvt_altpat_mnem1},
    {ALT_MNEM0A, mc32_alt_mnem0a, mc32_cvt_altpat_mnem0a},
    {ALT_MNEM0, mc32_alt_mnem0, mc32_cvt_altpat_mnem0},

};

#define ALTPATTERNS (sizeof(mc32_altArr) / sizeof(pcPattern))

// forward declarations
static void *mc32_DLL_append(DLList *list, DLList *next);

/*-----------------------------------------------------------------*/
/* mc32_cvt_extract_destination - helper function extracts the register */
/*                           destination from a parsedPattern.     */
/*                                                                 */
/*-----------------------------------------------------------------*/
static int mc32_cvt_extract_destination(parsedPattern *pp)
{
...
/*-----------------------------------------------------------------*/
/*  pCodeOp *mc32_cvt_extract_status(char *reg, char *bit)              */
...
static pCodeOp *mc32_cvt_extract_status(char *reg, char *bit)
{
  int len;

  if (STRCASECMP(reg, mc32_pc_status.pcop.name))
    return NULL;
  ...
  if (len == 1)
  {
    ...
    if (toupper((unsigned char)*bit) == 'C')
      return PCOP(mc32_popCopyGPR2Bit(PCOP(&mc32_pc_status), PIC_C_BIT));
    if (toupper((unsigned char)*bit) == 'Z')
      return PCOP(mc32_popCopyGPR2Bit(PCOP(&mc32_pc_status), PIC_Z_BIT));
  }

  // Check DC
  if (len == 2 && toupper((unsigned char)bit[0]) == 'D' && toupper((unsigned char)bit[1]) == 'C')
    return PCOP(mc32_popCopyGPR2Bit(PCOP(&mc32_pc_status), PIC_DC_BIT));

  return NULL;
}

/*-----------------------------------------------------------------*/
/* mc32_cvt_altpat_label - convert assembly line type to a pCode label  */
...
static void *mc32_cvt_altpat_label(void *pp, pCodeWildBlock *pcwb)
{
  ...
  return mc32_newpCodeLabel(NULL, -p->pct[1].tok.n);
}

/*-----------------------------------------------------------------*/
/* mc32_cvt_altpat_comment - convert assembly line type to a comment    */
/* INPUT: pointer to the parsedPattern                             */
/*                                                                 */
/*  pp[0] - comment                                                */
/*                                                                 */
...
static void *mc32_cvt_altpat_comment(void *pp, pCodeWildBlock *pcwb)
{
  ...
  return mc32_newpCodeCharP(p->pct[0].tok.s);
}

/*-----------------------------------------------------------------*/
/* mc32_cvt_altpat_mem0  - convert assembly line type to a wild pCode   */
/*                    instruction                                  */
/*                                                                 */
/*  pp[0] - str                                                    */
/*                                                                 */
...
static void *mc32_cvt_altpat_mnem0(void *pp, pCodeWildBlock *pcwb)
{
  ...
  opcode = mc32_getpCode(p->pct[0].tok.s, 0);
  ...
  if (opcode < 0)
  {
    ...
    opcode = mc32_getpCodePeepCommand(p->pct[0].tok.s);
    //if(opcode > 0)
    //  fprintf(stderr,"  but valid peep command: %s, key = %d\n",p->pct[0].tok.s,opcode);
    return NULL;
  }

  pci = PCI(mc32_newpCode(opcode, NULL));
...
/*-----------------------------------------------------------------*/
/* mc32_cvt_altpat_mem0a - convert assembly line type to a wild pCode   */
/*                    instruction                                  */
/*                                                                 */
/*  pp[0] - wild var                                               */
/*                                                                 */
...
static void *mc32_cvt_altpat_mnem0a(void *pp, pCodeWildBlock *pcwb)
{
  ...
  return mc32_newpCodeWild(p[0].pct[1].tok.n, NULL, NULL);
}

/*-----------------------------------------------------------------*/
/* mc32_cvt_altpat_mem1 - convert assembly line type to a pCode         */
...
static void *mc32_cvt_altpat_mnem1(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if (mc32_pic14Mnemonics[opcode]->isBitInst)
    pcosubtype = mc32_newpCodeOp(p[1].pct[0].tok.s, PO_BIT);
  else
    pcosubtype = mc32_newpCodeOp(p[1].pct[0].tok.s, PO_GPR_REGISTER);

  pci = PCI(mc32_newpCode(opcode, pcosubtype));
...
/*-----------------------------------------------------------------*/
/* mc32_cvt_altpat_mem1a - convert assembly line type to a pCode        */
...
static void *mc32_cvt_altpat_mnem1a(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if (opcode < 0)
  {
    int cmd_id = mc32_getpCodePeepCommand(p->pct[0].tok.s);
    pCode *pc = NULL;
    ...
    pc = mc32_newpCodeWild(p[1].pct[1].tok.n, NULL, NULL);
  ...
  if (mc32_pic14Mnemonics[opcode]->isBitInst)
    pcosubtype = mc32_newpCodeOpBit(NULL, -1, 0);
  else
    pcosubtype = mc32_newpCodeOp(NULL, PO_GPR_REGISTER);

  pci = PCI(mc32_newpCode(opcode,
                     mc32_newpCodeOpWild(p[1].pct[1].tok.n, pcwb, pcosubtype)));
...
static void *mc32_cvt_altpat_mnem1b(void *pp, pCodeWildBlock *pcwb)
{
  ...
  pci = PCI(mc32_newpCode(opcode, mc32_newpCodeOpLit(p[1].pct[0].tok.n)));
...
static void *mc32_cvt_altpat_mnem2(void *pp, pCodeWildBlock *pcwb)
{
  ...
  dest = mc32_cvt_extract_destination(&p[3]);
  ...
  opcode = mc32_getpCode(p->pct[0].tok.s, dest);
  if (opcode < 0)
  {
    fprintf(stderr, "Bad mnemonic\n");
    return NULL;
  ...
  if (mc32_pic14Mnemonics[opcode]->isBitInst)
  {
    pcosubtype = mc32_cvt_extract_status(p[1].pct[0].tok.s, p[3].pct[0].tok.s);
    if (pcosubtype == NULL)
    {
...
/*-----------------------------------------------------------------*/
/* mc32_cvt_altpat_mem2a - convert assembly line type to a pCode        */
...
static void *mc32_cvt_altpat_mnem2a(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if (mc32_pic14Mnemonics[opcode]->isBitInst)
    pcosubtype = mc32_newpCodeOp(NULL, PO_BIT);
  else
    pcosubtype = mc32_newpCodeOp(NULL, PO_GPR_REGISTER);
...
/*-----------------------------------------------------------------*/
/* mc32_cvt_altpat_mem3 -  convert assembly line type to a pCode        */
...
static void *mc32_cvt_altpat_mnem3(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if (mc32_pic14Mnemonics[opcode]->isBitInst)
  {
    ...
    //if(pcosubtype == NULL) {
    pcosubtype = mc32_newpCodeOpBit(p[1].pct[0].tok.s, p[3].pct[0].tok.n, 0);
    //}
  }
  else
    pcosubtype = mc32_newpCodeOp(p[1].pct[0].tok.s, PO_GPR_REGISTER);
...
/*-----------------------------------------------------------------*/
/* mc32_tokenizeLineNode - Convert a string (of char's) that was parsed */
...
static void mc32_tokenizeLineNode(char *ln)
{
  char *lnstart = ln;
  mc32_tokIdx = 0;              // Starting off at the beginning
  mc32_tokArr[0].tt = PCT_NULL; // and assume invalid character for first token.
  ...
  while (*ln)
  {
    ...
    if (isspace((unsigned char)*ln))
    {
      ...
      mc32_tokArr[mc32_tokIdx++].tt = PCT_SPACE;
    ...
    if (isdigit((unsigned char)*ln))
    {
      ...
      mc32_tokArr[mc32_tokIdx].tt = PCT_NUMBER;
      mc32_tokArr[mc32_tokIdx++].tok.n = strtol(ln, &ln, 0);
    ...
    {
    case '%':
      mc32_tokArr[mc32_tokIdx++].tt = PCT_PERCENT;
      break;
    case '<':
      mc32_tokArr[mc32_tokIdx++].tt = PCT_LESSTHAN;
      break;
    case '>':
      mc32_tokArr[mc32_tokIdx++].tt = PCT_GREATERTHAN;
      break;
    case ':':
      mc32_tokArr[mc32_tokIdx++].tt = PCT_COLON;
      break;
    case ';':
      mc32_tokArr[mc32_tokIdx].tok.s = Safe_strdup(ln);
      mc32_tokArr[mc32_tokIdx++].tt = PCT_COMMENT;
      mc32_tokArr[mc32_tokIdx].tt = PCT_NULL;
      return;
    case ',':
      mc32_tokArr[mc32_tokIdx++].tt = PCT_COMMA;
      break;
    ...
    default:
      if (isalpha((unsigned char)*ln) || (*ln == '_'))
      {
        ...
        mc32_tokArr[mc32_tokIdx].tok.s = Safe_strdup(buffer);
        mc32_tokArr[mc32_tokIdx++].tt = PCT_STRING;
      }
      else
      {
  ...
  mc32_tokArr[mc32_tokIdx].tt = 0;
}

/*-----------------------------------------------------------------*/
/*-----------------------------------------------------------------*/

static void mc32_dump1Token(pCodeTokens tt)
{
  ...
  case PCT_COMMENT:
    ...
    //fprintf(stderr,"%s",mc32_tokArr[i].tok.s);
    break;
  case PCT_STRING:
    fprintf(stderr, " str ");
    //fprintf(stderr,"%s",mc32_tokArr[i].tok.s);
  ...
  case PCT_NUMBER:
    fprintf(stderr, " num ");
    //fprintf(stderr,"%d",mc32_tokArr[i].tok.n);
    break;
  case PCT_NULL:
    fprintf(stderr, " null ");
  }
...
static int mc32_pcComparePattern(pCodeToken *pct, char *pat, int max_tokens)
{
  int i = 0;
  ...
  while (i < max_tokens)
  {
    ...
    //mc32_dump1Token(*pat); DFPRINTF((stderr,"\n"));
...
static int mc32_altComparePattern(char *pct, parsedPattern *pat, int max_tokens)
{
  int i = 0;
...
static int mc32_advTokIdx(int *v, int amt)
{

  if ((unsigned)(*v + amt) > mc32_tokIdx)
    return 1;
...
/*-----------------------------------------------------------------*/
/* mc32_parseTokens - convert the tokens corresponding to a single line */
...
static int mc32_parseTokens(pCodeWildBlock *pcwb, pCode **pcret)
{
  pCode *pc;
  int error = 0;
  ...
  {
    unsigned i;
    for (i = 0; i <= mc32_tokIdx; i++)
      mc32_dump1Token(mc32_tokArr[i].tt);
    fputc('\n', stderr);
  }
#endif

  {
    ...
    do
    {
      ...
      if (((mc32_tokArr[mc32_ltokIdx].tt == PCT_SPACE)) && (mc32_advTokIdx(&mc32_ltokIdx, 1))) // eat space
        break;

      do
      {
        j = mc32_pcComparePattern(&mc32_tokArr[mc32_ltokIdx], mc32_pcpArr[lpcpIdx].tokens, mc32_tokIdx + 1);
        if (j)
        {

          switch (mc32_pcpArr[lpcpIdx].pt)
          {
          case PCP_LABEL:
            if (state == PS_START)
            {
          ...
          case PCP_STR:
            DFPRINTF((stderr, "  %s is", mc32_tokArr[mc32_ltokIdx].tok.s));
            switch (state)
            {
            case PS_START:
            case PS_HAVE_LABEL:
            ...
            case PS_HAVE_MNEM:
              ...
              //pco1 = mc32_newpCodeOp(NULL,PO_GPR_REGISTER);
          ...
          matching = 1;
          mc32_parsedPatArr[lparsedPatIdx].pcp = &mc32_pcpArr[lpcpIdx];
          mc32_parsedPatArr[lparsedPatIdx].pct = &mc32_tokArr[mc32_ltokIdx];
          lparsedPatIdx++;

          //mc32_dump1Token(mc32_tokArr[mc32_ltokIdx].tt);

          if (mc32_advTokIdx(&mc32_ltokIdx, strlen(mc32_pcpArr[lpcpIdx].tokens)))
          {
    ...
    mc32_parsedPatArr[lparsedPatIdx].pcp = NULL;
    mc32_parsedPatArr[lparsedPatIdx].pct = NULL;
    ...
    do
    {
      ...
      if ((c = mc32_altComparePattern(mc32_altArr[k].tokens, &mc32_parsedPatArr[j], 10)))
      {

        if (mc32_altArr[k].f)
        {
          pc = mc32_altArr[k].f(&mc32_parsedPatArr[j], pcwb);
          //if(pc && pc->print)
          //  pc->print(stderr,pc);
          ...
          if (pc)
          {
            if (pcret)
            {
              *pcret = pc;
              ...
              mc32_addpCode2pBlock(pcwb->pb, pc);
          }
          else
            error++;
        }
  ...
  DFPRINTF((stderr,"\nConverting parsed line to pCode:\n\n"));
    ...
    j = 0;
    do {
    if(mc32_parsedPatArr[j].pcp && mc32_parsedPatArr[j].pcp->f )
    mc32_parsedPatArr[j].pcp->f(&mc32_parsedPatArr[j]);
    DFPRINTF((stderr,"  %d",mc32_parsedPatArr[j].pcp->pt));
    j++;
    }
...
/*                                                                 */
/*-----------------------------------------------------------------*/
static void mc32_peepRuleBlock2pCodeBlock(lineNode *ln, pCodeWildBlock *pcwb)
{
  ...
  for (; ln; ln = ln->next)
  {
    ...
    mc32_tokenizeLineNode(ln->line);

    if (mc32_parseTokens(pcwb, NULL))
    {
      int i;
...
/*-----------------------------------------------------------------*/
/* mc32_peepRuleCondition                                               */
/*-----------------------------------------------------------------*/
static void mc32_peepRuleCondition(char *cond, pCodePeep *pcp)
{
  if (!cond || !pcp)
    return;
...
static void mc32_initpCodeWildBlock(pCodeWildBlock *pcwb)
{
...
static void mc32_postinit_pCodeWildBlock(pCodeWildBlock *pcwb)
{
...
static void mc32_initpCodePeep(pCodePeep *pcp)
{
  ...
  mc32_initpCodeWildBlock(&pcp->target);
  pcp->target.pb = mc32_newpCodeChain(NULL, 'W', NULL);

  mc32_initpCodeWildBlock(&pcp->replace);
  pcp->replace.pb = mc32_newpCodeChain(NULL, 'W', NULL);
}

/*-----------------------------------------------------------------*/
/* mc32_peepRules2pCode - parse the "parsed" peep hole rules to generate*/
/*                   pCode.                                        */
/*                                                                 */
...
/*                                                                 */
/*-----------------------------------------------------------------*/
void mc32_peepRules2pCode(peepRule *rules)
{
  peepRule *pr;
  ...
  for (pr = rules; pr; pr = pr->next)
  {
    ...
    mc32_peepRuleBlock2pCodeBlock(pr->match, &currentRule->target);

    //DFPRINTF((stderr,"finished target, here it is in pcode form:\n"));
    //mc32_printpBlock(stderr, currentRule->target.pb);

    mc32_pBlockMergeLabels(currentRule->target.pb);
    //mc32_printpBlock(stderr, currentRule->replace.pb);

    /* Convert the replace block */
    mc32_peepRuleBlock2pCodeBlock(pr->replace, &currentRule->replace);

    //DFPRINTF((stderr,"replace with labels merged:\n"));

    mc32_pBlockMergeLabels(currentRule->replace.pb);
    //mc32_printpBlock(stderr, currentRule->replace.pb);

    mc32_peepRuleCondition(pr->cond, currentRule);

    /* The rule has been converted to pCode. Now allocate
    * space for the wildcards */

    mc32_postinit_pCodeWildBlock(&currentRule->target);
    mc32_postinit_pCodeWildBlock(&currentRule->replace);
  ...
  if (0)
  {
    ...
    peeprules = (DLList *)mc32_peepSnippets;
    fprintf(stderr, "target rules\n");
    while (peeprules)
    {
      fprintf(stderr, "   rule:\n");
      peepBlock = ((pCodePeepSnippets *)peeprules)->peep;
      mc32_printpBlock(stderr, peepBlock->target.pb);
...
/*-----------------------------------------------------------------*/
/* mc32_pCodeSearchCondition - Search a pCode chain for a 'condition'   */
/*                                                                 */
/* return conditions                                               */
    ...
    if (pc->type == PC_OPCODE)
    {
      if (PCI(pc)->inCond & cond)
      {
        if (contIfSkip)
        {
          ...
          pCode *pcp = mc32_findPrevInstruction(pc->prev);
...
/*-----------------------------------------------------------------
* int mc32_pCodeOpCompare(pCodeOp *pcops, pCodeOp *pcopd)
*
* Compare two pCodeOp's and return 1 if they're the same
*-----------------------------------------------------------------*/
static int mc32_pCodeOpCompare(pCodeOp *pcops, pCodeOp *pcopd)
{
  char b[50], *n2;
  ...
  /*
    ...
    mc32_get_op( pcops,NULL,0));

      fprintf(stderr," to %s\n",
      mc32_get_op( pcopd,NULL,0));
  */
  ...
  b[0] = 0;
  mc32_get_op(pcops, b, 50);

  n2 = mc32_get_op(pcopd, NULL, 0);
...
static int mc32_pCodePeepMatchLabels(pCodePeep *peepBlock, pCode *pcs, pCode *pcd)
{
  int labindex;
...
/*-----------------------------------------------------------------*/
/* mc32_pCodePeepMatchLine - Compare source and destination pCodes to   */
...
static int mc32_pCodePeepMatchLine(pCodePeep *peepBlock, pCode *pcs, pCode *pcd)
{
  ...
  * are not wild. However, there may be a label or a wild operand */
  ...
  if (pcs->type == pcd->type)
  {
    ...
    if (pcs->type == PC_OPCODE)
    {
      ...
      if (!mc32_pCodePeepMatchLabels(peepBlock, pcs, pcd))
        return 0;
...
//DFPRINTF((stderr,"destination is wild\n"));
          ...
          mc32_pCodeOpCompare(PCI(pcs)->pcop, peepBlock->target.wildpCodeOps[index])
          );
          */

            return mc32_pCodeOpCompare(PCI(pcs)->pcop, peepBlock->target.wildpCodeOps[index]);
          }
          /*
          {
            char *n;
        ...
        mc32_pCodeOpCompare(PCI(pcs)->pcop, PCI(pcd)->pcop));
        */
          return mc32_pCodeOpCompare(PCI(pcs)->pcop, PCI(pcd)->pcop);
        }
        else
        {
  ...
  if ((pcd->type == PC_WILD) && (pcs->type == PC_OPCODE))
  {
    ...
    pcs = mc32_findNextInstruction(pcs->next);
    if (pcs)
    {
...
/*-----------------------------------------------------------------*/
/*-----------------------------------------------------------------*/
static void mc32_pCodePeepClrVars(pCodePeep *pcp)
{
...
/*-----------------------------------------------------------------*/
/*-----------------------------------------------------------------*/
int mc32_pCodePeepMatchRule(pCode *pc)
{
  pCodePeep *peepBlock;
  pCode *pct, *pcin;
  ...
  while (peeprules)
  {
    ...
    mc32_pCodePeepClrVars(peepBlock);
    /*
    pcin = pc;
    if(IS_PCCOMMENT(pcin))
    pc = pcin = mc32_findNextInstruction(pcin->next);
    */
    pcin = pc = mc32_findNextInstruction(pc);
    ...
    while (pct && pcin)
    {
      ...
      if (!(matched = mc32_pCodePeepMatchLine(peepBlock, pcin, pct)))
        break;

      pcin = mc32_findNextInstruction(pcin->next);
      pct = pct->next;
      //debug:
      //DFPRINTF((stderr,"    matched\n"));
    ...
    */
      ...
      if (pcin && peepBlock->postFalseCond &&
          (mc32_pCodeSearchCondition(pcin, peepBlock->postFalseCond, 0) > 0))
        matched = 0;

      //fprintf(stderr," condition results = %d\n",mc32_pCodeSearchCondition(pcin,peepBlock->postFalseCond));
...
/* We matched a rule! Now we have to go through and remove the
...
#if 0
      {
        ...
        while(pc &&  pc!=pcin) {
          ...
          mc32_pCode2str(&buf[2], 254, pc);
          mc32_pCodeInsertAfter(pcprev, mc32_newpCodeCharP(buf));
          pcprev = pcprev->next;
          pc = pc->next;
          ...
          if (PCI(pcr)->pcop)
          {
            ...
            * Is it wild? */
            if (PCI(pcr)->pcop->type == PO_WILD)
            {
              ...
              if (peepBlock->target.wildpCodeOps[index])
                pcop = mc32_pCodeOpCopy(peepBlock->target.wildpCodeOps[index]);
              else
                DFPRINTF((stderr, "error, wildopcode in replace but not source?\n"));
            }
            else
              pcop = mc32_pCodeOpCopy(PCI(pcr)->pcop);
          }
          //DFPRINTF((stderr,"inserting pCode\n"));
          mc32_pCodeInsertAfter(pc, mc32_newpCode(PCI(pcr)->op, pcop));
        }
        else if (pcr->type == PC_WILD)
        {
          if (PCW(pcr)->invertBitSkipInst)
            DFPRINTF((stderr, "We need to invert the bit skip instruction\n"));
          mc32_pCodeInsertAfter(pc,
                           mc32_pCodeInstructionCopy(PCI(peepBlock->target.wildpCodes[PCW(pcr)->id]),
                                                PCW(pcr)->invertBitSkipInst));
        }
        else if (pcr->type == PC_COMMENT)
        {
          mc32_pCodeInsertAfter(pc, mc32_newpCodeCharP(((pCodeComment *)(pcr))->comment));
        }
