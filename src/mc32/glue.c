...
set *mc32_localFunctions = NULL;
int mc32_hasInterrupt = 0; // Indicates whether to emit interrupt handler or not

int mc32_stringInSet(const char *str, set **world, int autoAdd);
...
static set *mc32_emitted = NULL;

static void mc32_showAllMemmaps(FILE *of); // XXX: emits initialized symbols

static void
mc32_emitPseudoStack(struct dbuf_s *oBuf, struct dbuf_s *oBufExt)
{
    ...
    shared = mc32_getSharedStack(&low, &high, &size);
    if (!mc32_options.isLibrarySource)
    {
        dbuf_printf(oBuf, "\n");
        // zwr 1.0.0
    ...
    else
    {
        ...
        for (i = 6; i >= 0; i--)
        {
            ...
            mc32_stringInSet(&buffer[0], &mc32_emitted, 1);
        } // for i
    }
    dbuf_printf(oBuf, "\n");
}
...
static int
mc32_emitIfNew(struct dbuf_s *oBuf, set **mc32_emitted, const char *fmt,
          const char *name)
{
    int wasPresent = mc32_stringInSet(name, mc32_emitted, 1);
...
static void
mc32_constructAbsMap(struct dbuf_s *oBuf, struct dbuf_s *gloBuf)
{
    ...
    for (i = 0; maps[i] != NULL; i++)
    {
             ...
             sym; sym = setNextItem(maps[i]->syms))
        {
            if (IS_DEFINED_HERE(sym) && SPEC_ABSA(sym->etype))
            {
                ...
                if (MC32_IS_CONFIG_ADDRESS(addr))
                {
                    //fprintf( stderr, "%s: assignment to CONFIG@0x%x found\n", __FUNCTION__, addr );
                    //fprintf( stderr, "ival: %p (0x%x)\n", sym->ival, (int)list2int( sym->ival ) );
                    if (sym->ival)
                    {
                        mc32_assignConfigWordValue(addr, (int)list2int(sym->ival));
                    }
                    else
                    {
    ...
    for (addr = min; addr <= max; addr++)
    {
        ...
        if (aliases && elementsInSet(aliases))
        {
                 ...
                 sym = setNextItem(aliases))
            {
                ...
                {
                    //mc32_emitIfNew(gloBuf, &mc32_emitted, "\tglobal\t%s\n", sym->name);
                    mc32_emitIfNew(gloBuf, &mc32_emitted, "\tglobal\t%s\n", sym->rname);
                } // if
            }     // for
...
/*-----------------------------------------------------------------*/
static void
mc32_createInterruptVect(struct dbuf_s *vBuf)
{
...
/*-----------------------------------------------------------------*/
static void
mc32_initialComments(FILE *afile)
{
    initialComments(afile);
    fprintf(afile, "; MC32 port for the RISC core\n"); // zwr 1.1.0
...
static void
mc32_printLocals(struct dbuf_s *oBuf)
{
...
/*-----------------------------------------------------------------*/
static void
mc32_emitOverlay(struct dbuf_s *aBuf)
{
    set *ovrset;
         ...
         ovrset = setNextItem(ovrSetSets))
    {
             ...
             sym = setNextItem(ovrset))
        {
            ...
            if (!IS_AGGREGATE(sym->type) &&
                !(sym->_isparm && !IS_REGPARM(sym->etype)) && !sym->allocreq && sym->level)
                continue;
...
static void
mc32_emitInterruptHandler(FILE *asmFile)
{
    if (mc32_hasInterrupt)
    {
...
void mc32_picglue()
{
    ...
    mc32_movepBlock2Head('P'); // Last
    mc32_movepBlock2Head(code->dbName);
    mc32_movepBlock2Head('X');
    mc32_movepBlock2Head(statsg->dbName); // First
    ...
    mc32_emitOverlay(&ovrBuf);

    /* PENDING: this isnt the best place but it will do */
    if (port->general.glue_up_main)
    {
        /* create the interrupt vector table */
        mc32_createInterruptVect(&vBuf);
    }

    mc32_AnalyzepCode('*');

    mc32_ReuseReg(); // mc32_ReuseReg where call tree permits
    ...
    /* prepare statistics */
    mc32_resetpCodeStatistics();

    /* initial comments */
    mc32_initialComments(asmFile);
    ...
    mc32_emitInterruptHandler(asmFile);
    ...
    /* unknown */
    mc32_copypCode(asmFile, 'X');

    /* _main function */
    mc32_copypCode(asmFile, 'M');

    /* other functions */
    mc32_copypCode(asmFile, code->dbName);

    /* unknown */
    mc32_copypCode(asmFile, 'P');

    mc32_dumppCodeStatistics(asmFile);
...
static char *
mc32_parseIvalAst(ast *node, int *inCodeSpace)
{
#define LEN 4096
    char *buffer = NULL;
    char *left, *right;
    ...
    else if (IS_AST_OP(node))
    {
        ...
        case CAST:
            assert(node->right);
            buffer = mc32_parseIvalAst(node->right, inCodeSpace);
            DEBUGprintf("%s: %s\n", __FUNCTION__, buffer);
            break;
        case '&':
            assert(node->left && !node->right);
            buffer = mc32_parseIvalAst(node->left, inCodeSpace);
            DEBUGprintf("%s: %s\n", __FUNCTION__, buffer);
            break;
        case '+':
            assert(node->left && node->right);
            left = mc32_parseIvalAst(node->left, inCodeSpace);
            right = mc32_parseIvalAst(node->right, inCodeSpace);
            buffer = Safe_alloc(LEN);
 ...
 */
static void
mc32_emitIvals(struct dbuf_s *oBuf, symbol *sym, initList *list, long lit, int size)
{
    int i;
    ast *node;
    operand *op;
    ...
    in_code = mc32_emitIvalLabel(oBuf, sym);
    if (!in_code)
        dbuf_printf(oBuf, "\tdb\t");
    ...
    {
        str = mc32_parseIvalAst(node, &inCodeSpace);
    ...
    for (i = 0; i < size; i++)
    {
        ...
        if (op && IS_SYMOP(op) && IS_FUNC(OP_SYM_TYPE(op)))
        {
            ...
            text = mc32_get_op(AOP(op)->aopu.pcop, NULL, 0);
            PCOI(AOP(op)->aopu.pcop)->offset -= i;
        }
        else
        {
            text = op ? mc32_aopGet(AOP(op), i, 0, 0)
                      : mc32_get_op(mc32_newpCodeOpImmd(str, i, 0, inCodeSpace, 0), NULL, 0);
        } // if
        if (in_code)
        {
            // zwr 1.1.0
 ...
 */
static sym_link *
mc32_matchIvalToUnion(initList *list, sym_link *type, int size)
{
    symbol *sym;

    assert(type);

    if (IS_PTR(type) || IS_CHAR(type) || IS_INT(type) || IS_LONG(type) || IS_FLOAT(type))
    {
    ...
    else if (IS_STRUCT(type) && SPEC_STRUCT(getSpec(type))->type == STRUCT)
    {
        if (!list || (list->type == INIT_DEEP))
        {
            ...
            while (sym)
            {
                ...
                if (!mc32_matchIvalToUnion(list, sym->type, 0))
                {
    ...
    else if (IS_STRUCT(type) && SPEC_STRUCT(getSpec(type))->type == UNION)
    {
        if (!list || (list->type == INIT_DEEP))
        {
            ...
            while (sym)
            {
                ...
                if (((IS_STRUCT(sym->type) || getSize(sym->type) == size)) && mc32_matchIvalToUnion(list, sym->type, size))
                {
 ...
 */
static void
mc32_emitInitVal(struct dbuf_s *oBuf, symbol *topsym, sym_link *my_type, initList *list)
{
    symbol *sym;
    int size;
    long lit;
    ...
    if (IS_PTR(my_type))
    {
        ...
        mc32_emitIvals(oBuf, topsym, list, 0, size);
        return;
    }
    ...
    if (IS_ARRAY(my_type) && topsym && topsym->isstrlit)
    {
        ...
        mc32_emitIvalLabel(oBuf, topsym);
        do
        {
            // zwr 1.1.0
    ...
    if (IS_ARRAY(my_type))
    {
        ...
        {
            mc32_emitInitVal(oBuf, topsym, my_type->next, list);
            topsym = NULL;
            if (list)
                list = list->next;
        } // for i
        ...
        while (sym)
        {
            ...
            if (IS_BITFIELD(sym->type))
            {
                while (sym && IS_BITFIELD(sym->type))
                {
                    ...
                    assert(!list || ((list->type == INIT_NODE) && IS_AST_LIT_VALUE(list->init.node)));
                ...
                mc32_emitIvals(oBuf, topsym, NULL, bitfield, len / 8);
                topsym = NULL;
            } // if

            if (sym)
            {
                mc32_emitInitVal(oBuf, topsym, sym->type, list);
                topsym = NULL;
                sym = sym->next;
                if (list)
                    list = list->next;
    ...
    else if (IS_STRUCT(my_type) && SPEC_STRUCT(my_type)->type == UNION)
    {
        ...
        my_type = mc32_matchIvalToUnion(list, my_type, size);
        if (my_type)
        {
            mc32_emitInitVal(oBuf, topsym, my_type, list->init.deep);
            topsym = NULL;
            size -= getSize(my_type);
            if (size > 0)
            {
                // pad with (leading) zeros
                mc32_emitIvals(oBuf, NULL, NULL, 0, size);
            }
            return;
        } // if
 ...
 */
static void
mc32_emitSymbolSet(set *s, int type)
{
    symbol *sym;
    initList *list;
    ...
    for (sym = setFirstItem(s); sym; sym = setNextItem(s))
    {
        ...
        if (sym->etype && SPEC_ABSA(sym->etype) && MC32_IS_CONFIG_ADDRESS(SPEC_ADDR(sym->etype)) && sym->ival)
        {
            // handle config words
            mc32_assignConfigWordValue(SPEC_ADDR(sym->etype),
                                        (int)list2int(sym->ival));
            mc32_stringInSet(sym->rname, &mc32_emitted, 1);
            continue;
        }
        ...
        if (sym->isstrlit)
        {
            ...
            mc32_emitInitVal(ivalBuf, sym, sym->type, NULL);
            continue;
        }
        ...
        if (type != 0 || sym->cdef || (!IS_STATIC(sym->etype) && IS_GLOBAL(sym)))
        {
            ...
            if (sym->cdef && !sym->used)
                continue;

            /* export or import non-static globals */
            if (!mc32_stringInSet(sym->rname, &mc32_emitted, 0))
            {

                if (type == 2 || IS_EXTERN(sym->etype) || sym->cdef)
                {
                    /* do not add to mc32_emitted set, it might occur again! */
                    //if (!sym->used) continue;
                    // declare symbol
                    mc32_emitIfNew(extBuf, &mc32_emitted, "\textern\t%s\n", sym->rname);
                }
                else
                {
                    // declare symbol
                    ...
                    if (!sym->ival && !IS_FUNC(sym->type))
                    {
                        ...
                        if (IS_ABSOLUTE(sym->etype))
                        {
                            ...
                            // deferred to mc32_constructAbsMap
                        }
                        else
                        {
        ...
        if (list)
        {
            ...
            mc32_emitInitVal(ivalBuf, sym, sym->type, sym->ival);
            dbuf_printf(ivalBuf, "\n");
        }
    } // for sym
}
...
#endif
            mc32_emitSymbolSet(map->syms, 0);
        } // if (map)
    }     // for i
    DEBUGprintf("---end of memmaps---\n");

    mc32_emitSymbolSet(publics, 1);
    mc32_emitSymbolSet(externs, 2);

    mc32_emitPseudoStack(gloBuf, extBuf);
    mc32_constructAbsMap(gloDefBuf, gloBuf);
    mc32_printLocals(&locBuf);
    mc32_emitConfigWord(of); // must be done after all the rest
