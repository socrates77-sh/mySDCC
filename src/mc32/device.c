...
static int mc32_pic14_hasSetConfigWord = 0;
static unsigned int mc32_config_word[MAX_NUM_CONFIGS];
static memRange *mc32_rangeRAM = NULL;

/* parse a value from the configuration file */
static int
mc32_parse_config_value(char *str)
{
...
static int
mc32_split_words(char **result_word, char *str)
{
  ...
  for (num_words = 0; num_words < MC32_SPLIT_WORDS_MAX; num_words++)
  {
  ...
  num_words = 0;
  while (token && (num_words < MC32_SPLIT_WORDS_MAX))
  {
...
static char *
mc32_sanitise_processor_name(char *name)
{
  char *proc_pos = name;
           ...
           int io, int is_enhanced)
{
  ...
  new_pic->ram = mc32_rangeRAM;

  mc32_Pics[mc32_num_of_supported_PICS] = new_pic;
  mc32_num_of_supported_PICS++;
...
static void
mc32_register_map(int num_words, char **word)
{
  memRange *r;
  int pcount;
...
static void
mc32_ram_map(int num_words, char **word)
{
  memRange *r;
  ...
  r->end_address = mc32_parse_config_value(word[2]);
  r->alias = mc32_parse_config_value(word[3]);
  r->bank = (r->start_address >> 7) & 3;
...
static void
mc32_setMaxRAM(int size)
{
  mc32_maxRAMaddress = size;

  if (mc32_maxRAMaddress < 0)
  {
    fprintf(stderr, "invalid maxram 0x%x setting in %s\n",
            mc32_maxRAMaddress, DEVICE_FILE_NAME_MC32);
    return;
  } // if
}
...
static MC32_device *
mc32_find_device(char *pic_name)
{
  FILE *pic_file;
  ...
  pic_word = Safe_calloc(sizeof(char *), MC32_SPLIT_WORDS_MAX);
  processor_name = Safe_calloc(sizeof(char *), MC32_SPLIT_WORDS_MAX);

  for (i = 0; i < MAX_NUM_CONFIGS; i++)
  {
    mc32_config_word[i] = -1;
    pic_config[i] = -1;
  } // for

  /* allow abbreviations of the form "f877" - convert to "16f877" */
  simple_pic_name = mc32_sanitise_processor_name(pic_name);
  mc32_num_of_supported_PICS = 0;

  /* open the piclist file */
  /* first scan all include directories */
  pic_file = NULL;
  //fprintf (stderr, "%s: searching %s\n", __FUNCTION__, DEVICE_FILE_NAME_MC32);
       ...
       dir = setNextItem(userIncDirsSet))
  {
    ...
    SNPRINTF(&filename[0], len, "%s%s", dir,
             DIR_SEPARATOR_STRING DEVICE_FILE_NAME_MC32);
  ...
  if (!pic_file)
  {
    SNPRINTF(&filename[0], len, "%s",
             DATADIR LIB_DIR_SUFFIX
                 DIR_SEPARATOR_STRING "pic" DIR_SEPARATOR_STRING DEVICE_FILE_NAME_MC32);
    pic_file = fopen(filename, "rt");
  } // if
  ...
  while (fgets(pic_buf, sizeof(pic_buf) - 1, pic_file) != NULL && !done)
  {
    ...
    /* split into fields */
    num_pic_words = mc32_split_words(pic_word, pic_buf);
    ...
    if (num_pic_words > 0)
    {
        ...
        else
        {
          ...
          else
          {
            ...
            for (wcount = 1; wcount < num_pic_words; wcount++)
            {
              ...
              char *found_name = mc32_sanitise_processor_name(pic_word[wcount]);
      ...
      else
      {
        if (found_processor || pic_name == NULL)
        {
          ...
          {
            pic_maxram = mc32_parse_config_value(pic_word[1]);
            mc32_setMaxRAM(pic_maxram);
          } // if

          else if (STRCASECMP(pic_word[0], "bankmsk") == 0 && num_pic_words > 1)
            pic_bankmsk = mc32_parse_config_value(pic_word[1]);
  ...
  mc32_split_words(pic_word, NULL);
  free(pic_word);
  ...
  else
  {
    ...
    if (found_processor)
    {
      mc32_split_words(processor_name, NULL);
      free(processor_name);
...
/*-----------------------------------------------------------------*
 *  void mc32_list_valid_pics(int ncols, int list_alias)
 *
 ...
 *-----------------------------------------------------------------*/
static void
mc32_list_valid_pics(int ncols)
{
  int col = 0, longest;
  int i, k, l;

  if (mc32_num_of_supported_PICS == 0)
    mc32_find_device(NULL); /* load all the definitions */

  /* decrement the column number if it's greater than zero */
  ncols = (ncols > 1) ? ncols - 1 : 4;

  /* Find the device with the longest name */
  for (i = 0, longest = 0; i < mc32_num_of_supported_PICS; i++)
  {
    k = strlen(mc32_Pics[i]->name);
    if (k > longest)
      longest = k;
  }
  ...
  for (i = 0; i < mc32_num_of_supported_PICS; i++)
  {
    fprintf(stderr, "  %s", mc32_Pics[i]->name);
    l = longest + 2 - strlen(mc32_Pics[i]->name);
    for (k = 0; k < l; k++)
      fputc(' ', stderr);

    fprintf(stderr, "     ");
    if (mc32_Pics[i]->programMemSize % 1024 == 0)
      fprintf(stderr, "%4dK", mc32_Pics[i]->programMemSize / 1024);
    else
      fprintf(stderr, "%5d", mc32_Pics[i]->programMemSize);

    fprintf(stderr, "     %5d     %5d     %4d\n",
            mc32_Pics[i]->dataMemSize, mc32_Pics[i]->eepromMemSize, mc32_Pics[i]->ioPins);
  }

  col = 0;

  fprintf(stderr, "\nMC32 processors supported:\n"); // zwr 1.1.0
  for (i = 0; i < mc32_num_of_supported_PICS; i++)
  {
...
*-----------------------------------------------------------------*/
MC32_device *
mc32_init_pic(char *pic_type)
{
  char long_name[PIC14_STRING_LEN];

  mc32_pic = mc32_find_device(pic_type);
  ...
  if (mc32_pic == NULL)
  {
    ...
    if (mc32_pic == NULL)
    {
      ...
      mc32_list_valid_pics(7);
      exit(1);
    }
  }

  if (mc32_pic && mc32_pic->isEnhancedCore)
  {
    /* Hack: Fixup enhanced core's SFR(s). */
    mc32_pc_indf = &mc32_pc_indf0;
  }
...
*  
*-----------------------------------------------------------------*/
int mc32_picIsInitialized(void)
{
  if (mc32_pic && mc32_maxRAMaddress > 0)
    return 1;
...
/*-----------------------------------------------------------------*
*  char *mc32_processor_base_name(void) - Include file is derived from this.
*-----------------------------------------------------------------*/
char *mc32_processor_base_name(void)
{

  if (!mc32_pic)
    return NULL;

  return mc32_pic->name;
}

int MC32_IS_CONFIG_ADDRESS(int address)
{
  int i;

  for (i = 0; i < mc32_pic->num_configs; i++)
  {
    if ((-1 != address) && (address == mc32_pic->config[i]))
    {
...
/*-----------------------------------------------------------------*
 *  void mc32_assignConfigWordValue(int address, int value)
 *
...
void mc32_assignConfigWordValue(int address, int value)
{
  ...
  for (i = 0; i < mc32_pic->num_configs; i++)
  {
    if ((-1 != address) && (address == mc32_pic->config[i]))
    {
      mc32_config_word[i] = value;
      mc32_pic14_hasSetConfigWord = 1;
    } // if
  }   // for
}

/*-----------------------------------------------------------------*
 * int mc32_emitConfigWord (FILE * vFile)
 *
...
int mc32_emitConfigWord(FILE *vFile)
{
  ...
  if (mc32_pic14_hasSetConfigWord)
  {
    fprintf(vFile, "%s", iComments2);
    fprintf(vFile, "; config word(s)\n");
    fprintf(vFile, "%s", iComments2);
    if (mc32_pic->num_configs > 1)
    {
      for (i = 0; i < mc32_pic->num_configs; i++)
      {
        if (-1 != mc32_config_word[i])
        {
          fprintf(vFile, "\t__config _CONFIG%u, 0x%x\n", i + 1, mc32_config_word[i]);
        } //if
      }   // for
    }
    else
    {
      if (-1 != mc32_config_word[0])
      {
 ...
 *-----------------------------------------------------------------*/
static int mc32_hasSharebank(int *low, int *high, int *size)
{
  memRange *r;
  ...
  while (r)
  {
    ...
    if ((r->alias == mc32_pic->bankMask) && (r->end_address != r->start_address) // ignore SFRs
 ...
 */
static int mc32_isShared(int low, int high)
{
  memRange *r;
  ...
  while (r)
  {
    ...
    if ((r->alias == mc32_pic->bankMask) && (r->start_address <= low) && (r->end_address >= high))
    {
      return 1;
    } // if
    r = r->next;
 ...
 * memory to be used.
 */
int mc32_getSharedStack(int *low, int *high, int *size)
{
  int haveShared;
  int l, h, s;

  s = options.stack_size ? options.stack_size : 0x10;
  haveShared = mc32_hasSharebank(&l, &h, &s);
  if ((options.stack_loc != 0) || !haveShared)
  {
    ...
    //fprintf(stderr, "%s: low %x, high %x, size %x, shared %d\n", __FUNCTION__, l, h, s, mc32_isShared(l, h));
    return (mc32_isShared(l, h));
  }
  else
  {
