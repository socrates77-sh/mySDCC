...
#if 0
/*-----------------------------------------------------------------*
* void mc32_BuildFlowSegment(set *segment, pCodeFlow *pcflow)
*-----------------------------------------------------------------*/
static void mc32_BuildFlowSegment(pCodeFlow *pcflow)
{
	...
	if(pcflow->ancestor && pcflow->to) {
		flowset = pcflow->to;
		while(flowset) {
			mc32_BuildFlowSegment(PCFLINK(flowset->item)->pcflow);
...
void mc32_BuildFlowTree(pBlock *pb)
{
	pCodeFlow *first_pcflow, *pcflow;

	//  fprintf(stderr,"mc32_BuildFlowTree \n");

	first_pcflow = PCFL(mc32_findNextpCode(pb->pcHead, PC_FLOW));
	if (!first_pcflow)
		return;
	...
	*/
	...
	pcflow = PCFL(mc32_findNextpCode(first_pcflow->pc.next, PC_FLOW));
	...
	while (pcflow)
	{
		...
		pcflow = PCFL(mc32_findNextpCode(pcflow->pc.next, PC_FLOW));
	}
