...
/*
 * mc32_max_key keeps track of the largest label number used in
...
unsigned mc32_fReturnSizePic = 4; /* shared with ralloc.c */
static char *mc32_fReturnpic14[] = {"temp1", "temp2", "temp3", "temp4"};

static char **mc32_fReturn = mc32_fReturnpic14;
...
/*-----------------------------------------------------------------*/
/*  mc32_my_powof2(n) - If `n' is an integaer power of 2, then the      */
...
/*-----------------------------------------------------------------*/
static int
mc32_my_powof2(unsigned long num)
{
  if (num)
  {
    if ((num & (num - 1)) == 0)
...
void DEBUGmc32_pic14_AopType(int line_no, operand *left, operand *right, operand *result)
{
...
static void
DEBUGmc32_pic14_AopTypeSign(int line_no, operand *left, operand *right, operand *result)
{
...
void DEBUGmc32_pic14_emitcode(char *inst, char *fmt, ...)
{
  ...
  if (!mc32_debug_verbose && !options.debug)
    return;
  ...
  mc32_addpCode2pBlock(pb, mc32_newpCodeCharP(genLine.lineCurr->line));
}
...
{
  mc32_addpCode2pBlock(pb, mc32_newpCodeLabel(NULL, labelKey2num(key + mc32_labelOffset)));
}

/* gen.h defines a macro mc32_emitpcode that should be used to call mc32_emitpcode
 * as this allows for easy debugging (ever asked the question: where was
 * this instruction geenrated? Here is the answer... */
void mc32_emitpcode_real(PIC_OPCODE poc, pCodeOp *pcop)
{
  if (pcop)
...
// static void
// mc32_emitpcodeNULLop(PIC_OPCODE poc)
void mc32_emitpcodeNULLop(PIC_OPCODE poc)
{
  mc32_addpCode2pBlock(pb, mc32_newpCode(poc, NULL));
}
...
/*-----------------------------------------------------------------*/
/* mc32_emitDebuggerSymbol - associate the current code location  */
...
resolveIfx(resolvedIfx *resIfx, iCode *ifx)
{
  ...
  //  DEBUGmc32_pic14_emitcode("; ***","%s lbl->key=%d, (lab offset=%d)",__FUNCTION__,resIfx->lbl->key,mc32_labelOffset);
}
...
aopForSym(iCode *ic, symbol *sym, bool result)
{
  ...
  if (IN_DIRSPACE(space))
  {
    ...
    DEBUGmc32_pic14_emitcode(";", "%d sym->rname = %s, size = %d", __LINE__, sym->rname, aop->size);
    return aop;
  }
  ...
  aop->aopu.pcop = mc32_popGetImmd(sym->rname, 0, 0, 0);
  PCOI(aop->aopu.pcop)->_const = IN_CODESPACE(space);
  PCOI(aop->aopu.pcop)->index = 0;

  DEBUGmc32_pic14_emitcode(";", "%d: rname %s, val %d, const = %d", __LINE__, sym->rname, 0, PCOI(aop->aopu.pcop)->_const);
...
aopForRemat(operand *op) // x symbol *sym)
{
  ...
  if (IS_OP_POINTER(op))
  {
    DEBUGmc32_pic14_emitcode(";", "%s %d IS_OP_POINTER", __FUNCTION__, __LINE__);
  }
  for (;;)
  {
    if (ic->op == '+')
  ...
  aop->aopu.pcop = mc32_popGetImmd(OP_SYMBOL(IC_LEFT(ic))->rname, 0, val, 0);
...
/*-----------------------------------------------------------------*/
/* mc32_sameRegs - two asmops have the same registers             */
/*-----------------------------------------------------------------*/
...
/*-----------------------------------------------------------------*/
/* mc32_aopOp - allocates an asmop for an operand  :                    */
/*-----------------------------------------------------------------*/
void mc32_aopOp(operand *op, iCode *ic, bool result)
{
...

  {
    ...
    if (IS_PTR_CONST(type))
      DEBUGmc32_pic14_emitcode(";", "%d aop type is const pointer", __LINE__);
  }
  ...
  if (IS_TRUE_SYMOP(op))
  {
    //DEBUGmc32_pic14_emitcode(";","%d - true symop",__LINE__);
  ...
  if (sym->isspilt || sym->nRegs == 0)
  {
    ...
    if (sym->remat)
    {
      ...
      aop->size = getSize(sym->type);
      //DEBUGmc32_pic14_emitcode(";"," %d: size %d, %s\n",__LINE__,aop->size,aop->aopu.aop_immd);
      return;
    }
    ...
    if (sym->ruonly)
    {
      if (sym->isptr) // && sym->uptr
      {
        ...
        aop->aopu.pcop = mc32_newpCodeOp(NULL, PO_GPR_POINTER); //mc32_popCopyReg(&mc32_pc_fsr);
      ...
      else
      {
        ...
        aop->size = getSize(sym->type);
        for (i = 0; i < mc32_fReturnSizePic; i++)
          aop->aopu.aop_str[i] = mc32_fReturn[i];
    ...
    if (sym->usl.spillLoc)
    {
      ...
      //aop->aopu.pcop = mc32_popGetImmd(sym->usl.spillLoc->rname,0,sym->usl.spillLoc->offset);
      aop->aopu.pcop = mc32_popRegFromString(sym->usl.spillLoc->rname, getSize(sym->type), sym->usl.spillLoc->offset);
      aop->size = getSize(sym->type);
  ...
  DEBUGmc32_pic14_emitcode(";", "%d register type nRegs=%d", __LINE__, sym->nRegs);
...
/*-----------------------------------------------------------------*/
/* mc32_freeAsmop - free up the asmop given to an operand               */
/*----------------------------------------------------------------*/
...
/*-----------------------------------------------------------------*/
/* mc32_aopGet - for fetching value of the aop                          */
/*-----------------------------------------------------------------*/
char *
mc32_aopGet(asmop *aop, int offset, bool bit16, bool dname)
{
  ...
  case AOP_STR:
    ...
    if (strcmp(aop->aopu.aop_str[offset], "a") == 0 && dname)
      return "acc";
    DEBUGmc32_pic14_emitcode(";", "%d - %s", __LINE__, aop->aopu.aop_str[offset]);
  ...
  case AOP_PCODE:
  {
    ...
    DEBUGmc32_pic14_emitcode(";", "%d: mc32_aopGet AOP_PCODE type %s", __LINE__, mc32_pCodeOpType(pcop));
    if (pcop->name)
    {
...
/*-----------------------------------------------------------------*/
/* mc32_popGetTempReg - create a new temporary pCodeOp                  */
/*-----------------------------------------------------------------*/
static pCodeOp *
mc32_popGetTempReg(void)
{
...
mc32_popGetLabel(unsigned int key)
{
  ...
  DEBUGmc32_pic14_emitcode("; ***", "%s  key=%d, label offset %d", __FUNCTION__, key, mc32_labelOffset);

  if (key > (unsigned int)mc32_max_key)
    mc32_max_key = key;

  return mc32_newpCodeOpLabel(NULL, labelKey2num(key + mc32_labelOffset));
}

/*-------------------------------------------------------------------*/
/* mc32_popGetHighLabel - create a new pCodeOp of type PO_LABEL with offset=1 */
/*-------------------------------------------------------------------*/
static pCodeOp *
mc32_popGetHighLabel(unsigned int key)
{
  pCodeOp *pcop;
  pcop = mc32_popGetLabel(key);
  PCOLAB(pcop)->offset = 1;
  return pcop;
}

/*-----------------------------------------------------------------*/
/* mc32_popGetLit - asm operator to pcode operator conversion               */
/*-----------------------------------------------------------------*/
pCodeOp *
mc32_popGetLit(unsigned int lit)
{

  return mc32_newpCodeOpLit((unsigned char)lit);
}
...
mc32_popGetWithString(char *str, int isExtern)
{
  ...
  pcop = mc32_newpCodeOp(str, PO_STR);
...
mc32_popGetExternal(char *str, int isReg)
{
  ...
  if (isReg)
  {
    pcop = mc32_newpCodeOpRegFromStr(str);
  }
  else
  {
    pcop = mc32_popGetWithString(str, 1);
  }
...
mc32_popRegFromString(char *str, int size, int offset)
{
  ...
  PCOR(pcop)->r = mc32_dirregWithName(pcop->name);
  if (PCOR(pcop)->r == NULL)
  {
    //fprintf(stderr,"%d - couldn't find %s in allocated registers, size =%d\n",__LINE__,aop->aopu.aop_dir,aop->size);
    PCOR(pcop)->r = mc32_allocRegByName(pcop->name, size);
    DEBUGmc32_pic14_emitcode(";", "%d  %s   offset=%d - had to alloc by reg name", __LINE__, pcop->name, offset);
  }
  else
  {
    DEBUGmc32_pic14_emitcode(";", "%d  %s   offset=%d", __LINE__, pcop->name, offset);
  }
...
/*-----------------------------------------------------------------*/
/* mc32_popGet - asm operator to pcode operator conversion              */
/*-----------------------------------------------------------------*/
pCodeOp *
mc32_popGet(asmop *aop, int offset) //, bool bit16, bool dname)
{
  //char *s = buffer ;
  //char *rs;
  ...
  case AOP_IMMD:
    DEBUGmc32_pic14_emitcode(";", "%d", __LINE__);
    return mc32_popGetImmd(aop->aopu.aop_immd, offset, 0, 0);

  case AOP_DIR:
    return mc32_popRegFromString(aop->aopu.aop_dir, aop->size, offset);
  ...
  case AOP_REG:
  {
    ...
    DEBUGmc32_pic14_emitcode(";", "%d rIdx = r0x%X ", __LINE__, rIdx);
    return pcop;
  }

  case AOP_CRY:
    pcop = mc32_newpCodeOpBit(aop->aopu.aop_dir, -1, 1);
    PCOR(pcop)->r = mc32_dirregWithName(aop->aopu.aop_dir);
    //if(PCOR(pcop)->r == NULL)
    //fprintf(stderr,"%d - couldn't find %s in allocated registers\n",__LINE__,aop->aopu.aop_dir);
    return pcop;

  case AOP_LIT:
    return mc32_newpCodeOpLit(pic14aopLiteral(aop->aopu.aop_lit, offset));

  case AOP_STR:
    DEBUGmc32_pic14_emitcode(";", "%d  %s", __LINE__, aop->aopu.aop_str[offset]);
    return mc32_newpCodeOpRegFromStr(aop->aopu.aop_str[offset]);
  ...
  case AOP_PCODE:
    ...
    {
    case PO_IMMEDIATE:
      pcop = mc32_pCodeOpCopy(aop->aopu.pcop);
  ...
  werror(E_INTERNAL_ERROR, __FILE__, __LINE__, "mc32_popGet got unsupported aop->type");
  exit(0);
}

/*-----------------------------------------------------------------*/
/* mc32_popGetAddr - access the low/high word of a symbol (immediate)   */
/*              (for non-PO_IMMEDIATEs this is the same as mc32_popGet) */
/*-----------------------------------------------------------------*/
pCodeOp *
mc32_popGetAddr(asmop *aop, int offset, int index)
{
  if (aop->type == AOP_PCODE && aop->aopu.pcop->type == PO_IMMEDIATE)
  {
    ...
    if (offset == 2)
    {
      pcop = mc32_popGetLit(aop->code ? GPTRTAG_CODE : GPTRTAG_DATA);
      return pcop;
    }

    pcop = mc32_pCodeOpCopy(pcop);
    /* usually we want to access the memory at "<symbol> + offset" (using ->index),
       * but sometimes we want to access the high byte of the symbol's address (using ->offset) */
  ...
  {
    return mc32_popGet(aop, offset + index);
  }
}
...
void mc32_aopPut(asmop *aop, char *s, int offset)
{
  ...
  {
    werror(E_INTERNAL_ERROR, __FILE__, __LINE__, "mc32_aopPut got offset > aop->size");
    exit(0);
  }
  ...
  case AOP_DIR:
    ...
    if (strcmp(d, s))
    {
      ...
      if (strcmp(s, "W"))
      {
        mc32_pic14_emitcode(";BUG!? should have this:movf", "%s,w   %d", s, __LINE__);
        if (offset >= aop->size)
        {
          mc32_emitpcode(POC_CLRF, mc32_popGet(aop, offset));
          break;
        }
        else
        {
          mc32_emitpcode(POC_MOVFW, mc32_popGetImmd(s, 0, offset, 0));
        }
      }
      mc32_emitpcode(POC_MOVWF, mc32_popGet(aop, offset));
    }
    break;
  ...
  case AOP_REG:
    ...
    {
      if (strcmp(s, "W") == 0)
        mc32_pic14_emitcode("movf", "%s,w  ; %d", s, __LINE__);

      mc32_pic14_emitcode("movwf", "%s", aop->aopu.aop_reg[offset]->name);
      ...
      else if (strcmp(s, "W") == 0)
      {
        ...
        mc32_emitpcode(POC_MOVFW, pcop);
        mc32_emitpcode(POC_MOVWF, mc32_popGet(aop, offset));
      }
      else if (strcmp(s, one) == 0)
      {
        mc32_emitpcode(POC_CLRF, mc32_popGet(aop, offset));
        mc32_emitpcode(POC_INCF, mc32_popGet(aop, offset));
      }
      else
      {
        mc32_emitpcode(POC_MOVWF, mc32_popGet(aop, offset));
  ...
  case AOP_STK:
    if (strcmp(s, "a") == 0)
      mc32_pic14_emitcode("push", "acc");
    else
      mc32_pic14_emitcode("push", "%s", s);
  ...
  case AOP_CRY:
    ...
    if (!aop->aopu.aop_dir)
    {
      mc32_pic14_emitcode("clr", "a");
      mc32_pic14_emitcode("rlc", "a");
    }
    else
    {
      if (s == zero)
        mc32_pic14_emitcode("clr", "%s", aop->aopu.aop_dir);
      else if (s == one)
        mc32_pic14_emitcode("setb", "%s", aop->aopu.aop_dir);
      else if (!strcmp(s, "c"))
        mc32_pic14_emitcode("mov", "%s,c", aop->aopu.aop_dir);
      else
      {
        ...
        mc32_pic14_emitcode("clr", "c");
        mc32_pic14_emitcode("jz", "%05d_DS_", labelKey2num(lbl->key));
        mc32_pic14_emitcode("cpl", "c");
        mc32_pic14_emitcode("", "%05d_DS_:", labelKey2num(lbl->key));
        mc32_pic14_emitcode("mov", "%s,c", aop->aopu.aop_dir);
      }
    }
    break;
  ...
  case AOP_STR:
    ...
    if (strcmp(aop->aopu.aop_str[offset], s))
      mc32_pic14_emitcode("mov", "%s,%s ; %d", aop->aopu.aop_str[offset], s, __LINE__);
    break;

  default:
    werror(E_INTERNAL_ERROR, __FILE__, __LINE__, "mc32_aopPut got unsupported aop->type");
    exit(0);
  }
}

/*-----------------------------------------------------------------*/
/* mc32_mov2w_op - generate either a MOVLW or MOVFW based operand type  */
/*-----------------------------------------------------------------*/
static void
mc32_mov2w_op(operand *op, int offset)
{
  assert(op);
  FENTRY;

  /* for PO_IMMEDIATEs: use address or value? */
  if (mc32_op_isLitLike(op))
  {
    ...
    if (IS_SYMOP(op) && IS_GENPTR(OP_SYM_TYPE(op)) && AOP_SIZE(op) < offset)
    {
      if (offset == GPTRSIZE - 1)
        mc32_emitpcode(POC_MOVLW, mc32_popGetLit(GPTRTAG_DATA));
      else
        mc32_emitpcode(POC_MOVLW, mc32_popGetLit(0));
    }
    else
      mc32_emitpcode(POC_MOVLW, mc32_popGetAddr(AOP(op), offset, 0));
  }
  else
  {
    /* access value stored in op */
    mc32_mov2w(AOP(op), offset);
  }
}

/*-----------------------------------------------------------------*/
/* mc32_mov2w - generate either a MOVLW or MOVFW based operand type     */
/*-----------------------------------------------------------------*/
void mc32_mov2w(asmop *aop, int offset)
{

  if (!aop)
    return;

  DEBUGmc32_pic14_emitcode("; ***", "%s  %d  offset=%d", __FUNCTION__, __LINE__, offset);

  if (mc32_aop_isLitLike(aop))
    mc32_emitpcode(POC_MOVLW, mc32_popGetAddr(aop, offset, 0));
  else
    mc32_emitpcode(POC_MOVFW, mc32_popGet(aop, offset));
}

static void
movwf(asmop *op, int offset)
{
  mc32_emitpcode(POC_MOVWF, mc32_popGet(op, offset));
}
...
get_argument_pcop(int idx)
{
  ...
  return popRegFromIdx(mc32_Gstack_base_addr - (idx - 1));
}
...
get_return_val_pcop(int offset)
{
  ...
  return popRegFromIdx(mc32_Gstack_base_addr - (offset - 1));
}
...
{
  if (op)
    mc32_mov2w_op(op, offset);
  if (idx != 0)
    mc32_emitpcode(POC_MOVWF, get_argument_pcop(idx));
}
...
{
  if (idx != 0)
    mc32_emitpcode(POC_MOVFW, get_return_val_pcop(idx));
  movwf(AOP(op), offset);
}
...
call_libraryfunc(char *name)
{
  ...
  //mc32_emitpcode(POC_PAGESEL, mc32_popGetWithString(name, 1));
  /* call the library function */
  mc32_emitpcode(POC_CALL, mc32_popGetExternal(name, 0));
  /* might return from different page... */
  //mc32_emitpcode(POC_PAGESEL, mc32_popGetWithString("$", 0));
...
/*-----------------------------------------------------------------*/
/* mc32_getDataSize - get the operand data size                   */
/*-----------------------------------------------------------------*/
int mc32_getDataSize(operand *op)
{
  int size;
...
/*-----------------------------------------------------------------*/
/* mc32_outAcc - output Acc                                       */
/*-----------------------------------------------------------------*/
void mc32_outAcc(operand *result)
{
  int size, offset;
  DEBUGmc32_pic14_emitcode("; ***", "%s  %d - ", __FUNCTION__, __LINE__);
  DEBUGmc32_pic14_AopType(__LINE__, NULL, NULL, result);

  size = mc32_getDataSize(result);
  if (size)
  {
    mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), 0));
    size--;
    offset = 1;
...
pic14_outBitC(operand *result)
{
  ...
  if (AOP_TYPE(result) == AOP_CRY)
    mc32_aopPut(AOP(result), "c", 0);
  else
  {
    mc32_pic14_emitcode("clr", "a  ; %d", __LINE__);
    mc32_pic14_emitcode("rlc", "a");
    mc32_outAcc(result);
  }
}
...
pic14_toBoolean(operand *oper)
{
  ...
  {
    mc32_emitpcode(POC_MOVFW, mc32_popGet(AOP(oper), 0));
    offset = 1;
  }

  while (offset < size)
  {
    mc32_emitpcode(POC_IORFW, mc32_popGet(AOP(oper), offset++));
  }
  /* Z is set iff (oper == 0) */
}
...
genNot(iCode *ic)
{
  ...
  mc32_aopOp(IC_LEFT(ic), ic, FALSE);
  mc32_aopOp(IC_RESULT(ic), ic, TRUE);

  DEBUGmc32_pic14_AopType(__LINE__, IC_LEFT(ic), NULL, IC_RESULT(ic));
  /* if in bit space then a special case */
  if (AOP_TYPE(IC_LEFT(ic)) == AOP_CRY)
  {
    if (AOP_TYPE(IC_RESULT(ic)) == AOP_CRY)
    {
      mc32_emitpcode(POC_MOVLW, mc32_popGet(AOP(IC_LEFT(ic)), 0));
      mc32_emitpcode(POC_XORWF, mc32_popGet(AOP(IC_RESULT(ic)), 0));
    }
    else
    {
      mc32_emitpcode(POC_CLRF, mc32_popGet(AOP(IC_RESULT(ic)), 0));
      mc32_emitpcode(POC_BTFSS, mc32_popGet(AOP(IC_LEFT(ic)), 0));
      mc32_emitpcode(POC_INCF, mc32_popGet(AOP(IC_RESULT(ic)), 0));
    }
    goto release;
  }

  size = AOP_SIZE(IC_LEFT(ic));
  mc32_mov2w(AOP(IC_LEFT(ic)), 0);
  while (--size > 0)
  {
    if (mc32_op_isLitLike(IC_LEFT(ic)))
      mc32_emitpcode(POC_IORLW, mc32_popGetAddr(AOP(IC_LEFT(ic)), size, 0));
    else
      mc32_emitpcode(POC_IORFW, mc32_popGet(AOP(IC_LEFT(ic)), size));
  }
  mc32_emitpcode(POC_MOVLW, mc32_popGetLit(0));
  mc32_emitSKPNZ;
  ...
  {
    mc32_emitpcode(POC_CLRF, mc32_popGet(AOP(IC_RESULT(ic)), size));
  }
  goto release;

release:
  /* release the aops */
  mc32_freeAsmop(IC_LEFT(ic), NULL, ic, (RESULTONSTACK(ic) ? 0 : 1));
...
genCpl(iCode *ic)
{
  ...
  DEBUGmc32_pic14_emitcode("; ***", "%s  %d", __FUNCTION__, __LINE__);
  mc32_aopOp((left = IC_LEFT(ic)), ic, FALSE);
  mc32_aopOp((result = IC_RESULT(ic)), ic, TRUE);
  ...
  if (AOP_TYPE(result) == AOP_CRY && AOP_TYPE(left) == AOP_CRY)
  {
    ...
    mc32_pic14_emitcode("mov", "c,%s", left->aop->aopu.aop_dir);
    mc32_pic14_emitcode("cpl", "c");
    mc32_pic14_emitcode("mov", "%s,c", result->aop->aopu.aop_dir);
    goto release;
  }
  ...
  while (size--)
  {
    mc32_emitpcode(POC_COMFW, mc32_popGet(AOP(left), offset));
    mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offset));
    offset++;
  }
  mc32_addSign(result, AOP_SIZE(left), !SPEC_USIGN(operandType(result)));

release:
  /* release the aops */
  mc32_freeAsmop(left, NULL, ic, (RESULTONSTACK(ic) ? 0 : 1));
  mc32_freeAsmop(result, NULL, ic, TRUE);
}
...
genUminusFloat(operand *op, operand *result)
{
  ...
  mc32_mov2w_op(op, size);
  mc32_emitpcode(POC_XORLW, mc32_popGetLit(0x80));
...
genUminus(iCode *ic)
{
  ...
  for (i = 0; i < size; i++)
  {
    if (mc32_sameRegs(AOP(IC_LEFT(ic)), AOP(IC_RESULT(ic))))
      mc32_emitpcode(POC_COMF, mc32_popGet(AOP(IC_LEFT(ic)), i));
    else
    {
      mc32_emitpcode(POC_COMFW, mc32_popGet(AOP(IC_LEFT(ic)), i));
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(IC_RESULT(ic)), i));
    }
  }
  ...
  {
    mc32_emitSKPNZ;
    mc32_emitpcode(POC_INCF, mc32_popGet(AOP(IC_RESULT(ic)), i));
  }
...
saverbank(int bank, iCode *ic, bool pushPsw)
{
  ...
  DEBUGmc32_pic14_emitcode("; ***", "%s  %d - WARNING no code generated", __FUNCTION__, __LINE__);
#if 0
  int i;
  asmop *aop;
  regs *r = NULL;
  ...
  if (options.useXstack)
    {
      ...
      mc32_pic14_emitcode ("mov", "%s,_spx", r->name);
  ...
  for (i = 0; i < pic14_nRegs; i++)
    {
      if (options.useXstack)
        {
          mc32_pic14_emitcode ("inc", "%s", r->name);
          //mc32_pic14_emitcode("mov","a,(%s+%d)",
          //       regspic14[i].base,8*bank+regspic14[i].offset);
          mc32_pic14_emitcode ("movx", "@%s,a", r->name);
        }
      else
        mc32_pic14_emitcode ("push", "");    // "(%s+%d)",
      //regspic14[i].base,8*bank+regspic14[i].offset);
    }
  ...
  if (pushPsw)
    {
      if (options.useXstack)
        {
          mc32_pic14_emitcode ("mov", "a,psw");
          mc32_pic14_emitcode ("movx", "@%s,a", r->name);
          mc32_pic14_emitcode ("inc", "%s", r->name);
          mc32_pic14_emitcode ("mov", "_spx,%s", r->name);
          mc32_freeAsmop (NULL, aop, ic, TRUE);

        }
      else
        mc32_pic14_emitcode ("push", "psw");

      mc32_pic14_emitcode ("mov", "psw,#0x%02x", (bank << 3) & 0x00ff);
    }
  ic->bankSaved = 1;
#endif
}
...
unsaveRegisters(iCode *ic)
{
  ...
  if (options.useXstack)
  {
    mc32_pic14_emitcode("mov", "r0,%s", spname);
    for (i = pic14_nRegs; i >= 0; i--)
    {
      if (bitVectBitValue(rsave, i))
      {
        mc32_pic14_emitcode("dec", "r0");
        mc32_pic14_emitcode("movx", "a,@r0");
        mc32_pic14_emitcode("mov", "%s,a", mc32_pic14_regWithIdx(i)->name);
      }
    }
    mc32_pic14_emitcode("mov", "%s,r0", spname);
  } //else
  //for (i =  pic14_nRegs ; i >= 0 ; i--) {
  //  if (bitVectBitValue(rsave,i))
  //  mc32_pic14_emitcode("pop","%s",mc32_pic14_regWithIdx(i)->dname);
  //}
}
...
pushSide(operand *oper, int size)
{
  ...
  while (size--)
    {
      char *l = mc32_aopGet (AOP (oper), offset++, FALSE, TRUE);
      if (AOP_TYPE (oper) != AOP_REG && AOP_TYPE (oper) != AOP_DIR && strcmp (l, "a"))
        {
          mc32_pic14_emitcode ("mov", "a,%s", l);
          mc32_pic14_emitcode ("push", "acc");
        }
      else
        mc32_pic14_emitcode ("push", "%s", l);
    }
#endif
}
...
assignResultValue(operand *oper)
{
  ...
  DEBUGmc32_pic14_AopType(__LINE__, oper, NULL, NULL);
...
genIpush(iCode *ic)
{
  ...
  if (!ic->parmPush)
    {
      ...
      while (size--)
        {
          l = mc32_aopGet (AOP (IC_LEFT (ic)), offset++, FALSE, TRUE);
          if (*l == '#')
            {
              MOVA (l);
              l = "acc";
  ...
  mc32_freeAsmop (IC_LEFT (ic), NULL, ic, TRUE);
#endif
}
...
genIpop(iCode *ic)
{
  ...
  offset = (size - 1);
  while (size--)
    mc32_pic14_emitcode ("pop", "%s", mc32_aopGet (AOP (IC_LEFT (ic)), offset--, FALSE, TRUE));
      ...
      if (options.useXstack)
        {
          ...
          mc32_pic14_emitcode ("mov", "%s,_spx", r->name);
          mc32_pic14_emitcode ("movx", "a,@%s", r->name);
          mc32_pic14_emitcode ("mov", "psw,a");
          mc32_pic14_emitcode ("dec", "%s", r->name);

        }
      else
        mc32_pic14_emitcode ("pop", "psw");
    }
  ...
  for (i = (pic14_nRegs - 1); i >= 0; i--)
    {
      if (options.useXstack)
        {
          mc32_pic14_emitcode ("movx", "a,@%s", r->name);
          //mc32_pic14_emitcode("mov","(%s+%d),a",
          //     regspic14[i].base,8*bank+regspic14[i].offset);
          mc32_pic14_emitcode ("dec", "%s", r->name);

        }
      else
        mc32_pic14_emitcode ("pop", "");     //"(%s+%d)",
      //regspic14[i].base,8*bank); //+regspic14[i].offset);
    }
...
genCall(iCode *ic)
{
  ...
  if (_G.sendSet)
  {
    ...
    for (sic = setFirstItem(_G.sendSet); sic; sic = setNextItem(_G.sendSet))
    {
      ...
      while (size--)
      {
        DEBUGmc32_pic14_emitcode("; ", "%d left %s", __LINE__, mc32_AopType(AOP_TYPE(IC_LEFT(sic))));
        ...
        mc32_mov2w_op(IC_LEFT(sic), offset);
        offset++;
      }
      mc32_freeAsmop(IC_LEFT(sic), NULL, sic, TRUE);
    }
    _G.sendSet = NULL;
  }
  /* make the call */
  ...
  if (isExtern)
  {
    /* Extern functions and ISRs maybe on a different page;
       * must call pagesel */
    //mc32_emitpcode(POC_PAGESEL, mc32_popGetWithString(name, 1));    // zwr 1.0.0
  }
  mc32_emitpcode(POC_CALL, mc32_popGetWithString(name, isExtern));
  if (isExtern)
  {
      ...
      IS_TRUE_SYMOP(IC_RESULT(ic)))
  {
    ...
    DEBUGmc32_pic14_emitcode("; ", "%d left %s", __LINE__, mc32_AopType(AOP_TYPE(IC_RESULT(ic))));
...
genPcall(iCode *ic)
{
  ...
  left = IC_LEFT(ic);
  mc32_aopOp(left, ic, FALSE);
  DEBUGmc32_pic14_AopType(__LINE__, left, NULL, NULL);
  ...
  if (_G.sendSet)
  {
    ...
    DEBUGmc32_pic14_emitcode("; ***", "%s  %d - WARNING arg-passing to indirect call not supported", __FUNCTION__, __LINE__);
  ...
  /* first idea - factor out a common helper function and call it.
     ...
     DEBUGmc32_pic14_emitcode ("; ***","%s  %d AOP_DIR %s",__FUNCTION__,__LINE__,rname);
     buffer = Safe_calloc(1,strlen(rname)+16);
     sprintf(buffer, "%s_goto_helper", rname);
     mc32_addpCode2pBlock(pb,mc32_newpCode(POC_CALL,mc32_newpCodeOp(buffer,PO_STR)));
     free(buffer);
     }
   */
  mc32_emitpcode(POC_CALL, mc32_popGetLabel(albl->key));
  pcop = mc32_popGetLabel(blbl->key);
  mc32_emitpcode(POC_PAGESEL, pcop); /* Must restore PCLATH before goto, without destroying W */
  mc32_emitpcode(POC_GOTO, pcop);
  mc32_emitpLabel(albl->key);

  mc32_emitpcode(poc, mc32_popGetAddr(AOP(left), 1, 0));
  mc32_emitpcode(POC_MOVWF, mc32_popCopyReg(&mc32_pc_pclath));
  mc32_emitpcode(poc, mc32_popGetAddr(AOP(left), 0, 0));
  mc32_emitpcode(POC_MOVWF, mc32_popCopyReg(&mc32_pc_pcl));

  mc32_emitpLabel(blbl->key);
...
genFunction(iCode *ic)
{
  ...
  mc32_pic14_emitcode(";", "-----------------------------------------");
  mc32_pic14_emitcode(";", " function %s", (sym = OP_SYMBOL(IC_LEFT(ic)))->name);
  mc32_pic14_emitcode(";", "-----------------------------------------");

  /* prevent this symbol from being mc32_emitted as 'extern' */
...
/* here we need to generate the equates for the
      ...
      for (i = 0; i < pic14_nRegs; i++)
        {
          if (strcmp (regspic14[i].base, "0") == 0)
            mc32_pic14_emitcode ("", "%s = 0x%02x", regspic14[i].dname, 8 * rbank + regspic14[i].offset);
          else
            mc32_pic14_emitcode ("", "%s = %s + 0x%02x", regspic14[i].dname, regspic14[i].base, 8 * rbank + regspic14[i].offset);
        }
    }
#endif
    ...
    if (IFFUNC_CALLEESAVES(sym->type))
    {
      ...
      if (sym->regsUsed)
      {
        ...
        for (i = 0; i < sym->regsUsed->size; i++)
        {
          ...
          {
            //mc32_pic14_emitcode("push","%s",mc32_pic14_regWithIdx(i)->dname);
            _G.nRegsSaved++;
          }
        }
      }
  ...
  {
    mc32_genCritical(NULL);
    if (IFFUNC_ARGS(sym->type))
    {
      // zwr 1.0.0
      fprintf(stderr, "MC32: Functions with __critical (%s) must not have arguments for now.\n", sym->name);
      exit(1);
    } // if
  }   // if
  ...
  {
    mc32_pic14_emitcode("push", "psw");
    mc32_pic14_emitcode("mov", "psw,#0x%02x", (FUNC_REGBANK(sym->type) << 3) & 0x00ff);
  }
  ...
  if (IFFUNC_ISREENT(sym->type) || options.stackAuto)
  {
    ...
    {
      mc32_pic14_emitcode("mov", "r0,%s", spname);
      mc32_pic14_emitcode("mov", "a,_bp");
      mc32_pic14_emitcode("movx", "@r0,a");
      mc32_pic14_emitcode("inc", "%s", spname);
    }
    else
    {
      /* set up the stack */
      mc32_pic14_emitcode("push", "_bp"); /* save the callers stack  */
    }
    mc32_pic14_emitcode("mov", "_bp,%s", spname);
  }
  ...
  if (sym->stack)
  {
    ...
    if (i > 3 && sym->recvSize < 4)
    {
      ...
      mc32_pic14_emitcode("mov", "a,sp");
      mc32_pic14_emitcode("add", "a,#0x%02x", ((char)sym->stack & 0xff));
      mc32_pic14_emitcode("mov", "sp,a");
    }
    else
      while (i--)
        mc32_pic14_emitcode("inc", "sp");
  }

  if (sym->xstack)
  {

    mc32_pic14_emitcode("mov", "a,_spx");
    mc32_pic14_emitcode("add", "a,#0x%02x", ((char)sym->xstack & 0xff));
    mc32_pic14_emitcode("mov", "_spx,a");
  }
}
...
genEndFunction(iCode *ic)
{
  ...
  if (IFFUNC_ISREENT(sym->type) || options.stackAuto)
  {
    mc32_pic14_emitcode("mov", "%s,_bp", spname);
  }
  ...
  if ((IFFUNC_ISREENT(sym->type) || options.stackAuto))
  {
    if (options.useXstack)
    {
      mc32_pic14_emitcode("mov", "r0,%s", spname);
      mc32_pic14_emitcode("movx", "a,@r0");
      mc32_pic14_emitcode("mov", "_bp,a");
      mc32_pic14_emitcode("dec", "%s", spname);
    }
    else
    {
      mc32_pic14_emitcode("pop", "_bp");
    }
  }
  ...
  if (IFFUNC_ISISR(sym->type))
  {
      ...
      if (!IFFUNC_HASFCALL(sym->type))
      {
        ...
        if (sym->regsUsed)
        {
          ...
          for (i = sym->regsUsed->size; i >= 0; i--)
          {
            if (bitVectBitValue(sym->regsUsed, i))
            {
              mc32_pic14_emitcode("pop", "junk"); //"%s",mc32_pic14_regWithIdx(i)->dname);
            }
          }
        }
      }
    ...
    else
    {
      ...
      // //mc32_emitpcode(POC_MOVFW,  mc32_popGetExternal("___sdcc_saved_pclath", 1));
      // mc32_emitpcode(POC_MOVFW, mc32_popCopyReg(&mc32_pc_psave));
      // mc32_emitpcode(POC_MOVWF, mc32_popCopyReg(&mc32_pc_pclath));
      // mc32_emitpcode(POC_CLRF, mc32_popCopyReg(&mc32_pc_status)); // see genFunction
      // //mc32_emitpcode(POC_SWAPFW, mc32_popGetExternal("___sdcc_saved_status", 1));
      // mc32_emitpcode(POC_SWAPFW, mc32_popCopyReg(&mc32_pc_ssave));
      // mc32_emitpcode(POC_MOVWF, mc32_popCopyReg(&mc32_pc_status));
      // mc32_emitpcode(POC_SWAPF, mc32_popCopyReg(&mc32_pc_wsave));
      // mc32_emitpcode(POC_SWAPFW, mc32_popCopyReg(&mc32_pc_wsave));
    } // if
    mc32_addpCode2pBlock(pb, mc32_newpCodeLabel("END_OF_INTERRUPT", -1));
    mc32_emitpcodeNULLop(POC_RETFIE);
  }
  else
  {
    if (IFFUNC_ISCRITICAL(sym->type))
      mc32_pic14_emitcode("setb", "ea");
...
jumpret:
  ...
  if (!(ic->next && ic->next->op == LABEL && IC_LABEL(ic->next) == returnLabel))
  {
    ...
    mc32_emitpcode(POC_GOTO, mc32_popGetLabel(returnLabel->key));
  }
}
...
mc32_genCritical(iCode *ic)
{
  ...
  saved_intcon = mc32_popGetTempReg();
  addSetHead(&critical_temps, saved_intcon);

  /* This order saves one BANKSEL back to INTCON. */
  mc32_emitpcode(POC_MOVFW, mc32_popCopyReg(&mc32_pc_intcon));
  mc32_emitpcode(POC_BCF, mc32_popCopyGPR2Bit(mc32_popCopyReg(&mc32_pc_intcon), 7));
  mc32_emitpcode(POC_MOVWF, mc32_pCodeOpCopy(saved_intcon));
}

static void
mc32_genEndCritical(iCode *ic)
{
  ...
  mc32_emitpcode(POC_BTFSC, mc32_popCopyGPR2Bit(mc32_pCodeOpCopy(saved_intcon), 7));
  mc32_emitpcode(POC_BSF, mc32_popCopyGPR2Bit(mc32_popCopyReg(&mc32_pc_intcon), 7));
...
genLabel(iCode *ic)
{
  ...
  mc32_emitpLabel(IC_LABEL(ic)->key);
  mc32_pic14_emitcode("", "_%05d_DS_:", labelKey2num(IC_LABEL(ic)->key + mc32_labelOffset));
}
...
genGoto(iCode *ic)
{
  ...
  mc32_emitpcode(POC_GOTO, mc32_popGetLabel(IC_LABEL(ic)->key));
  mc32_pic14_emitcode("goto", "_%05d_DS_", labelKey2num(IC_LABEL(ic)->key + mc32_labelOffset));
}
...
genMultbits(operand *left, operand *right, operand *result)
{
  ...
  if (!mc32_sameRegs(AOP(result), AOP(right)))
    mc32_emitpcode(POC_BSF, mc32_popGet(AOP(result), 0));

  mc32_emitpcode(POC_BTFSC, mc32_popGet(AOP(right), 0));
  mc32_emitpcode(POC_BTFSS, mc32_popGet(AOP(left), 0));
  mc32_emitpcode(POC_BCF, mc32_popGet(AOP(result), 0));
}
...
genMultOneByte(operand *left, operand *right, operand *result)
{
  ...
  DEBUGmc32_pic14_emitcode("; ***", "%s  %d", __FUNCTION__, __LINE__);
  DEBUGmc32_pic14_AopType(__LINE__, left, right, result);
  DEBUGmc32_pic14_AopTypeSign(__LINE__, left, right, result);
  ...
  offset = mc32_Gstack_base_addr - (2 * size - 1);

  /* pass right operand as argument */
  for (i = 0; i < size; i++)
  {
    mc32_mov2w(AOP(right), i);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(++offset));
  } // for

  /* pass left operand as argument */
  for (i = 0; i < size; i++)
  {
    mc32_mov2w(AOP(left), i);
    if (i != size - 1)
      mc32_emitpcode(POC_MOVWF, popRegFromIdx(++offset));
  } // for
  assert(offset == mc32_Gstack_base_addr);
  ...
  for (i = 0; i < size - 1; i++)
  {
    mc32_emitpcode(POC_MOVFW, popRegFromIdx(mc32_Gstack_base_addr - i));
...
genMult(iCode *ic)
{
  ...
  /* assign the amsops */
  mc32_aopOp(left, ic, FALSE);
  mc32_aopOp(right, ic, FALSE);
  mc32_aopOp(result, ic, TRUE);
...
genDivbits(operand *left, operand *right, operand *result)
{
  ...
  mc32_pic14_emitcode("mov", "b,%s", mc32_aopGet(AOP(right), 0, FALSE, FALSE));
  l = mc32_aopGet(AOP(left), 0, FALSE, FALSE);

  MOVA(l);

  mc32_pic14_emitcode("div", "ab");
  mc32_pic14_emitcode("rrc", "a");
  mc32_aopPut(AOP(result), "c", 0);
}
...
/* unsigned division */
#if 1
    mc32_mov2w(AOP(right), 0);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr));
    mc32_mov2w(AOP(left), 0);
    ...
    mc32_mov2w(AOP(left), 0);
    mc32_emitpcode(POC_MOVWF, temp);
    mc32_mov2w(AOP(right), 0);
    mc32_emitpcode(POC_CLRF, mc32_popGet(AOP(result), 0));

    mc32_emitpLabel(lbl->key);
    mc32_emitpcode(POC_INCF, mc32_popGet(AOP(result), 0));
    mc32_emitpcode(POC_SUBWF, temp);
    mc32_emitSKPNC;
    mc32_emitpcode(POC_GOTO, mc32_popGetLabel(lbl->key));
...
/* unsigned division */
    ...
    assert(!mc32_sameRegs(AOP(right), AOP(result)));
    ...
    if (!mc32_sameRegs(AOP(left), AOP(result)))
    {
      mc32_mov2w(AOP(left), 0);
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), 0));
    }
    ...
    mc32_emitpLabel(lbl->key);
    mc32_emitpcode(POC_SUBWF, mc32_popGet(AOP(result), 0));
    mc32_emitSKPNC;
    mc32_emitpcode(POC_GOTO, mc32_popGetLabel(lbl->key));
...
genIfxJump(iCode *ic, char *jval)
{
  ...
  if (IC_TRUE(ic))
  {
    ...
    {
      DEBUGmc32_pic14_emitcode("; ***", "%d - assuming %s is in bit space", __LINE__, jval);
      mc32_emitpcode(POC_BTFSC, mc32_newpCodeOpBit(jval, -1, 1));
    }

    mc32_emitpcode(POC_GOTO, mc32_popGetLabel(IC_TRUE(ic)->key));
    mc32_pic14_emitcode(" goto", "_%05d_DS_", labelKey2num(IC_TRUE(ic)->key + mc32_labelOffset));
  }
  else
  {
    ...
    {
      DEBUGmc32_pic14_emitcode("; ***", "%d - assuming %s is in bit space", __LINE__, jval);
      mc32_emitpcode(POC_BTFSS, mc32_newpCodeOpBit(jval, -1, 1));
    }

    mc32_emitpcode(POC_GOTO, mc32_popGetLabel(IC_FALSE(ic)->key));
    mc32_pic14_emitcode(" goto", "_%05d_DS_", labelKey2num(IC_FALSE(ic)->key + mc32_labelOffset));
  }
...
#define DEBUGpc mc32_emitpComment
...
/*-----------------------------------------------------------------*/
static void
mc32_mov2w_regOrLit(asmop *aop, unsigned long lit, int offset)
{
  if (aop->type == AOP_LIT)
  {
    mc32_emitpcode(POC_MOVLW, mc32_popGetLit((lit >> (offset * 8)) & 0x00FF));
  }
  else
  {
    mc32_emitpcode(POC_MOVFW, mc32_popGet(aop, offset));
...
/**********************************************************************
  ...
  if (AOP_TYPE (left) == AOP_CRY && AOP_TYPE (right) == AOP_CRY)
    {
      pCodeOp *pcleft = pic16_mc32_popGet (AOP (left), 0);
      pCodeOp *pcright = pic16_mc32_popGet (AOP (right), 0);
      assert (pcleft->type == PO_GPR_BIT && pcright->type == PO_GPR_BIT);

      mc32_emitSETC;
      // 1 < {0,1} is false --> clear C by skipping the next instruction
      //pic16_mc32_emitpcode (POC_BTFSS, pic16_popCopyGPR2Bit (AOP (left),0), PCORB (pcleft)->bit);
      pic16_mc32_emitpcode (POC_BTFSS, pic16_mc32_popGet (AOP (left), 0));
      // {0,1} < 0 is false --> clear C by NOT skipping the next instruction
      pic16_mc32_emitpcode (POC_BTFSS, pic16_popCopyGPR2Bit (pic16_mc32_popGet (AOP (right), 0), PCORB (pcright)->bit));
      mc32_emitCLRC;                 // only skipped for left=0 && right=1
    ...
    else
    {
      ...
      else if (lit == 0)
      {
        ...
        if (performedLt)
          ...
          mc32_emitCLRC;
        mc32_emitpcode(POC_BTFSS, mc32_newpCodeOpBit(mc32_aopGet(AOP(left), size, FALSE, FALSE), 7, 0));
        if (performedLt)
          mc32_emitCLRC;
        else
          mc32_emitSETC;
  ...
  {
    // unsigned comparison
    mc32_mov2w_regOrLit(AOP(right), lit, size);
    mc32_emitpcode(POC_SUBFW, mc32_popGet(AOP(left), size));
  }
  else
  {
    // signed comparison
    ...
    if (isAOP_LIT(right))
    {
      ...
      if (litbyte == 0x80)
      {
        ...
        mc32_mov2w(AOP(left), size);
        mc32_emitpcode(POC_XORLW, mc32_popGetLit(0x80)); // set ZERO flag
        mc32_emitSETC;
      }
      else
      {
        ...
        mc32_mov2w(AOP(left), size);
        mc32_emitpcode(POC_ADDLW, mc32_popGetLit(0x80));
        mc32_emitpcode(POC_ADDLW, mc32_popGetLit((0x100 - (litbyte + 0x80)) & 0x00FF));
      } // if
    }
    else
    {
      pCodeOp *pctemp = mc32_popGetTempReg();
      mc32_mov2w(AOP(left), size);
      mc32_emitpcode(POC_ADDLW, mc32_popGetLit(0x80));
      mc32_emitpcode(POC_MOVWF, pctemp);
      mc32_mov2w(AOP(right), size);
      mc32_emitpcode(POC_ADDLW, mc32_popGetLit(0x80));
      mc32_emitpcode(POC_SUBFW, pctemp);
  ...
  while (offs--)
  {
    ...
    mc32_emitSKPZ;
    mc32_emitpcode(POC_GOTO, mc32_popGetLabel(templbl->key));
    mc32_mov2w_regOrLit(AOP(right), lit, offs);
    mc32_emitpcode(POC_SUBFW, mc32_popGet(AOP(left), offs));
  } // while (offs)
  mc32_emitpLabel(templbl->key);
...
correct_result_in_carry:
  ...
  if (result && AOP_TYPE(result) != AOP_CRY)
  {
    ...
    while (size--)
    {
      mc32_emitpcode(POC_CLRF, mc32_popGet(AOP(result), size));
    } // while
    if (invert_result)
    {
      mc32_emitSKPC;
      mc32_emitpcode(POC_BSF, mc32_newpCodeOpBit(mc32_aopGet(AOP(result), 0, FALSE, FALSE), 0, 0));
    }
    else
    {
      mc32_emitpcode(POC_RLF, mc32_popGet(AOP(result), 0));
      if (ifx)
      {
        /* Result is expected to be in CARRY by genSkipc () below. */
        mc32_emitpcode(POC_RRFW, mc32_popGet(AOP(result), 0));
      } // if
    }   // if
  }     // if (result)
...
genCmpEq(iCode *ic, iCode *ifx)
{
  ...
  if (ifx)
    DEBUGmc32_pic14_emitcode("; ifx is non-null", "");
  else
    DEBUGmc32_pic14_emitcode("; ifx is null", "");

  mc32_aopOp((left = IC_LEFT(ic)), ic, FALSE);
  mc32_aopOp((right = IC_RIGHT(ic)), ic, FALSE);
  mc32_aopOp((result = IC_RESULT(ic)), ic, TRUE);
  ...
  assert(!mc32_sameRegs(AOP(result), AOP(right)));
  ...
  if (AOP_TYPE(right) == AOP_LIT)
  {
    ...
    case 0:
      mc32_mov2w(AOP(left), 0);
      for (i = 1; i < size; i++)
        mc32_emitpcode(POC_IORFW, mc32_popGet(AOP(left), i));
      /* now Z is set iff `left == right' */
      mc32_emitSKPZ;
      if (!false_label)
        false_label = newiTempLabel(NULL);
      mc32_emitpcode(POC_GOTO, mc32_popGetLabel(false_label->key));
      break;
    ...
    default:
      ...
      {
        mc32_mov2w(AOP(left), i);
        mc32_emitpcode(POC_XORLW, mc32_popGetLit(lit >> (8 * i)));
  ...
  else
  {
    ...
    {
      mc32_mov2w(AOP(right), i);
      mc32_emitpcode(POC_XORFW, mc32_popGet(AOP(left), i));
      /* now Z is cleared if `left != right' */
      mc32_emitSKPZ;
      if (!false_label)
        false_label = newiTempLabel(NULL);
...
genAndOp(iCode *ic)
{
  ...
  mc32_aopOp((left = IC_LEFT(ic)), ic, FALSE);
  mc32_aopOp((right = IC_RIGHT(ic)), ic, FALSE);
  mc32_aopOp((result = IC_RESULT(ic)), ic, FALSE);

  DEBUGmc32_pic14_AopType(__LINE__, left, right, result);

  mc32_emitpcode(POC_MOVFW, mc32_popGet(AOP(left), 0));
  mc32_emitpcode(POC_ANDFW, mc32_popGet(AOP(right), 0));
  mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), 0));

  /* if both are bit variables */
  /*     if (AOP_TYPE(left) == AOP_CRY && */
  /*         AOP_TYPE(right) == AOP_CRY ) { */
  /*         mc32_pic14_emitcode("mov","c,%s",AOP(left)->aopu.aop_dir); */
  /*         mc32_pic14_emitcode("anl","c,%s",AOP(right)->aopu.aop_dir); */
  /*         pic14_outBitC(result); */
  /*     } else { */
  /*         tlbl = newiTempLabel(NULL); */
  /*         pic14_toBoolean(left);     */
  /*         mc32_pic14_emitcode("jz","%05d_DS_",labelKey2num (tlbl->key)); */
...
genOrOp(iCode *ic)
{
  ...
  pic14_toBoolean(left);
  mc32_emitSKPZ;
  mc32_emitpcode(POC_GOTO, mc32_popGetLabel(tlbl->key));
  pic14_toBoolean(right);
...
jmpTrueOrFalse(iCode *ic, symbol *tlbl)
{
  ...
  if (IC_TRUE(ic))
  {
    ...
    mc32_pic14_emitcode("sjmp", "%05d_DS_", labelKey2num(nlbl->key));
    mc32_pic14_emitcode("", "%05d_DS_:", labelKey2num(tlbl->key));
    mc32_pic14_emitcode("ljmp", "%05d_DS_", labelKey2num(IC_TRUE(ic)->key));
    mc32_pic14_emitcode("", "%05d_DS_:", labelKey2num(nlbl->key));
  }
  else
  {
    mc32_pic14_emitcode("ljmp", "%05d_DS_", labelKey2num(IC_FALSE(ic)->key));
...
genAnd(iCode *ic, iCode *ifx)
{
  ...
  if (AOP_TYPE(left) == AOP_CRY)
  {
    ...
    if (AOP_TYPE(right) == AOP_LIT)
    {
      if (lit & 1)
      {
        if (size && mc32_sameRegs(AOP(result), AOP(left)))
          // no change
          goto release;
        mc32_pic14_emitcode("mov", "c,%s", AOP(left)->aopu.aop_dir);
      }
      else
      {
        ...
        {
          mc32_pic14_emitcode("clr", "%s", AOP(result)->aopu.aop_dir);
          goto release;
        }
    ...
    else
    {
      ...
      {
        // c = bit & bit;
        mc32_pic14_emitcode("mov", "c,%s", AOP(right)->aopu.aop_dir);
        mc32_pic14_emitcode("anl", "c,%s", AOP(left)->aopu.aop_dir);
      }
      else
      {
        // c = bit & val;
        MOVA(mc32_aopGet(AOP(right), 0, FALSE, FALSE));
        // c = lsb
        mc32_pic14_emitcode("rrc", "a");
        mc32_pic14_emitcode("anl", "c,%s", AOP(left)->aopu.aop_dir);
      }
  ...
  if ((AOP_TYPE(right) == AOP_LIT) && (AOP_TYPE(result) == AOP_CRY) && (AOP_TYPE(left) != AOP_CRY))
  {
    ...
    {
      posbit--;
      //MOVA(mc32_aopGet(AOP(left),posbit>>3,FALSE,FALSE));
      // bit = left & 2^n
      if (size)
        mc32_pic14_emitcode("mov", "c,acc.%d", posbit & 0x07);
      // if(left &  2^n)
      else
      {
        if (ifx)
  ...
  if (mc32_sameRegs(AOP(result), AOP(left)))
  {
    int know_W = -1;
    for (; size--; offset++, lit >>= 8)
    {
      if (AOP_TYPE(right) == AOP_LIT)
      {
        ...
        default:
        {
          int p = mc32_my_powof2((~lit) & 0xff);
          if (p >= 0)
          {
            /* only one bit is set in the literal, so use a bcf instruction */
            mc32_emitpcode(POC_BCF, mc32_newpCodeOpBit(mc32_aopGet(AOP(left), offset, FALSE, FALSE), p, 0));
          }
          else
          {
            ...
            know_W = lit & 0xff;
            mc32_emitpcode(POC_ANDWF, mc32_popGet(AOP(left), offset));
          }
        }
        }
      }
  ...
  else
  {
    ...
    if (AOP_TYPE(result) == AOP_CRY)
    {
      ...
      while (sizer--)
      {
        MOVA(mc32_aopGet(AOP(right), offset, FALSE, FALSE));
        mc32_pic14_emitcode("anl", "a,%s", mc32_aopGet(AOP(left), offset, FALSE, FALSE));
        mc32_pic14_emitcode("jnz", "%05d_DS_", labelKey2num(tlbl->key));
        offset++;
      }
      if (size)
      ...
      for (; (size--); offset++)
      {
        ...
        if (AOP_TYPE(right) == AOP_LIT)
        {
          ...
          case 0x00:
            ...
            break;
          case 0xff:
            mc32_emitpcode(POC_MOVFW, mc32_popGet(AOP(left), offset));
            mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offset));
            break;
          default:
            mc32_emitpcode(POC_MOVLW, mc32_popGetLit(t));
            mc32_emitpcode(POC_ANDFW, mc32_popGet(AOP(left), offset));
            mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offset));
          }
          continue;
        }
...
genOr(iCode *ic, iCode *ifx)
{
  ...
  if (AOP_TYPE(left) == AOP_CRY)
  {
    if (AOP_TYPE(right) == AOP_LIT)
    {
      ...
      else
      {
        ...
        if (size && mc32_sameRegs(AOP(result), AOP(left)))
          goto release;
        mc32_pic14_emitcode(";XXX mov", "c,%s  %s,%d", AOP(left)->aopu.aop_dir, __FILE__, __LINE__);
      }
    }
    else
    {
      if (AOP_TYPE(right) == AOP_CRY)
      {
        if (mc32_sameRegs(AOP(result), AOP(left)))
        {
          ...
          mc32_pic14_emitcode("bcf", "(%s >> 3), (%s & 7)", AOP(result)->aopu.aop_dir, AOP(result)->aopu.aop_dir);
          mc32_pic14_emitcode("btfsc", "(%s >> 3), (%s & 7)", AOP(right)->aopu.aop_dir, AOP(right)->aopu.aop_dir);
          mc32_pic14_emitcode("bsf", "(%s >> 3), (%s & 7)", AOP(result)->aopu.aop_dir, AOP(result)->aopu.aop_dir);
        }
        else
        {
          mc32_emitpcode(POC_BCF, mc32_popGet(AOP(result), 0));
          mc32_emitpcode(POC_BTFSS, mc32_popGet(AOP(right), 0));
          mc32_emitpcode(POC_BTFSC, mc32_popGet(AOP(left), 0));
          mc32_emitpcode(POC_BSF, mc32_popGet(AOP(result), 0));
        }
      }
      else
      {
        // c = bit | val;
        symbol *tlbl = newiTempLabel(NULL);
        mc32_pic14_emitcode(";XXX ", " %s,%d", __FILE__, __LINE__);

        mc32_emitpcode(POC_BCF, mc32_popGet(AOP(result), 0));

        if (!((AOP_TYPE(result) == AOP_CRY) && ifx))
          mc32_pic14_emitcode(";XXX setb", "c");
        mc32_pic14_emitcode(";XXX jb", "%s,%05d_DS_", AOP(left)->aopu.aop_dir, labelKey2num(tlbl->key));
        pic14_toBoolean(right);
        mc32_pic14_emitcode(";XXX jnz", "%05d_DS_", labelKey2num(tlbl->key));
        if ((AOP_TYPE(result) == AOP_CRY) && ifx)
        {
          jmpTrueOrFalse(ifx, tlbl);
          goto release;
  ...
  if ((AOP_TYPE(right) == AOP_LIT) && (AOP_TYPE(result) == AOP_CRY) && (AOP_TYPE(left) != AOP_CRY))
  {
    if (lit)
    {
      ...
      // result = 1
      if (size)
        mc32_pic14_emitcode(";XXX setb", "%s", AOP(result)->aopu.aop_dir);
      else
        continueIfTrue(ifx);
      goto release;
    }
      ...
      if (AOP_TYPE(right) == AOP_LIT)
      {
        ...
        else
        {
          ...
          if (p >= 0)
          {
            ...
            mc32_emitpcode(POC_BSF, mc32_newpCodeOpBit(mc32_aopGet(AOP(left), offset, FALSE, FALSE), p, 0));
          }
          else
          {
            if (know_W != (int)(lit & 0xff))
              mc32_emitpcode(POC_MOVLW, mc32_popGetLit(lit & 0xff));
            know_W = lit & 0xff;
            mc32_emitpcode(POC_IORWF, mc32_popGet(AOP(left), offset));
          }
        }
      }
      else
  ...
  else
  {
    ...
    if (AOP_TYPE(result) == AOP_CRY)
    {
      ...
      {
        MOVA(mc32_aopGet(AOP(right), offset, FALSE, FALSE));
        mc32_pic14_emitcode(";XXX orl", "a,%s", mc32_aopGet(AOP(left), offset, FALSE, FALSE));
        mc32_pic14_emitcode(";XXX jnz", "%05d_DS_", labelKey2num(tlbl->key));
        offset++;
      }
      if (size)
      ...
      for (; (size--); offset++)
      {
        ...
        if (AOP_TYPE(right) == AOP_LIT)
        {
          ...
          default:
            mc32_emitpcode(POC_MOVLW, mc32_popGetLit(t));
            mc32_emitpcode(POC_IORFW, mc32_popGet(AOP(left), offset));
            mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offset));
          }
          continue;
        }
  ...
  if (AOP_TYPE(left) == AOP_CRY)
  {
    if (AOP_TYPE(right) == AOP_LIT)
    {
      ...
      if (lit >> 1)
      {
        ...
        if (AOP_TYPE(result) == AOP_CRY)
        {
          if (size)
          {
            mc32_emitpcode(POC_BSF, mc32_popGet(AOP(result), offset));
            mc32_pic14_emitcode("setb", "%s", AOP(result)->aopu.aop_dir);
          }
          else if (ifx)
            continueIfTrue(ifx);
          goto release;
      ...
      else
      {
        ...
        else
        {
          ...
          if (size && mc32_sameRegs(AOP(result), AOP(left)))
          {
            mc32_emitpcode(POC_MOVLW, mc32_popGet(AOP(result), offset));
            mc32_emitpcode(POC_XORWF, mc32_popGet(AOP(result), offset));
            mc32_pic14_emitcode("cpl", "%s", AOP(result)->aopu.aop_dir);
            goto release;
          }
          else
          {
    ...
    else
    {
      ...
      else
      {
        ...
        while (sizer)
        {
          MOVA(mc32_aopGet(AOP(right), sizer - 1, FALSE, FALSE));
          if (sizer == 1)
            // test the msb of the lsb
            mc32_pic14_emitcode("anl", "a,#0xfe");
          mc32_pic14_emitcode("jnz", "%05d_DS_", labelKey2num(tlbl->key));
          sizer--;
        }
        // val = (0,1)
      ...
      mc32_pic14_emitcode("jnb", "%s,%05d_DS_", AOP(left)->aopu.aop_dir, (labelKey2num(tlbl->key)));
      mc32_pic14_emitcode("cpl", "c");
      mc32_pic14_emitcode("", "%05d_DS_:", (labelKey2num(tlbl->key)));
    }
    // bit = c
  ...
  if (mc32_sameRegs(AOP(result), AOP(left)))
  {
    ...
    for (; size--; offset++)
    {
      if (AOP_TYPE(right) == AOP_LIT)
      {
        ...
        {
          mc32_emitpcode(POC_MOVLW, mc32_popGetLit(t));
          mc32_emitpcode(POC_XORWF, mc32_popGet(AOP(left), offset));
        }
      }
      else
      {
  ...
  else
  {
    ...
    if (AOP_TYPE(result) == AOP_CRY)
    {
      ...
      while (sizer--)
      {
        ...
        {
          MOVA(mc32_aopGet(AOP(left), offset, FALSE, FALSE));
        }
        else
        {
          MOVA(mc32_aopGet(AOP(right), offset, FALSE, FALSE));
          mc32_pic14_emitcode("xrl", "a,%s", mc32_aopGet(AOP(left), offset, FALSE, FALSE));
        }
        mc32_pic14_emitcode("jnz", "%05d_DS_", labelKey2num(tlbl->key));
        offset++;
      }
      ...
      for (; (size--); offset++)
      {
        ...
        if (AOP_TYPE(right) == AOP_LIT)
        {
          ...
          default:
            mc32_emitpcode(POC_MOVLW, mc32_popGetLit(t));
            mc32_emitpcode(POC_XORFW, mc32_popGet(AOP(left), offset));
            mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offset));
          }
          continue;
        }
...
pic14_genInline(iCode *ic)
{
  ...
  while (*bp)
  {
    ...
    case '\n':
      ...
      *bp++ = '\0';
      if (*bp1)
        mc32_addpCode2pBlock(pb, mc32_newpCodeAsmDir(bp1, NULL)); // inline directly, no process
      bp1 = bp;
      break;
    ...
    default:
      ...
      if (!inComment && (*bp == ':') && (isspace((unsigned char)bp[1])))
      {
        ...
        *bp = '\0';
        ...
        mc32_addpCode2pBlock(pb, mc32_newpCodeAsmDir(NULL, bp1)); // inline directly, no process
        bp1 = bp;
      }
      else
        ++bp;
...
genRRC(iCode *ic)
{
  ...
  DEBUGmc32_pic14_AopType(__LINE__, left, NULL, result);

  same = mc32_sameRegs(AOP(result), AOP(left));

  size = AOP_SIZE(result);

  /* get the lsb and put it into the carry */
  mc32_emitpcode(POC_RRFW, mc32_popGet(AOP(left), size - 1));
  ...
  while (size--)
  {
    ...
    if (same)
    {
      mc32_emitpcode(POC_RRF, mc32_popGet(AOP(left), offset));
    }
    else
    {
      mc32_emitpcode(POC_RRFW, mc32_popGet(AOP(left), offset));
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offset));
    }
  ...
  mc32_freeAsmop(left, NULL, ic, TRUE);
  mc32_freeAsmop(result, NULL, ic, TRUE);
}
...
genRLC(iCode *ic)
{
  ...
  mc32_emitpcode(POC_RLFW, mc32_popGet(AOP(left), size - 1));
  ...
  while (size--)
  {
    ...
    if (same)
    {
      mc32_emitpcode(POC_RLF, mc32_popGet(AOP(left), offset));
    }
    else
    {
      mc32_emitpcode(POC_RLFW, mc32_popGet(AOP(left), offset));
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offset));
    }
...
genGetABit(iCode *ic)
{
  ...
  mc32_mov2w(AOP(left), offset);
  mc32_emitpcode(POC_ANDLW, mc32_popGetLit(1 << shCount));
...
genGetHbit(iCode *ic)
{
  ...
  MOVA(mc32_aopGet(AOP(left), AOP_SIZE(left) - 1, FALSE, FALSE));
  if (AOP_TYPE(result) == AOP_CRY)
  {
    mc32_pic14_emitcode("rlc", "a");
    pic14_outBitC(result);
  ...
  {
    mc32_pic14_emitcode("rl", "a");
    mc32_pic14_emitcode("anl", "a,#0x01");
    mc32_outAcc(result);
  }
...
AccLsh(pCodeOp *pcop, int shCount)
{
  ...
  case 6:
    ...
    break;
  case 7:
    mc32_emitpcode(POC_RRFW, pcop);
    mc32_emitpcode(POC_RRF, pcop);
    break;
  }
  /* clear invalid bits */
  mc32_emitpcode(POC_MOVLW, mc32_popGetLit((unsigned char)(~((1UL << shCount) - 1))));
  mc32_emitpcode(POC_ANDWF, pcop);
}
...
AccRsh(pCodeOp *pcop, int shCount, int mask_mode)
{
  ...
  case 7:
    if (mask_mode == 2)
    {
      /* load sign */
      mc32_emitpcode(POC_RLFW, pcop);
      mc32_emitpcode(POC_CLRF, pcop);
      mc32_emitSKPNC;
      mc32_emitpcode(POC_COMF, pcop);
      return;
    }
    else
    {
  ...
  mc32_emitpcode(POC_MOVLW, mc32_popGetLit(0x00ff >> shCount));
  mc32_emitpcode(POC_ANDWF, pcop);

  if (mask_mode == 2)
  {
    /* sign-extend */
    mc32_emitpcode(POC_MOVLW, mc32_popGetLit(0x00ff << (8 - shCount)));
    mc32_emitpcode(POC_BTFSC, mc32_newpCodeOpBit(mc32_get_op(pcop, NULL, 0), 7 - shCount, 0));
    mc32_emitpcode(POC_IORWF, pcop);
  }
}
...
movLeft2Result(operand *left, int offl, operand *result, int offr)
{
  FENTRY;
  DEBUGmc32_pic14_emitcode("; ***", "%s  %d", __FUNCTION__, __LINE__);
  if (!mc32_sameRegs(AOP(left), AOP(result)) || (offl != offr))
  {
    mc32_aopGet(AOP(left), offl, FALSE, FALSE);

    mc32_emitpcode(POC_MOVFW, mc32_popGet(AOP(left), offl));
    mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offr));
  }
}
...
shiftLeft_Left2ResultLit(operand *left, operand *result, int shCount)
{
  ...
  same = mc32_sameRegs(AOP(left), AOP(result));
  ...
  case 1: /* takes 1N+1 or 2N+1 cycles (or offr==0) */
    ...
    else
    {
      ...
      for (i = 0; i < size; i++)
      {
        if (same && !offr)
        {
          mc32_emitpcode(POC_RLF, mc32_popGet(AOP(left), i));
        }
        else
        {
          mc32_emitpcode(POC_RLFW, mc32_popGet(AOP(left), i));
          mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), i + offr));
        } // if
      }   // for
    }     // if (offr)
    break;
  ...
  case 4: /* takes 3+5(N-1) = 5N-2 cycles (for offr==0) */
    ...
    mc32_emitpcode(POC_SWAPFW, mc32_popGet(AOP(left), size - 1));
    mc32_emitpcode(POC_ANDLW, mc32_popGetLit(0xF0));
    mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), size - 1 + offr));

    for (i = size - 2; i >= 0; i--)
    {
      mc32_emitpcode(POC_SWAPFW, mc32_popGet(AOP(left), i));
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), i + offr));
      mc32_emitpcode(POC_ANDLW, mc32_popGetLit(0x0F));
      mc32_emitpcode(POC_IORWF, mc32_popGet(AOP(result), i + offr + 1));
      mc32_emitpcode(POC_XORWF, mc32_popGet(AOP(result), i + offr));
    } // for i
    break;
  ...
  case 7: /* takes 2(N-1)+3 = 2N+1 cycles */
    ...
    for (i = size - 2; i >= 0; i--)
    {
      mc32_emitpcode(POC_RRFW, mc32_popGet(AOP(left), i));
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offr + i + 1));
    } // for i
    mc32_emitpcode(POC_CLRF, mc32_popGet(AOP(result), offr));
    mc32_emitpcode(POC_RRF, mc32_popGet(AOP(result), offr));
    break;
  ...
  if (size)
  {
    ...
    case 1: /* takes 1N+1(3) or 2N+1(3) cycles (or offr==0) */
      ...
      else
      {
        ...
        if (sign)
        {
          mc32_emitpcode(POC_BTFSC, mc32_newpCodeOpBit(mc32_aopGet(AOP(left), AOP_SIZE(left) - 1, FALSE, FALSE), 7, 0));
          mc32_emitSETC;
        }
        for (i = size - 1; i >= 0; i--)
        {
          if (same && !offr)
          {
            mc32_emitpcode(POC_RRF, mc32_popGet(AOP(left), i));
          }
          else
          {
            mc32_emitpcode(POC_RRFW, mc32_popGet(AOP(left), i + offr));
            mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), i));
          }
        } // for i
      }   // if (offr)
      break;
    ...
    case 4: /* takes 3(6)+5(N-1) = 5N-2(+1) cycles (for offr==0) */
      /* works in-place/with offr as well */
      mc32_emitpcode(POC_SWAPFW, mc32_popGet(AOP(left), offr));
      mc32_emitpcode(POC_ANDLW, mc32_popGetLit(0x0F));
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), 0));

      for (i = 1; i < size; i++)
      {
        mc32_emitpcode(POC_SWAPFW, mc32_popGet(AOP(left), i + offr));
        mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), i));
        mc32_emitpcode(POC_ANDLW, mc32_popGetLit(0xF0));
        mc32_emitpcode(POC_IORWF, mc32_popGet(AOP(result), i - 1));
        mc32_emitpcode(POC_XORWF, mc32_popGet(AOP(result), i));
      } // for i

      if (sign)
      {
        mc32_emitpcode(POC_MOVLW, mc32_popGetLit(0xF0));
        mc32_emitpcode(POC_BTFSC, mc32_newpCodeOpBit(mc32_aopGet(AOP(result), size - 1, FALSE, FALSE), 3, 0));
        mc32_emitpcode(POC_IORWF, mc32_popGet(AOP(result), size - 1));
      } // if
      break;

    case 7: /* takes 2(N-1)+3(4) = 2N+1(2) cycles */
      /* works in-place/with offr as well */
      mc32_emitpcode(POC_RLFW, mc32_popGet(AOP(left), offr));
      for (i = 0; i < size - 1; i++)
      {
        mc32_emitpcode(POC_RLFW, mc32_popGet(AOP(left), offr + i + 1));
        mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), i));
      } // for i
      mc32_emitpcode(POC_CLRF, mc32_popGet(AOP(result), size - 1));
      if (!sign)
      {
        mc32_emitpcode(POC_RLF, mc32_popGet(AOP(result), size - 1));
      }
      else
      {
        mc32_emitSKPNC;
        mc32_emitpcode(POC_DECF, mc32_popGet(AOP(result), size - 1));
      }
      break;
...
genMultiAsm(PIC_OPCODE poc, operand *reg, int size, int endian)
{
  ...
  while (size--)
  {
    mc32_emitpcode(poc, mc32_popGet(AOP(reg), offset));
    offset += endian;
  }
}
...
loadSignToC(operand *op)
{
  ...
  mc32_emitCLRC;
  mc32_emitpcode(POC_BTFSC, mc32_newpCodeOpBit(mc32_aopGet(AOP(op), AOP_SIZE(op) - 1, FALSE, FALSE), 7, 0));
  mc32_emitSETC;
}
...
genGenericShift(iCode *ic, int shiftRight)
{
  ...
  if (!mc32_sameRegs(AOP(left), AOP(result)))
  {
    ...
    mc32_addSign(result, AOP_SIZE(left), !SPEC_USIGN(operandType(left)));
  }
  ...
  mc32_emitSKPNZ;
  mc32_emitpcode(POC_GOTO, mc32_popGetLabel(tlbl1->key));
  mc32_emitpLabel(tlbl->key);
  ...
  mc32_emitpcode(POC_ADDLW, mc32_popGetLit(1)); /* clears CARRY (unless W==0 afterwards) */
  mc32_emitSKPC;
  mc32_emitpcode(POC_GOTO, mc32_popGetLabel(tlbl->key));
  ...
  if (!SPEC_USIGN(operandType(right)))
  {
    ...
    mc32_emitpLabel(inverselbl->key);
    if (!shiftRight || SPEC_USIGN(operandType(result)))
      mc32_emitCLRC;
    mc32_emitpLabel(inv_loop->key);
    /* propagate the sign bit inwards for SIGNED result */
    if (shiftRight && !SPEC_USIGN(operandType(result)))
      loadSignToC(result);
    genMultiAsm(shiftRight ? POC_RRF : POC_RLF, result, size, shiftRight);
    mc32_emitpcode(POC_ADDLW, mc32_popGetLit(1));
    mc32_emitSKPC;
    mc32_emitpcode(POC_GOTO, mc32_popGetLabel(inv_loop->key));
  } // if

  mc32_emitpLabel(tlbl1->key);
...
inc_fsr(int delta)
{
  ...
  if (mc32_getPIC()->isEnhancedCore)
  {
    ...
    mc32_addpCode2pBlock(pb, mc32_newpCodeAsmDir("ADDFSR", "FSR0, %d", delta));
  }
  else
  {
    switch (delta)
...
emitPtrByteGet(operand *src, int p_type, bool alreadyAddressed)
{
  ...
  case FPOINTER:
    if (!alreadyAddressed)
      setup_fsr(src);
    mc32_emitpcode(POC_MOVFW, mc32_popCopyReg(mc32_pc_indf));
    break;

  case CPOINTER:
    assert(AOP_SIZE(src) == 2);
    mc32_mov2w_op(src, 0);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr - 1));
    mc32_mov2w_op(src, 1);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr));
    mc32_emitpcode(POC_MOVLW, mc32_popGetLit(GPTRTAG_CODE)); /* GPOINTER tag for __code space */
...
emitPtrByteSet(operand *dst, int p_type, bool alreadyAddressed)
{
  ...
  case FPOINTER:
    if (!alreadyAddressed)
      setup_fsr(dst);
    mc32_emitpcode(POC_MOVWF, mc32_popCopyReg(mc32_pc_indf));
    break;
  ...
  case GPOINTER:
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr - 2));
    mc32_mov2w_op(dst, 0);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr - 1));
    mc32_mov2w_op(dst, 1);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr));
...
genUnpackBits(operand *result, operand *left, int ptype, iCode *ifx)
{
  ...
  if (blen == 1)
  {
    if (ifx)
    {
      ...
      resolveIfx(&rIfx, ifx);
      if (ptype == -1) /* direct */
        pcop = mc32_newpCodeOpBit(mc32_aopGet(AOP(left), 0, FALSE, FALSE), bstr, 0);
      else
      {
        setup_fsr(left);
        pcop = mc32_newpCodeOpBit(mc32_pc_indf->pcop.name, bstr, 0);
      }
  ...
  else if (blen <= 8 && ((blen + bstr) <= 8))
  {
    ...
    if (blen < 8)
      mc32_emitpcode(POC_ANDLW, mc32_popGetLit((((1UL << blen) - 1) << bstr) & 0x00ff));
    movwf(AOP(result), 0);
    AccRsh(mc32_popGet(AOP(result), 0), bstr, 1); /* zero extend the bitfield */

    if (!SPEC_USIGN(OP_SYM_ETYPE(left)) && (bstr + blen != 8))
    {
      /* signed bitfield */
      assert(bstr + blen > 0);
      mc32_emitpcode(POC_MOVLW, mc32_popGetLit(0x00ff << (bstr + blen)));
      mc32_emitpcode(POC_BTFSC, mc32_newpCodeOpBit(mc32_aopGet(AOP(result), 0, FALSE, FALSE), bstr + blen - 1, 0));
      mc32_emitpcode(POC_IORWF, mc32_popGet(AOP(result), 0));
    }
    mc32_addSign(result, 1, !SPEC_USIGN(OP_SYM_ETYPE(left)));
    return;
  }
...
/*-----------------------------------------------------------------*/
/* genNearPointerGet - mc32_pic14_emitcode for near pointer fetch       */
/*-----------------------------------------------------------------*/
static void
genNearPointerGet(operand *left, operand *result, iCode *ic)
{
  ...
  /* if bitfield then unpack the bits */
  {
    ...
    {
      if (direct)
        mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(left), 0));
      else
        mc32_emitpcode(POC_MOVFW, mc32_popCopyReg(mc32_pc_indf));
      if (AOP_TYPE(result) == AOP_LIT)
      {
...
genGenPointerGet(operand *left, operand *result, iCode *ic)
{
...

  {
    ...
    mc32_mov2w(AOP(left), 0);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr - 1));
    mc32_mov2w(AOP(left), 1);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr));
    mc32_mov2w(AOP(left), 2);
    call_libraryfunc(func[size]);
    ...
    while (size--)
    {
      mc32_emitpcode(POC_MOVFW, popRegFromIdx(mc32_Gstack_base_addr - idx++));
      movwf(AOP(result), size);
    } // while
  }
...
//sym_link *retype = getSpec(operandType(result));
  ...
  DEBUGmc32_pic14_emitcode("; ", " %d getting const pointer", __LINE__);

  lit = mc32_op_isLitLike(left);
...

  {
    ...
    for (i = 1; i < size; i++)
    {
      mc32_emitpcode(POC_MOVFW, popRegFromIdx(mc32_Gstack_base_addr + 1 - i));
...
genPointerGet(iCode *ic)
{
  ...
  if (IS_PTR_CONST(type))
    DEBUGmc32_pic14_emitcode("; ***", "%d - const pointer", __LINE__);
  ...
  else
  {
    ...
    DEBUGmc32_pic14_emitcode("; ***", "%d - resolve pointer by storage class", __LINE__);

    if (SPEC_OCLS(etype)->codesp)
    {
      DEBUGmc32_pic14_emitcode("; ***", "%d - cpointer", __LINE__);
      //p_type = CPOINTER ;
    }
    else if (SPEC_OCLS(etype)->fmap && !SPEC_OCLS(etype)->paged)
      DEBUGmc32_pic14_emitcode("; ***", "%d - fpointer", __LINE__);
    /*p_type = FPOINTER ; */
    else if (SPEC_OCLS(etype)->fmap && SPEC_OCLS(etype)->paged)
      DEBUGmc32_pic14_emitcode("; ***", "%d - ppointer", __LINE__);
    /*        p_type = PPOINTER; */
    else if (SPEC_OCLS(etype) == idata)
      DEBUGmc32_pic14_emitcode("; ***", "%d - ipointer", __LINE__);
    /*      p_type = IPOINTER; */
    else
      DEBUGmc32_pic14_emitcode("; ***", "%d - pointer", __LINE__);
    /*      p_type = POINTER ; */
  }
  ...
  if ((blen <= 8) && ((bstr + blen) <= 8))
  {
    ...
    if (AOP_TYPE(right) == AOP_LIT)
    {
      ...
      if (blen == 1)
      {
        ...
        case -1:
          if (AOP(result)->type == AOP_PCODE)
            pcop = mc32_newpCodeOpBit(mc32_aopGet(AOP(result), 0, FALSE, FALSE), bstr, 0);
          else
            pcop = mc32_popGet(AOP(result), 0);
          mc32_emitpcode(lit ? POC_BSF : POC_BCF, pcop);
          break;

        case POINTER:
        case FPOINTER:
          setup_fsr(result);
          mc32_emitpcode(lit ? POC_BSF : POC_BCF, mc32_newpCodeOpBit(PCOP(mc32_pc_indf)->name, bstr, 0));
          break;
        ...
        case GPOINTER:
          ...
          if (lit)
          {
            mc32_emitpcode(POC_IORLW, mc32_newpCodeOpLit(1UL << bstr));
          }
          else
          {
            mc32_emitpcode(POC_ANDLW, mc32_newpCodeOpLit((~(1UL << bstr)) & 0x0ff));
          }
      ...
      else
      {
        ...
        {
        case -1:
          mc32_mov2w(AOP(result), 0);
          if ((litval | mask) != 0x00ff)
            mc32_emitpcode(POC_ANDLW, mc32_popGetLit(mask));
          if (litval != 0x00)
            mc32_emitpcode(POC_IORLW, mc32_popGetLit(litval));
          movwf(AOP(result), 0);
          break;
    ...
    else
    {
      ...
      if (blen == 1)
      {
        ...
        case -1:
          ...
          mc32_emitpcode(POC_RRFW, mc32_popGet(AOP(right), 0));
          mc32_emitSKPC;
          mc32_emitpcode(POC_BCF, mc32_newpCodeOpBit(mc32_aopGet(AOP(result), 0, FALSE, FALSE), bstr, 0));
          mc32_emitSKPNC;
          mc32_emitpcode(POC_BSF, mc32_newpCodeOpBit(mc32_aopGet(AOP(result), 0, FALSE, FALSE), bstr, 0));
          break;
        ...
        case GPOINTER:
          emitPtrByteGet(result, p_type, FALSE);
          mc32_emitpcode(POC_BTFSS, mc32_newpCodeOpBit(mc32_aopGet(AOP(right), 0, FALSE, FALSE), bstr, 0));
          mc32_emitpcode(POC_ANDLW, mc32_newpCodeOpLit(~(1UL << bstr) & 0x0ff));
          mc32_emitpcode(POC_BTFSC, mc32_newpCodeOpBit(mc32_aopGet(AOP(right), 0, FALSE, FALSE), bstr, 0));
          mc32_emitpcode(POC_IORLW, mc32_newpCodeOpLit((1UL << bstr) & 0x0ff));
          emitPtrByteSet(result, p_type, TRUE);
          break;
      ...
      else
      {
        ...
        pCodeOp *temp = mc32_popGetTempReg();

        mc32_mov2w(AOP(right), 0);
        if (blen < 8)
        {
          mc32_emitpcode(POC_ANDLW, mc32_popGetLit((1UL << blen) - 1));
        }
        mc32_emitpcode(POC_MOVWF, temp);
        if (bstr)
        {
...
/*-----------------------------------------------------------------*/
/* genNearPointerSet - mc32_pic14_emitcode for near pointer put         */
/*-----------------------------------------------------------------*/
static void
genNearPointerSet(operand *right, operand *result, iCode *ic)
{
  ...
  DEBUGmc32_pic14_emitcode("; ***", "%s  %d", __FUNCTION__, __LINE__);
  mc32_aopOp(right, ic, FALSE);
  DEBUGmc32_pic14_AopType(__LINE__, NULL, right, result);
...

  {
    ...
    while (size--)
    {
      char *l = mc32_aopGet(AOP(right), offset, FALSE, TRUE);
      if (*l == '@')
      {
        mc32_emitpcode(POC_MOVFW, mc32_popCopyReg(mc32_pc_indf));
      }
      else
      {
        if (AOP_TYPE(right) == AOP_LIT)
        {
          mc32_emitpcode(POC_MOVLW, mc32_popGet(AOP(right), offset));
        }
        else
        {
          mc32_emitpcode(POC_MOVFW, mc32_popGet(AOP(right), offset));
...
genGenPointerSet(operand *right, operand *result, iCode *ic)
{
  ...
  DEBUGmc32_pic14_AopType(__LINE__, right, NULL, result);
...

  {
    ...
    /* pass arguments */
    /* - value (MSB in mc32_Gstack_base_addr-2, growing downwards) */
    {
      int off = size;
      idx = 2;
      while (off--)
      {
        mc32_mov2w_op(right, off);
        mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr - idx++));
      }
      idx = 0;
    }
    /* - address */
    ...
    mc32_mov2w(AOP(result), 0);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr - 1));
    mc32_mov2w(AOP(result), 1);
    mc32_emitpcode(POC_MOVWF, popRegFromIdx(mc32_Gstack_base_addr));
    mc32_mov2w(AOP(result), 2);
    call_libraryfunc(func[size]);
  }
...
genIfx(iCode *ic, iCode *popIc)
{
  ...
  if (isbit)
  {
    ...
    {
      assert(!IC_FALSE(ic));
      mc32_emitpcode(POC_BTFSC, mc32_popGet(AOP(cond), 0));
      //mc32_emitSKPNC;
      mc32_emitpcode(POC_GOTO, mc32_popGetLabel(IC_TRUE(ic)->key));
    }
    else
    {
      assert(IC_FALSE(ic));
      mc32_emitpcode(POC_BTFSS, mc32_popGet(AOP(cond), 0));
      //mc32_emitSKPC;
      mc32_emitpcode(POC_GOTO, mc32_popGetLabel(IC_FALSE(ic)->key));
    }
    if (0)
...
/* sanity check: generic pointers to code space are not yet supported,
  ...
  while (size--)
  {
    ...
    mc32_emitpcode(POC_MOVLW, mc32_popGetImmd(OP_SYMBOL(left)->rname, offset, 0, IS_FUNC(OP_SYM_TYPE(left))));
    mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), offset));

#if 0
      mc32_emitpcode (POC_MOVLW, mc32_popGet (AOP (left), offset));
      mc32_emitpcode (POC_MOVWF, mc32_popGet (AOP (result), offset));
#endif
    offset++;
  }
  ...
  if (IS_SYMOP(result) && IS_GENPTR(OP_SYM_TYPE(result)))
  {
    ...
    mc32_emitpcode(POC_MOVLW, mc32_popGetLit(isCode ? GPTRTAG_CODE : GPTRTAG_DATA));
    movwf(AOP(result), 2);
  }

  mc32_freeAsmop(left, NULL, ic, FALSE);
  mc32_freeAsmop(result, NULL, ic, TRUE);
}
...
genAssign(iCode *ic)
{
  ...
  if (mc32_sameRegs(AOP(right), AOP(result)))
    goto release;
  ...
  {
    mc32_emitpComment("genAssign from CODESPACE");
  ...
  if (AOP_TYPE(result) == AOP_CRY)
  {
    ...
    if (AOP_TYPE(right) == AOP_LIT)
    {
      ...
      mc32_emitpcode((((int)operandLitValue(right)) ? POC_BSF : POC_BCF), mc32_popGet(AOP(result), 0));
  ...
  if (AOP_TYPE(right) == AOP_DIR && (AOP_TYPE(result) == AOP_REG) && size == 1)
  {
    ...
    if (aopIdx(AOP(result), 0) == 4)
    {
      ...
      DEBUGmc32_pic14_emitcode("; WARNING", "%s  %d ignoring register storage", __FUNCTION__, __LINE__);
  }
...
release:
  mc32_freeAsmop(right, NULL, ic, FALSE);
  mc32_freeAsmop(result, NULL, ic, TRUE);
}
...
genJumpTab(iCode *ic)
{
  ...
  mc32_aopOp(IC_JTCOND(ic), ic, FALSE);
  /* get the condition into accumulator */
  l = mc32_aopGet(AOP(IC_JTCOND(ic)), 0, FALSE, FALSE);
  MOVA(l);
  /* multiply by three */
  mc32_pic14_emitcode("add", "a,acc");
  mc32_pic14_emitcode("add", "a,%s", mc32_aopGet(AOP(IC_JTCOND(ic)), 0, FALSE, FALSE));

  jtab = newiTempLabel(NULL);
  mc32_pic14_emitcode("mov", "dptr,#%05d_DS_", labelKey2num(jtab->key));
  mc32_pic14_emitcode("jmp", "@a+dptr");
  mc32_pic14_emitcode("", "%05d_DS_:", labelKey2num(jtab->key));

  // zwr 1.0.0, revise for switch

  // //mc32_emitpcode(POC_MOVLW, mc32_popGetHighLabel(jtab->key));
  // //mc32_emitpcode(POC_MOVWF, mc32_popCopyReg(&mc32_pc_pclath));
  // mc32_emitpcode(POC_MOVLW, mc32_popGetLabel(jtab->key));
  // mc32_emitpcode(POC_ADDFW, mc32_popGet(AOP(IC_JTCOND(ic)), 0));
  // //mc32_emitSKPNC;
  ...
  mc32_freeAsmop(IC_JTCOND(ic), NULL, ic, TRUE);

  /* now generate the jump labels */
  for (jtab = setFirstItem(IC_JTLABELS(ic)); jtab; jtab = setNextItem(IC_JTLABELS(ic)))
  {
    mc32_pic14_emitcode("ljmp", "%05d_DS_", labelKey2num(jtab->key));
    mc32_emitpcode(POC_GOTO, mc32_popGetLabel(jtab->key));
  }
}
...
genCast(iCode *ic)
{
  ...
  if (IS_PTR(restype))
  {
    ...
    if (IS_GENPTR(restype) && (size < AOP_SIZE(result)) && (!IS_GENPTR(rtype) || AOP_SIZE(right) < GPTRSIZE))
    {
      ...
      if (tag == 0)
      {
        mc32_emitpcode(POC_CLRF, mc32_popGet(AOP(result), 2));
      }
      else
      {
        mc32_emitpcode(POC_MOVLW, mc32_popGetLit(tag));
        movwf(AOP(result), 2);
      }
    }
    else
    {
      mc32_addSign(result, max, 0);
    } // if
    goto release;
  }
  ...
  if (AOP_SIZE(result) <= AOP_SIZE(right))
  {
    ...
    if (IS_PTR_CONST(rtype))
      DEBUGmc32_pic14_emitcode("; ***", "%d - right is const pointer", __LINE__);
    if (IS_PTR_CONST(operandType(IC_RESULT(ic))))
      DEBUGmc32_pic14_emitcode("; ***", "%d - result is const pointer", __LINE__);

    if ((AOP_TYPE(right) == AOP_PCODE) && AOP(right)->aopu.pcop->type == PO_IMMEDIATE)
    {
      mc32_emitpcode(POC_MOVLW, mc32_popGetAddr(AOP(right), 0, 0));
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), 0));
      mc32_emitpcode(POC_MOVLW, mc32_popGetAddr(AOP(right), 1, 0));
      mc32_emitpcode(POC_MOVWF, mc32_popGet(AOP(result), 1));
    ...
    else
    {
      ...
      while (size--)
      {
        ...
        //mc32_aopPut(AOP(result),
        // mc32_aopGet(AOP(right),offset,FALSE,FALSE),
        // offset);
  ...
  mc32_addSign(result, AOP_SIZE(right), !SPEC_USIGN(rtype));
...
genDjnz(iCode *ic, iCode *ifx)
{
  ...
  mc32_emitpcode(POC_DECFSZ, mc32_popGet(AOP(IC_RESULT(ic)), 0));
  mc32_emitpcode(POC_GOTO, mc32_popGetLabel(IC_TRUE(ifx)->key));
...
genReceive(iCode *ic)
{
  ...
  if (isOperandInFarSpace(IC_RESULT(ic)) && (OP_SYMBOL(IC_RESULT(ic))->isspilt || IS_TRUE_SYMOP(IC_RESULT(ic))))
  {
    ...
    int offset = mc32_fReturnSizePic - size;
    while (size--)
    {
      mc32_pic14_emitcode("push", "%s", (strcmp(mc32_fReturn[mc32_fReturnSizePic - offset - 1], "a") ? mc32_fReturn[mc32_fReturnSizePic - offset - 1] : "acc"));
      offset++;
    }
    mc32_aopOp(IC_RESULT(ic), ic, FALSE);
    size = AOP_SIZE(IC_RESULT(ic));
    ...
    while (size--)
    {
      mc32_pic14_emitcode("pop", "acc");
      mc32_aopPut(AOP(IC_RESULT(ic)), "a", offset++);
    }
  }
  else
  {
...
{
  FENTRY;
  mc32_pic14_emitcode("; genDummyRead", "");
  mc32_pic14_emitcode("; not implemented", "");
...
void mc32_newVal()
{
  ...
  if (!mc32_ValList)
  {
    mc32_ValList = tempval;
    mc32_ValList->firstItem = mc32_ValList;
    mc32_ValList->next = NULL;
  }
  else
  {
    tempval->firstItem = mc32_ValList->firstItem;
    mc32_ValList->next = tempval;
    mc32_ValList = tempval;
  }
...
/*-----------------------------------------------------------------*/
/* mc32_genpic14Code - generate code for pic14 based controllers        */
/*-----------------------------------------------------------------*/
/*
...
void mc32_genpic14Code(iCode *lic)
{
  iCode *ic;
  int cln = 0;
  const char *cline;
  ...
  if (currFunc)
  {
    ...
    mc32_newVal();
    SaveAllocInfo(currFunc, mc32_ValList);
  ...
  for (ic = lic; ic; ic = ic->next)
  {
    ...
    //DEBUGmc32_pic14_emitcode(";ic","");
    //fprintf (stderr, "in ic loop\n");
    //mc32_pic14_emitcode ("", ";\t%s:%d: %s", ic->filename,
    ...
    if (!options.noCcodeInAsm && (cln != ic->lineno))
    {
      ...
      if (!cline || strlen(cline) == 0)
        cline = printCLine(ic->filename, ic->lineno);
      mc32_addpCode2pBlock(pb, mc32_newpCodeCSource(ic->lineno, ic->filename, cline));
      //mc32_emitpComment ("[C-SRC] %s:%d: %s", ic->filename, cln, cline);
    }

    if (options.iCodeInAsm)
    {
      const char *iLine = printILine(ic);
      mc32_emitpComment("[ICODE] %s:%d: %s", ic->filename, ic->lineno, printILine(ic));
      dbuf_free(iLine);
    }
