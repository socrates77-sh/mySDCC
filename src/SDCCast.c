  ...
  rloop = newNode(NULLOP,
                  createIf(newAst_VALUE(symbolVal(sym)),
                           newNode(GOTO,
                                   ...
                                   NULL),
                           NULL),
                  newNode('=', newAst_VALUE(symbolVal(sym)), end));
...
/*   _forbreak_n:                                                  */
/*-----------------------------------------------------------------*/
ast *createDoFor(symbol *trueLabel, symbol *continueLabel, symbol *falseLabel,
...
/*------------------------------------------------------------------*/
void gatherImplicitVariables(ast *tree, ast *block)
{
  if (!tree)
    return;
...
/*-----------------------------------------------------------*/
ast *rewriteStructAssignment(ast *tree)
{
...
/*--------------------------------------------------------------------*/
ast *decorateType(ast *tree, RESULT_TYPE resultType)
{
  int parmNumber;
  sym_link *p;
...
/*-----------------------------------------------------------------*/
ast *createDefault(ast *swStat, ast *defaultVal, ast *stmnt)
{
  struct dbuf_s dbuf;
  ast *ret;
...
/*        _dobreak_n:                                              */
/*-----------------------------------------------------------------*/
ast *createDo(symbol *trueLabel, symbol *continueLabel, symbol *falseLabel, ast *condAst, ast *doBody)
{
  ast *doTree;
...
/*   _forbreak_n:                                                  */
/*-----------------------------------------------------------------*/
ast *createFor(symbol *trueLabel, symbol *continueLabel, symbol *falseLabel,
...
/*      _while_break_n:                                            */
/*-----------------------------------------------------------------*/
ast *createWhile(symbol *trueLabel, symbol *continueLabel, symbol *falseLabel, ast *condExpr, ast *whileBody)
{
  ast *whileTree;
...
/*-----------------------------------------------------------------*/
ast *optimizeGetHbit(ast *tree, RESULT_TYPE resultType)
{
...
/*-----------------------------------------------------------------*/
ast *optimizeGetAbit(ast *tree, RESULT_TYPE resultType)
{
  ast *expr;
  ast *count = NULL;
...
/*-----------------------------------------------------------------*/
ast *optimizeGetByte(ast *tree, RESULT_TYPE resultType)
{
  unsigned int i = 1;
  unsigned int size;
  ast *expr;
...
/*-----------------------------------------------------------------*/
ast *optimizeGetWord(ast *tree, RESULT_TYPE resultType)
{
  unsigned int i = 1;
  unsigned int size;
  ast *expr;
...
#define INDENT(x, f)                                    \
  do                                                    \
  {                                                     \
    fprintf(f, "%s:%d:", tree->filename, tree->lineno); \
    fprintf(f, "%*s", (x)&0xff, "");                    \
  } while (0)
/*-----------------------------------------------------------------*/
