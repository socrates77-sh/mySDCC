...
dwFindFileIndex(char *filename)
{
  ...
  {
    if (!strncmp(includeDir, filename, strlen(includeDir)) && strlen(filename) > strlen(includeDir))
    {
...
dwHashType(sym_link *type)
{
  ...
  while (type)
  {
    ...
    {
      hash ^= SPEC_NOUN(type) | (SPEC_CONST(type) << 4) | (SPEC_VOLATILE(type) << 5) | (SPEC_LONG(type) << 6);
    }
    ...
    else
    {
      if (IS_DECL(type2))
      {
        ...
        if (DCL_PTR_VOLATILE(type1) != DCL_PTR_VOLATILE(type2))
          return 0;
        if (DCL_TYPE(type1) == ARRAY && DCL_ELEM(type1) != DCL_ELEM(type2))
          return 0;
...
/*                      debug file                                       */
/*-----------------------------------------------------------------------*/
int dwarf2FinalizeFile(FILE *of)
{
  int tagAddress = 11;
  int abbrevNum = 0;
  int attr;
