...
/*-----------------------------------------------------------------*/
void hashiCodeKeys(eBBlock **ebbs, int count)
{
  int i;
    ...
    for (ic = ebbs[i]->sch; ic; ic = ic->next)
    {
      ...
      for (key1 = 1; key1 < ic->rlive->size; key1++)
      {
        ...
        for (key2 = key1 + 1; key2 < ic->rlive->size; key2++)
        {
          ...
          {
            if (OP_SYMBOL(IC_RESULT(ic))->key == key1 && sym1->liveFrom == ic->seq && sym2->liveTo == ic->seq)
            {
              if (IS_SYMOP(IC_LEFT(ic)))
            ...
            if (OP_SYMBOL(IC_RESULT(ic))->key == key2 && sym2->liveFrom == ic->seq && sym1->liveTo == ic->seq)
            {
              if (IS_SYMOP(IC_LEFT(ic)))
          ...
          {
            if (sym1->reqv && !sym2->reqv)
              SYM_SPIL_LOC(sym2) = NULL;
            else if (sym2->reqv && !sym1->reqv)
              SYM_SPIL_LOC(sym1) = NULL;
            else if (sym1->used > sym2->used)
              SYM_SPIL_LOC(sym2) = NULL;
            else
              SYM_SPIL_LOC(sym1) = NULL;
          }
