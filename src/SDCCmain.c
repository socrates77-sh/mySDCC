...
#define DEVICE_DIR DIR_SEPARATOR_STRING "share"	//zwr
#define INCLUDE_DIR_SUFFIX1 DIR_SEPARATOR_STRING "include"	//zwr
#define NON_FREE_INCLUDE_DIR_SUFFIX1 DIR_SEPARATOR_STRING "non-free" DIR_SEPARATOR_STRING "include"	//zwr
#define LIB_DIR_SUFFIX1 DIR_SEPARATOR_STRING "lib"	//zwr
#define NON_FREE_LIB_DIR_SUFFIX1 DIR_SEPARATOR_STRING "non-free" DIR_SEPARATOR_STRING "lib"	//zwr
...
#endif
           " (%s) (%s)\n", getBuildDate (), getBuildEnvironment ());//zwr
}
...
setIncludePath (void)
{
   ...
   *
   *  2. - $SDCC_INCLUDE
   *  4. - $SDCC_HOME/DEVICE_DIR/INCLUDE_DIR_SUFFIX1
   */
  ...
  if (!options.nostdinc)
    {
      ...
      tempSet = appendStrSet (dataDirsSet, NULL, INCLUDE_DIR_SUFFIX1); //zwr
      //includeDirsSet = appendStrSet (dataDirsSet, NULL, INCLUDE_DIR_SUFFIX1); //zwr
      ...
      if (options.use_non_free)
        {
          ...
          tempSet = appendStrSet (dataDirsSet, NULL, NON_FREE_INCLUDE_DIR_SUFFIX1); //zwr
...
setLibPath (void)
{
   ...
   * 1. - $SDCC_LIB/<model>
   * 2. - $SDCC_LIB
   * 3. - $SDCC_HOME/DEVICE_DIR/LIB_DIR_SUFFIX1/<model>
   * 4. - $SDCC_HOME/DEVICE_DIR/NON_FREE_LIB_DIR_SUFFIX1/<model>
   */
...
//zwr
	 tempSet = appendStrSet (dataDirsSet, NULL, LIB_DIR_SUFFIX1);
	 mergeSets (&libDirsSet, tempSet);
      //dbuf_makePath (&dbuf, LIB_DIR_SUFFIX1, port->general.get_model ? port->general.get_model () : targetname);
      //dbuf_makePath (&dbuf, LIB_DIR_SUFFIX1, "");
      //libDirsSet = appendStrSet (dataDirsSet, NULL, dbuf_c_str (&dbuf));
/*
      if (options.use_non_free)
        {
          dbuf_set_length (&dbuf, 0);
          dbuf_makePath (&dbuf, NON_FREE_LIB_DIR_SUFFIX1, port->general.get_model ? port->general.get_model () : targetname); //zwr
  ...
  if ((p = getenv (SDCC_DIR_NAME)) != NULL)
    {
      ...
      dbuf_append_str (&dbuf, DEVICE_DIR);	//zwr
...
#undef DEVICE_DIR //zwr
#undef INCLUDE_DIR_SUFFIX1
#undef NON_FREE_INCLUDE_DIR_SUFFIX1
#undef LIB_DIR_SUFFIX1
#undef NON_FREE_LIB_DIR_SUFFIX1
