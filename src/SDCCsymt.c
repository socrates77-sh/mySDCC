...
/*-----------------------------------------------------------------*/
void addSym(bucket **stab, void *sym, char *sname, long level, int block, int checkType)
{
...
/*-----------------------------------------------------------------*/
void deleteSym(bucket **stab, void *sym, const char *sname)
{
  int i = 0;
  bucket *bp;
...
findSymWithLevel(bucket **stab, symbol *sym)
{
  ...
  while (bp)
  {
    if (strcmp(bp->name, sym->name) == 0 && bp->level <= sym->level)
    {
      ...
      if (bp->level && bp->level == sym->level && bp->block == sym->block && ((symbol *)(bp->sym))->seqPoint <= sym->seqPoint)
        return (bp->sym);
      /* if levels don't match then we are okay if the symbol is in scope */
      if (bp->level && bp->level != sym->level && bp->block <= sym->block && ((symbol *)(bp->sym))->isinscope && (stab == LabelTab || ((symbol *)(bp->sym))->seqPoint <= sym->seqPoint))
        return (bp->sym);
...
void checkTypeSanity(sym_link *etype, const char *name)
{
  ...
  if ((SPEC_NOUN(etype) == V_BOOL ||
       ...
       SPEC_NOUN(etype) == V_DOUBLE || SPEC_NOUN(etype) == V_VOID) &&
...
/*-------------------------------------------------------------------*/
void promoteAnonStructs(int su, structdef *sdef)
{
  symbol *field;
  symbol *subfield;
  symbol **tofield;
...
/*------------------------------------------------------------------*/
int checkDecl(symbol *sym, int isProto)
{
...
/*------------------------------------------------------------------*/
void cleanUpLevel(bucket **table, long level)
{
  int i;
  bucket *chain;
...
computeType(sym_link *type1, sym_link *type2, RESULT_TYPE resultType, int op)
{
  ...
  if (!IS_FLOAT(reType) && ((SPEC_USIGN(etype1)
                            ...
                            (SPEC_USIGN(etype1) && SPEC_USIGN(etype2) && IS_CHAR(etype1) && IS_CHAR(etype2) && IS_CHAR(reType))))
    SPEC_USIGN(reType) = 1;
  else
    SPEC_USIGN(reType) = 0;
...
/*------------------------------------------------------------------*/
int compareFuncType(sym_link *dest, sym_link *src)
{
  value *exargs, *acargs;
  value *checkValue;
  int argCnt = 0;
...
int comparePtrType(sym_link *dest, sym_link *src, bool bMustCast)
{
  int res;
...
/*--------------------------------------------------------------------*/
int compareType(sym_link *dest, sym_link *src)
{
  if (!dest && !src)
    return 1;
...
/*--------------------------------------------------------------------*/
int compareTypeExact(sym_link *dest, sym_link *src, long level)
{
...
/*---------------------------------------------------------------------------*/
int compareTypeInexact(sym_link *dest, sym_link *src)
{
  if (!dest && !src)
    return 1;
...
/*------------------------------------------------------------------*/
int checkFunction(symbol *sym, symbol *csym)
{
  value *exargs, *acargs;
  value *checkValue;
  int argCnt = 0;
...
/*-----------------------------------------------------------------*/
void processFuncPtrArgs(sym_link *funcType)
{
...
/*-----------------------------------------------------------------*/
void processFuncArgs(symbol *func)
{
  value *val;
  int pNum = 1;
...
/*-----------------------------------------------------------------*/
int isSymbolEqual(const symbol *dest, const symbol *src)
{
...
/*-----------------------------------------------------------------*/
void printTypeChain(sym_link *start, FILE *of)
{
  struct dbuf_s dbuf;
  int nlr = 0;
...
void dbuf_printTypeChain(sym_link *start, struct dbuf_s *dbuf)
{
  value *args;
...
/*-----------------------------------------------------------------*/
int powof2(TYPE_TARGET_ULONG num)
{
  int nshifts = 0;
  int n1s = 0;
