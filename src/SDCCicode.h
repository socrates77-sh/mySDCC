...
#define POINTER_SET(ic) (ic && ic->op == '=' && (IS_ITEMP(IC_RESULT(ic)) || IS_OP_LITERAL(IC_RESULT(ic))) && IC_RESULT(ic)->isaddr)

#define POINTER_GET(ic) (ic && ic->op == GET_VALUE_AT_ADDRESS && (IS_ITEMP(IC_LEFT(ic)) || IS_OP_LITERAL(IC_LEFT(ic))) && IC_LEFT(ic)->isaddr)
...
#define SET_ISADDR(op, v)        \
  {                              \
    op = operandFromOperand(op); \
    op->isaddr = v;              \
  }
#define SET_RESULT_RIGHT(ic)      \
  {                               \
