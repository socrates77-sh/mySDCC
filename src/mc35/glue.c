...
set *mc35_localFunctions = NULL;
int mc35_hasInterrupt = 0; // Indicates whether to emit interrupt handler or not

int mc35_stringInSet(const char *str, set **world, int autoAdd);
...
static set *mc35_emitted = NULL;

static void mc35_showAllMemmaps(FILE *of); // XXX: emits initialized symbols

static void
mc35_emitPseudoStack(struct dbuf_s *oBuf, struct dbuf_s *oBufExt)
{
    ...
    shared = mc35_getSharedStack(&low, &high, &size);
    if (!mc35_options.isLibrarySource)
    {
        dbuf_printf(oBuf, "\n");
        // zwr 1.0.0
    ...
    else
    {
        ...
        for (i = 6; i >= 0; i--)
        {
            ...
            mc35_stringInSet(&buffer[0], &mc35_emitted, 1);
        } // for i
    }
    dbuf_printf(oBuf, "\n");
}
...
static void
mc35_constructAbsMap(struct dbuf_s *oBuf, struct dbuf_s *gloBuf)
{
    ...
    for (i = 0; maps[i] != NULL; i++)
    {
             ...
             sym; sym = setNextItem(maps[i]->syms))
        {
            if (IS_DEFINED_HERE(sym) && SPEC_ABSA(sym->etype))
            {
                ...
                if (MC35_IS_CONFIG_ADDRESS(addr))
                {
                    //fprintf( stderr, "%s: assignment to CONFIG@0x%x found\n", __FUNCTION__, addr );
                    //fprintf( stderr, "ival: %p (0x%x)\n", sym->ival, (int)list2int( sym->ival ) );
                    if (sym->ival)
                    {
                        mc35_assignConfigWordValue(addr, (int)list2int(sym->ival));
                    }
                    else
                    {
...
/*-----------------------------------------------------------------*/
static void
mc35_createInterruptVect(struct dbuf_s *vBuf)
{
...
static void
mc35_printLocals(struct dbuf_s *oBuf)
{
...
/*-----------------------------------------------------------------*/
static void
mc35_emitOverlay(struct dbuf_s *aBuf)
{
    set *ovrset;
...
static void
mc35_emitInterruptHandler(FILE *asmFile)
{
    if (mc35_hasInterrupt)
    {
...
void mc35_picglue(void)
{
    ...
    mc35_movepBlock2Head('P'); // Last
    mc35_movepBlock2Head(code->dbName);
    mc35_movepBlock2Head('X');
    mc35_movepBlock2Head(statsg->dbName); // First
    ...
    mc35_emitOverlay(&ovrBuf);

    /* PENDING: this isnt the best place but it will do */
    if (port->general.glue_up_main)
    {
        /* create the interrupt vector table */
        mc35_createInterruptVect(&vBuf);
    }
    ...
    /* prepare statistics */
    mc35_resetpCodeStatistics();

    /* initial comments */
    mc35_initialComments(asmFile);
    ...
    mc35_emitInterruptHandler(asmFile);
    ...
    /* unknown */
    mc35_copypCode(asmFile, 'X');

    /* _main function */
    mc35_copypCode(asmFile, 'M');

    /* other functions */
    mc35_copypCode(asmFile, code->dbName);

    /* unknown */
    mc35_copypCode(asmFile, 'P');

    mc35_dumppCodeStatistics(asmFile);
...
static char *
mc35_parseIvalAst(ast *node, int *inCodeSpace)
{
#define LEN 4096
    char *buffer = NULL;
    char *left, *right;
    ...
    else if (IS_AST_OP(node))
    {
        ...
        case CAST:
            assert(node->right);
            buffer = mc35_parseIvalAst(node->right, inCodeSpace);
            DEBUGprintf("%s: %s\n", __FUNCTION__, buffer);
            break;
        case '&':
            assert(node->left && !node->right);
            buffer = mc35_parseIvalAst(node->left, inCodeSpace);
            DEBUGprintf("%s: %s\n", __FUNCTION__, buffer);
            break;
        case '+':
            assert(node->left && node->right);
            left = mc35_parseIvalAst(node->left, inCodeSpace);
            right = mc35_parseIvalAst(node->right, inCodeSpace);
            buffer = Safe_alloc(LEN);
 ...
 */
static void
mc35_emitIvals(struct dbuf_s *oBuf, symbol *sym, initList *list, long lit, int size)
{
    int i;
    ast *node;
    operand *op;
    ...
    in_code = mc35_emitIvalLabel(oBuf, sym);
    if (!in_code)
        dbuf_printf(oBuf, "\tdb\t");
    ...
    {
        str = mc35_parseIvalAst(node, &inCodeSpace);
    ...
    for (i = 0; i < size; i++)
    {
        ...
        if (op && IS_SYMOP(op) && IS_FUNC(OP_SYM_TYPE(op)))
        {
            ...
            text = mc35_get_op(AOP(op)->aopu.pcop, NULL, 0);
            PCOI(AOP(op)->aopu.pcop)->offset -= i;
        }
        else
        {
            text = op ? mc35_aopGet(AOP(op), i, 0, 0)
                      : mc35_get_op(mc35_newpCodeOpImmd(str, i, 0, inCodeSpace, 0), NULL, 0);
        } // if
        if (in_code)
        {
            // zwr 1.1.0
 ...
 */
static sym_link *
mc35_matchIvalToUnion(initList *list, sym_link *type, int size)
{
    symbol *sym;
    ...
    else if (IS_STRUCT(type) && SPEC_STRUCT(getSpec(type))->type == STRUCT)
    {
        if (!list || (list->type == INIT_DEEP))
        {
            ...
            while (sym)
            {
                ...
                if (!mc35_matchIvalToUnion(list, sym->type, 0))
                {
    ...
    else if (IS_STRUCT(type) && SPEC_STRUCT(getSpec(type))->type == UNION)
    {
        if (!list || (list->type == INIT_DEEP))
        {
            ...
            while (sym)
            {
                ...
                if (((IS_STRUCT(sym->type) || getSize(sym->type) == size)) && mc35_matchIvalToUnion(list, sym->type, size))
                {
 ...
 */
static void
mc35_emitInitVal(struct dbuf_s *oBuf, symbol *topsym, sym_link *my_type, initList *list)
{
    symbol *sym;
    int size;
    long lit;
    ...
    if (IS_PTR(my_type))
    {
        ...
        mc35_emitIvals(oBuf, topsym, list, 0, size);
        return;
    }
    ...
    if (IS_ARRAY(my_type))
    {
        ...
        {
            mc35_emitInitVal(oBuf, topsym, my_type->next, list);
            topsym = NULL;
            if (list)
                list = list->next;
        } // for i
    ...
    else if (IS_STRUCT(my_type) && SPEC_STRUCT(my_type)->type == STRUCT)
    {
        ...
        while (sym)
        {
            ...
            if (IS_BITFIELD(sym->type))
            {
                ...
                mc35_emitIvals(oBuf, topsym, NULL, bitfield, len / 8);
                topsym = NULL;
            } // if

            if (sym)
            {
                mc35_emitInitVal(oBuf, topsym, sym->type, list);
                topsym = NULL;
                sym = sym->next;
                if (list)
                    list = list->next;
    ...
    else if (IS_STRUCT(my_type) && SPEC_STRUCT(my_type)->type == UNION)
    {
        ...
        my_type = mc35_matchIvalToUnion(list, my_type, size);
        if (my_type)
        {
            mc35_emitInitVal(oBuf, topsym, my_type, list->init.deep);
            topsym = NULL;
            size -= getSize(my_type);
            if (size > 0)
            {
                // pad with (leading) zeros
                mc35_emitIvals(oBuf, NULL, NULL, 0, size);
            }
            return;
        } // if
 ...
 */
static void
mc35_emitSymbolSet(set *s, int type)
{
    symbol *sym;
    initList *list;
    ...
    for (sym = setFirstItem(s); sym; sym = setNextItem(s))
    {
        ...
        if (sym->etype && SPEC_ABSA(sym->etype) && MC35_IS_CONFIG_ADDRESS(SPEC_ADDR(sym->etype)) && sym->ival)
        {
            // handle config words
            mc35_assignConfigWordValue(SPEC_ADDR(sym->etype),
                                        (int)list2int(sym->ival));
            mc35_stringInSet(sym->rname, &mc35_emitted, 1);
            continue;
        }
        ...
        if (sym->isstrlit)
        {
            ...
            mc35_emitInitVal(ivalBuf, sym, sym->type, NULL);
            continue;
        }
        ...
        if (type != 0 || sym->cdef || (!IS_STATIC(sym->etype) && IS_GLOBAL(sym)))
        {
            ...
            if (!mc35_stringInSet(sym->rname, &mc35_emitted, 0))
            {
                ...
                else
                {
                    ...
                    if (!sym->ival && !IS_FUNC(sym->type))
                    {
                        ...
                        if (IS_ABSOLUTE(sym->etype))
                        {
                            ...
                            // deferred to mc35_constructAbsMap
                        }
                        else
                        {
        ...
        if (list)
        {
            ...
            mc35_emitInitVal(ivalBuf, sym, sym->type, sym->ival);
            dbuf_printf(ivalBuf, "\n");
        }
    } // for sym
}
...
#endif
            mc35_emitSymbolSet(map->syms, 0);
        } // if (map)
    }     // for i
    DEBUGprintf("---end of memmaps---\n");

    mc35_emitSymbolSet(publics, 1);
    mc35_emitSymbolSet(externs, 2);

    mc35_emitPseudoStack(gloBuf, extBuf);
    mc35_constructAbsMap(gloDefBuf, gloBuf);
    mc35_printLocals(&locBuf);
    mc35_emitConfigWord(of); // must be done after all the rest
