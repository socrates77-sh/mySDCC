...
#if 0
/*-----------------------------------------------------------------*
* void mc35_BuildFlowSegment(set *segment, pCodeFlow *pcflow)
*-----------------------------------------------------------------*/
static void mc35_BuildFlowSegment(pCodeFlow *pcflow)
{
	...
	if(pcflow->ancestor && pcflow->to) {
		flowset = pcflow->to;
		while(flowset) {
			mc35_BuildFlowSegment(PCFLINK(flowset->item)->pcflow);
...
void mc35_BuildFlowTree(pBlock *pb)
{
	pCodeFlow *first_pcflow, *pcflow;

	//  fprintf(stderr,"mc35_BuildFlowTree \n");

	first_pcflow = PCFL(mc35_findNextpCode(pb->pcHead, PC_FLOW));
	if (!first_pcflow)
		return;
	...
	*/
	...
	pcflow = PCFL(mc35_findNextpCode(first_pcflow->pc.next, PC_FLOW));
	...
	while (pcflow)
	{
		...
		pcflow = PCFL(mc35_findNextpCode(pcflow->pc.next, PC_FLOW));
	}
