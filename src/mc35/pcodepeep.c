...
static pCodePeepSnippets *mc35_peepSnippets = NULL;
...
static pCodeToken mc35_tokArr[50];
static unsigned mc35_tokIdx = 0;
...
static parsedPattern mc35_parsedPatArr[MAX_PARSEDPATARR];
...
static char mc35_pcpat_label[] = {PCT_PERCENT, PCT_NUMBER, PCT_COLON, 0};
static char mc35_pcpat_number[] = {PCT_NUMBER, 0};
static char mc35_pcpat_string[] = {PCT_STRING, 0};
static char mc35_pcpat_wildString[] = {PCT_PERCENT, PCT_STRING, 0};
static char mc35_pcpat_wildVar[] = {PCT_PERCENT, PCT_NUMBER, 0};
static char mc35_pcpat_comma[] = {PCT_COMMA, 0};
static char mc35_pcpat_comment[] = {PCT_COMMENT, 0};
...
static pcPattern mc35_pcpArr[] = {
    {PCP_LABEL, mc35_pcpat_label, NULL},
    {PCP_WILDSTR, mc35_pcpat_wildString, NULL},
    {PCP_STR, mc35_pcpat_string, NULL},
    {PCP_WILDVAR, mc35_pcpat_wildVar, NULL},
    {PCP_COMMA, mc35_pcpat_comma, NULL},
    {PCP_COMMENT, mc35_pcpat_comment, NULL},
    {PCP_NUMBER, mc35_pcpat_number, NULL}};

#define PCPATTERNS (sizeof(mc35_pcpArr) / sizeof(pcPattern))
...
static char mc35_alt_comment[] = {PCP_COMMENT, 0};
static char mc35_alt_label[] = {PCP_LABEL, 0};
static char mc35_alt_mnem0[] = {PCP_STR, 0};
static char mc35_alt_mnem0a[] = {PCP_WILDVAR, 0};
static char mc35_alt_mnem1[] = {PCP_STR, PCP_STR, 0};
static char mc35_alt_mnem1a[] = {PCP_STR, PCP_WILDVAR, 0};
static char mc35_alt_mnem1b[] = {PCP_STR, PCP_NUMBER, 0};
static char mc35_alt_mnem2[] = {PCP_STR, PCP_STR, PCP_COMMA, PCP_STR, 0};
static char mc35_alt_mnem2a[] = {PCP_STR, PCP_WILDVAR, PCP_COMMA, PCP_STR, 0};
static char mc35_alt_mnem3[] = {PCP_STR, PCP_STR, PCP_COMMA, PCP_NUMBER, 0};

static void *mc35_cvt_altpat_label(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_comment(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_mnem0(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_mnem0a(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_mnem1(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_mnem1a(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_mnem1b(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_mnem2(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_mnem2a(void *pp, pCodeWildBlock *pcwb);
static void *mc35_cvt_altpat_mnem3(void *pp, pCodeWildBlock *pcwb);

static pcPattern mc35_altArr[] = {
    {ALT_LABEL, mc35_alt_label, mc35_cvt_altpat_label},
    {ALT_COMMENT, mc35_alt_comment, mc35_cvt_altpat_comment},
    {ALT_MNEM3, mc35_alt_mnem3, mc35_cvt_altpat_mnem3},
    {ALT_MNEM2A, mc35_alt_mnem2a, mc35_cvt_altpat_mnem2a},
    {ALT_MNEM2, mc35_alt_mnem2, mc35_cvt_altpat_mnem2},
    {ALT_MNEM1B, mc35_alt_mnem1b, mc35_cvt_altpat_mnem1b},
    {ALT_MNEM1A, mc35_alt_mnem1a, mc35_cvt_altpat_mnem1a},
    {ALT_MNEM1, mc35_alt_mnem1, mc35_cvt_altpat_mnem1},
    {ALT_MNEM0A, mc35_alt_mnem0a, mc35_cvt_altpat_mnem0a},
    {ALT_MNEM0, mc35_alt_mnem0, mc35_cvt_altpat_mnem0},

};

#define ALTPATTERNS (sizeof(mc35_altArr) / sizeof(pcPattern))

// forward declarations
static void *mc35_DLL_append(DLList *list, DLList *next);

/*-----------------------------------------------------------------*/
/* mc35_cvt_extract_destination - helper function extracts the register */
/*                           destination from a parsedPattern.     */
/*                                                                 */
/*-----------------------------------------------------------------*/
static int mc35_cvt_extract_destination(parsedPattern *pp)
{
...
/*-----------------------------------------------------------------*/
/*  pCodeOp *mc35_cvt_extract_status(char *reg, char *bit)              */
...
static pCodeOp *mc35_cvt_extract_status(char *reg, char *bit)
{
  int len;

  if (STRCASECMP(reg, mc35_pc_status.pcop.name))
    return NULL;
  ...
  if (len == 1)
  {
    ...
    if (toupper((unsigned char)*bit) == 'C')
      return PCOP(mc35_popCopyGPR2Bit(PCOP(&mc35_pc_status), PIC_C_BIT));
    if (toupper((unsigned char)*bit) == 'Z')
      return PCOP(mc35_popCopyGPR2Bit(PCOP(&mc35_pc_status), PIC_Z_BIT));
  }

  // Check DC
  if (len == 2 && toupper((unsigned char)bit[0]) == 'D' && toupper((unsigned char)bit[1]) == 'C')
    return PCOP(mc35_popCopyGPR2Bit(PCOP(&mc35_pc_status), PIC_DC_BIT));

  return NULL;
}

/*-----------------------------------------------------------------*/
/* mc35_cvt_altpat_label - convert assembly line type to a pCode label  */
...
static void *mc35_cvt_altpat_label(void *pp, pCodeWildBlock *pcwb)
{
  ...
  return mc35_newpCodeLabel(NULL, -p->pct[1].tok.n);
}

/*-----------------------------------------------------------------*/
/* mc35_cvt_altpat_comment - convert assembly line type to a comment    */
/* INPUT: pointer to the parsedPattern                             */
/*                                                                 */
/*  pp[0] - comment                                                */
/*                                                                 */
...
static void *mc35_cvt_altpat_comment(void *pp, pCodeWildBlock *pcwb)
{
  ...
  return mc35_newpCodeCharP(p->pct[0].tok.s);
}

/*-----------------------------------------------------------------*/
/* mc35_cvt_altpat_mem0  - convert assembly line type to a wild pCode   */
/*                    instruction                                  */
/*                                                                 */
/*  pp[0] - str                                                    */
/*                                                                 */
...
static void *mc35_cvt_altpat_mnem0(void *pp, pCodeWildBlock *pcwb)
{
  ...
  opcode = mc35_getpCode(p->pct[0].tok.s, 0);
  ...
  if (opcode < 0)
  {
    ...
    opcode = mc35_getpCodePeepCommand(p->pct[0].tok.s);
    //if(opcode > 0)
    //  fprintf(stderr,"  but valid peep command: %s, key = %d\n",p->pct[0].tok.s,opcode);
    return NULL;
  }

  pci = PCI(mc35_newpCode(opcode, NULL));
...
/*-----------------------------------------------------------------*/
/* mc35_cvt_altpat_mem0a - convert assembly line type to a wild pCode   */
/*                    instruction                                  */
/*                                                                 */
/*  pp[0] - wild var                                               */
/*                                                                 */
...
static void *mc35_cvt_altpat_mnem0a(void *pp, pCodeWildBlock *pcwb)
{
  ...
  return mc35_newpCodeWild(p[0].pct[1].tok.n, NULL, NULL);
}

/*-----------------------------------------------------------------*/
/* mc35_cvt_altpat_mem1 - convert assembly line type to a pCode         */
...
static void *mc35_cvt_altpat_mnem1(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if (mc35_pic14Mnemonics[opcode]->isBitInst)
    pcosubtype = mc35_newpCodeOp(p[1].pct[0].tok.s, PO_BIT);
  else
    pcosubtype = mc35_newpCodeOp(p[1].pct[0].tok.s, PO_GPR_REGISTER);

  pci = PCI(mc35_newpCode(opcode, pcosubtype));
...
/*-----------------------------------------------------------------*/
/* mc35_cvt_altpat_mem1a - convert assembly line type to a pCode        */
...
static void *mc35_cvt_altpat_mnem1a(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if (opcode < 0)
  {
    int cmd_id = mc35_getpCodePeepCommand(p->pct[0].tok.s);
    pCode *pc = NULL;
    ...
    pc = mc35_newpCodeWild(p[1].pct[1].tok.n, NULL, NULL);
  ...
  if (mc35_pic14Mnemonics[opcode]->isBitInst)
    pcosubtype = mc35_newpCodeOpBit(NULL, -1, 0);
  else
    pcosubtype = mc35_newpCodeOp(NULL, PO_GPR_REGISTER);

  pci = PCI(mc35_newpCode(opcode,
                     mc35_newpCodeOpWild(p[1].pct[1].tok.n, pcwb, pcosubtype)));
...
static void *mc35_cvt_altpat_mnem1b(void *pp, pCodeWildBlock *pcwb)
{
  ...
  pci = PCI(mc35_newpCode(opcode, mc35_newpCodeOpLit(p[1].pct[0].tok.n)));
...
static void *mc35_cvt_altpat_mnem2(void *pp, pCodeWildBlock *pcwb)
{
  ...
  dest = mc35_cvt_extract_destination(&p[3]);
  ...
  opcode = mc35_getpCode(p->pct[0].tok.s, dest);
  if (opcode < 0)
  {
    fprintf(stderr, "Bad mnemonic\n");
    return NULL;
  ...
  if (mc35_pic14Mnemonics[opcode]->isBitInst)
  {
    pcosubtype = mc35_cvt_extract_status(p[1].pct[0].tok.s, p[3].pct[0].tok.s);
    if (pcosubtype == NULL)
    {
...
/*-----------------------------------------------------------------*/
/* mc35_cvt_altpat_mem2a - convert assembly line type to a pCode        */
...
static void *mc35_cvt_altpat_mnem2a(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if (mc35_pic14Mnemonics[opcode]->isBitInst)
    pcosubtype = mc35_newpCodeOp(NULL, PO_BIT);
  else
    pcosubtype = mc35_newpCodeOp(NULL, PO_GPR_REGISTER);
...
/*-----------------------------------------------------------------*/
/* mc35_cvt_altpat_mem3 -  convert assembly line type to a pCode        */
...
static void *mc35_cvt_altpat_mnem3(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if (mc35_pic14Mnemonics[opcode]->isBitInst)
  {
    ...
    //if(pcosubtype == NULL) {
    pcosubtype = mc35_newpCodeOpBit(p[1].pct[0].tok.s, p[3].pct[0].tok.n, 0);
    //}
  }
  else
    pcosubtype = mc35_newpCodeOp(p[1].pct[0].tok.s, PO_GPR_REGISTER);
...
/*-----------------------------------------------------------------*/
/* mc35_tokenizeLineNode - Convert a string (of char's) that was parsed */
...
static void mc35_tokenizeLineNode(char *ln)
{
  char *lnstart = ln;
  mc35_tokIdx = 0;              // Starting off at the beginning
  mc35_tokArr[0].tt = PCT_NULL; // and assume invalid character for first token.
  ...
  while (*ln)
  {
    ...
    if (isspace((unsigned char)*ln))
    {
      ...
      mc35_tokArr[mc35_tokIdx++].tt = PCT_SPACE;
    ...
    if (isdigit((unsigned char)*ln))
    {
      ...
      mc35_tokArr[mc35_tokIdx].tt = PCT_NUMBER;
      mc35_tokArr[mc35_tokIdx++].tok.n = strtol(ln, &ln, 0);
    ...
    {
    case '%':
      mc35_tokArr[mc35_tokIdx++].tt = PCT_PERCENT;
      break;
    case '<':
      mc35_tokArr[mc35_tokIdx++].tt = PCT_LESSTHAN;
      break;
    case '>':
      mc35_tokArr[mc35_tokIdx++].tt = PCT_GREATERTHAN;
      break;
    case ':':
      mc35_tokArr[mc35_tokIdx++].tt = PCT_COLON;
      break;
    case ';':
      mc35_tokArr[mc35_tokIdx].tok.s = Safe_strdup(ln);
      mc35_tokArr[mc35_tokIdx++].tt = PCT_COMMENT;
      mc35_tokArr[mc35_tokIdx].tt = PCT_NULL;
      return;
    case ',':
      mc35_tokArr[mc35_tokIdx++].tt = PCT_COMMA;
      break;
    ...
    default:
      if (isalpha((unsigned char)*ln) || (*ln == '_'))
      {
        ...
        mc35_tokArr[mc35_tokIdx].tok.s = Safe_strdup(buffer);
        mc35_tokArr[mc35_tokIdx++].tt = PCT_STRING;
      }
      else
      {
  ...
  mc35_tokArr[mc35_tokIdx].tt = 0;
}

/*-----------------------------------------------------------------*/
/*-----------------------------------------------------------------*/

static void mc35_dump1Token(pCodeTokens tt)
{
  ...
  case PCT_COMMENT:
    ...
    //fprintf(stderr,"%s",mc35_tokArr[i].tok.s);
    break;
  case PCT_STRING:
    fprintf(stderr, " str ");
    //fprintf(stderr,"%s",mc35_tokArr[i].tok.s);
  ...
  case PCT_NUMBER:
    fprintf(stderr, " num ");
    //fprintf(stderr,"%d",mc35_tokArr[i].tok.n);
    break;
  case PCT_NULL:
    fprintf(stderr, " null ");
  }
...
static int mc35_pcComparePattern(pCodeToken *pct, char *pat, int max_tokens)
{
  int i = 0;
  ...
  while (i < max_tokens)
  {
    ...
    //mc35_dump1Token(*pat); DFPRINTF((stderr,"\n"));
...
static int mc35_altComparePattern(char *pct, parsedPattern *pat, int max_tokens)
{
  int i = 0;
...
static int mc35_advTokIdx(int *v, int amt)
{

  if ((unsigned)(*v + amt) > mc35_tokIdx)
    return 1;
...
/*-----------------------------------------------------------------*/
/* mc35_parseTokens - convert the tokens corresponding to a single line */
...
static int mc35_parseTokens(pCodeWildBlock *pcwb, pCode **pcret)
{
  pCode *pc;
  int error = 0;
  ...
  {
    unsigned i;
    for (i = 0; i <= mc35_tokIdx; i++)
      mc35_dump1Token(mc35_tokArr[i].tt);
    fputc('\n', stderr);
  }
#endif

  {
        ...
        if (j)
        {
          ...
          case PCP_STR:
            ...
            case PS_HAVE_MNEM:
              ...
              //pco1 = mc35_newpCodeOp(NULL,PO_GPR_REGISTER);
    ...
    mc35_parsedPatArr[lparsedPatIdx].pcp = NULL;
    mc35_parsedPatArr[lparsedPatIdx].pct = NULL;
    ...
    do
    {
      ...
      if ((c = mc35_altComparePattern(mc35_altArr[k].tokens, &mc35_parsedPatArr[j], 10)))
      {

        if (mc35_altArr[k].f)
        {
          pc = mc35_altArr[k].f(&mc35_parsedPatArr[j], pcwb);
          //if(pc && pc->print)
          //  pc->print(stderr,pc);
          ...
          if (pc)
          {
            if (pcret)
            {
              *pcret = pc;
              ...
              mc35_addpCode2pBlock(pcwb->pb, pc);
          }
          else
            error++;
        }
  ...
  DFPRINTF((stderr,"\nConverting parsed line to pCode:\n\n"));
    ...
    j = 0;
    do {
    if(mc35_parsedPatArr[j].pcp && mc35_parsedPatArr[j].pcp->f )
    mc35_parsedPatArr[j].pcp->f(&mc35_parsedPatArr[j]);
    DFPRINTF((stderr,"  %d",mc35_parsedPatArr[j].pcp->pt));
    j++;
    }
...
/*                                                                 */
/*-----------------------------------------------------------------*/
static void mc35_peepRuleBlock2pCodeBlock(lineNode *ln, pCodeWildBlock *pcwb)
{
  ...
  for (; ln; ln = ln->next)
  {
    ...
    mc35_tokenizeLineNode(ln->line);

    if (mc35_parseTokens(pcwb, NULL))
    {
      int i;
...
/*-----------------------------------------------------------------*/
/* mc35_peepRuleCondition                                               */
/*-----------------------------------------------------------------*/
static void mc35_peepRuleCondition(char *cond, pCodePeep *pcp)
{
  if (!cond || !pcp)
    return;
...
static void mc35_initpCodeWildBlock(pCodeWildBlock *pcwb)
{
...
static void mc35_postinit_pCodeWildBlock(pCodeWildBlock *pcwb)
{
...
static void mc35_initpCodePeep(pCodePeep *pcp)
{
  ...
  mc35_initpCodeWildBlock(&pcp->target);
  pcp->target.pb = mc35_newpCodeChain(NULL, 'W', NULL);

  mc35_initpCodeWildBlock(&pcp->replace);
  pcp->replace.pb = mc35_newpCodeChain(NULL, 'W', NULL);
}

/*-----------------------------------------------------------------*/
/* mc35_peepRules2pCode - parse the "parsed" peep hole rules to generate*/
/*                   pCode.                                        */
/*                                                                 */
...
/*                                                                 */
/*-----------------------------------------------------------------*/
void mc35_peepRules2pCode(peepRule *rules)
{
  peepRule *pr;
  ...
  for (pr = rules; pr; pr = pr->next)
  {
    ...
    mc35_peepRuleBlock2pCodeBlock(pr->match, &currentRule->target);

    //DFPRINTF((stderr,"finished target, here it is in pcode form:\n"));
    //mc35_printpBlock(stderr, currentRule->target.pb);

    mc35_pBlockMergeLabels(currentRule->target.pb);
    //mc35_printpBlock(stderr, currentRule->replace.pb);

    /* Convert the replace block */
    mc35_peepRuleBlock2pCodeBlock(pr->replace, &currentRule->replace);

    //DFPRINTF((stderr,"replace with labels merged:\n"));

    mc35_pBlockMergeLabels(currentRule->replace.pb);
    //mc35_printpBlock(stderr, currentRule->replace.pb);

    mc35_peepRuleCondition(pr->cond, currentRule);

    /* The rule has been converted to pCode. Now allocate
    * space for the wildcards */

    mc35_postinit_pCodeWildBlock(&currentRule->target);
    mc35_postinit_pCodeWildBlock(&currentRule->replace);
  ...
  if (0)
  {
    ...
    peeprules = (DLList *)mc35_peepSnippets;
    fprintf(stderr, "target rules\n");
    while (peeprules)
    {
      fprintf(stderr, "   rule:\n");
      peepBlock = ((pCodePeepSnippets *)peeprules)->peep;
      mc35_printpBlock(stderr, peepBlock->target.pb);
...
/*-----------------------------------------------------------------*/
/* mc35_pCodeSearchCondition - Search a pCode chain for a 'condition'   */
/*                                                                 */
/* return conditions                                               */
    ...
    if (pc->type == PC_OPCODE)
    {
      if (PCI(pc)->inCond & cond)
      {
        if (contIfSkip)
        {
          ...
          pCode *pcp = mc35_findPrevInstruction(pc->prev);
...
/*-----------------------------------------------------------------
* int mc35_pCodeOpCompare(pCodeOp *pcops, pCodeOp *pcopd)
*
* Compare two pCodeOp's and return 1 if they're the same
*-----------------------------------------------------------------*/
static int mc35_pCodeOpCompare(pCodeOp *pcops, pCodeOp *pcopd)
{
  char b[50], *n2;
  ...
  /*
    ...
    mc35_get_op( pcops,NULL,0));

      fprintf(stderr," to %s\n",
      mc35_get_op( pcopd,NULL,0));
  */
  ...
  b[0] = 0;
  mc35_get_op(pcops, b, 50);

  n2 = mc35_get_op(pcopd, NULL, 0);
...
static int mc35_pCodePeepMatchLabels(pCodePeep *peepBlock, pCode *pcs, pCode *pcd)
{
  int labindex;
...
/*-----------------------------------------------------------------*/
/* mc35_pCodePeepMatchLine - Compare source and destination pCodes to   */
...
static int mc35_pCodePeepMatchLine(pCodePeep *peepBlock, pCode *pcs, pCode *pcd)
{
  ...
  * are not wild. However, there may be a label or a wild operand */
  ...
  if (pcs->type == pcd->type)
  {
    ...
    if (pcs->type == PC_OPCODE)
    {
      ...
      if (!mc35_pCodePeepMatchLabels(peepBlock, pcs, pcd))
        return 0;
...
//DFPRINTF((stderr,"destination is wild\n"));
          ...
          mc35_pCodeOpCompare(PCI(pcs)->pcop, peepBlock->target.wildpCodeOps[index])
          );
          */

            return mc35_pCodeOpCompare(PCI(pcs)->pcop, peepBlock->target.wildpCodeOps[index]);
          }
          /*
          {
            char *n;
        ...
        mc35_pCodeOpCompare(PCI(pcs)->pcop, PCI(pcd)->pcop));
        */
          return mc35_pCodeOpCompare(PCI(pcs)->pcop, PCI(pcd)->pcop);
        }
        else
        {
  ...
  if ((pcd->type == PC_WILD) && (pcs->type == PC_OPCODE))
  {
    ...
    pcs = mc35_findNextInstruction(pcs->next);
    if (pcs)
    {
...
/*-----------------------------------------------------------------*/
/*-----------------------------------------------------------------*/
static void mc35_pCodePeepClrVars(pCodePeep *pcp)
{
...
/*-----------------------------------------------------------------*/
/*-----------------------------------------------------------------*/
int mc35_pCodePeepMatchRule(pCode *pc)
{
  pCodePeep *peepBlock;
  pCode *pct, *pcin;
  ...
  while (peeprules)
  {
    ...
    mc35_pCodePeepClrVars(peepBlock);
    /*
    pcin = pc;
    if(IS_PCCOMMENT(pcin))
    pc = pcin = mc35_findNextInstruction(pcin->next);
    */
    pcin = pc = mc35_findNextInstruction(pc);
    ...
    while (pct && pcin)
    {
      ...
      if (!(matched = mc35_pCodePeepMatchLine(peepBlock, pcin, pct)))
        break;

      pcin = mc35_findNextInstruction(pcin->next);
      pct = pct->next;
      //debug:
      //DFPRINTF((stderr,"    matched\n"));
    ...
    */
      ...
      if (pcin && peepBlock->postFalseCond &&
          (mc35_pCodeSearchCondition(pcin, peepBlock->postFalseCond, 0) > 0))
        matched = 0;

      //fprintf(stderr," condition results = %d\n",mc35_pCodeSearchCondition(pcin,peepBlock->postFalseCond));
...
/* We matched a rule! Now we have to go through and remove the
...
#if 0
      {
        ...
        while(pc &&  pc!=pcin) {
          ...
          mc35_pCode2str(&buf[2], 254, pc);
          mc35_pCodeInsertAfter(pcprev, mc35_newpCodeCharP(buf));
          pcprev = pcprev->next;
          pc = pc->next;
          ...
          if (PCI(pcr)->pcop)
          {
            ...
            * Is it wild? */
            if (PCI(pcr)->pcop->type == PO_WILD)
            {
              ...
              if (peepBlock->target.wildpCodeOps[index])
                pcop = mc35_pCodeOpCopy(peepBlock->target.wildpCodeOps[index]);
              else
                DFPRINTF((stderr, "error, wildopcode in replace but not source?\n"));
            }
            else
              pcop = mc35_pCodeOpCopy(PCI(pcr)->pcop);
          }
          //DFPRINTF((stderr,"inserting pCode\n"));
          mc35_pCodeInsertAfter(pc, mc35_newpCode(PCI(pcr)->op, pcop));
        }
        else if (pcr->type == PC_WILD)
        {
          if (PCW(pcr)->invertBitSkipInst)
            DFPRINTF((stderr, "We need to invert the bit skip instruction\n"));
          mc35_pCodeInsertAfter(pc,
                           mc35_pCodeInstructionCopy(PCI(peepBlock->target.wildpCodes[PCW(pcr)->id]),
                                                PCW(pcr)->invertBitSkipInst));
        }
        else if (pcr->type == PC_COMMENT)
        {
          mc35_pCodeInsertAfter(pc, mc35_newpCodeCharP(((pCodeComment *)(pcr))->comment));
        }
