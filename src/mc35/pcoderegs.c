...
static int mc35_total_registers_saved = 0;
static int mc35_register_optimization = 1;

/*-----------------------------------------------------------------*
* void mc35_pCodeRegMapLiveRangesInFlow(pCodeFlow *pcfl)
*-----------------------------------------------------------------*/
static void mc35_pCodeRegMapLiveRangesInFlow(pCodeFlow *pcfl)
{
	...
	pc = mc35_findNextInstruction(pcfl->pc.next);
...
/*-----------------------------------------------------------------*
* void mc35_pCodeRegMapLiveRanges(pBlock *pb) 
*-----------------------------------------------------------------*/
void mc35_pCodeRegMapLiveRanges(pBlock *pb)
{
	pCode *pcflow;
		 ...
		 pcflow = mc35_findNextpCode(pcflow->next, PC_FLOW))
	{
		...
		if (!isPCFL(pcflow))
		{
			fprintf(stderr, "mc35_pCodeRegMapLiveRanges - pcflow is not a flow object ");
			continue;
		}
		mc35_pCodeRegMapLiveRangesInFlow(PCFL(pcflow));
	}
...
*
*-----------------------------------------------------------------*/
static void mc35_Remove1pcode(pCode *pc, reg_info *reg, int debug_code)
{
	...
	if (PCI(pc)->label)
	{
		pcn = mc35_findNextInstruction(pc->next);

		if (pcn)
			PCI(pcn)->label = mc35_pBranchAppend(PCI(pcn)->label, PCI(pc)->label);
	}
	...
	if (1)
	{
		...
		size -= strlen(pbuff);
		pbuff += strlen(pbuff);
		mc35_pCode2str(pbuff, size, pc);
		mc35_pCodeInsertBefore(pc, mc35_newpCodeCharP(buff1));
...
/*-----------------------------------------------------------------*
* void mc35_RemoveRegsFromSet(set *regset)
*
*-----------------------------------------------------------------*/
static void mc35_RemoveRegsFromSet(set *regset)
{
	reg_info *reg;
	int used;
		...
		if (used <= 1)
		{
			...
			else
			{
				...
				if (isPCI(pc))
				{
					if (PCI(pc)->label)
					{
						pCode *pcn = mc35_findNextInstruction(pc->next);
					...
					if (isPCI_SKIP(pc))
					{
						reg_info *r = mc35_getRegFromInstruction(pc);
...
static void mc35_ReMapLiveRanges(void)
{
	pBlock *pb;
	if (!mc35_the_pFile)
		return;
	...
	{
#if 0
		pCode *pc = mc35_findNextpCode(pb->pcHead, PC_FLOW);
		if (pc) {
			pc->print( stderr, pc );
		} else {
...
void mc35_RemoveUnusedRegisters(void)
{
	...
	mc35_ReMapLiveRanges();

	//mc35_RemoveRegsFromSet(mc35_dynInternalRegs);
	mc35_RemoveRegsFromSet(mc35_dynAllocRegs);
	mc35_RemoveRegsFromSet(mc35_dynStackRegs);
	/*
	don't do DirectRegs yet - there's a problem with arrays
	mc35_RemoveRegsFromSet(mc35_dynDirectRegs);
	*/
	mc35_RemoveRegsFromSet(mc35_dynDirectBitRegs);

	if (mc35_total_registers_saved)
		DFPRINTF((stderr, " *** Saved %d registers ***\n", mc35_total_registers_saved));
}

/*-----------------------------------------------------------------*
*
*-----------------------------------------------------------------*/
static void mc35_Remove2pcodes(pCode *pcflow, pCode *pc1, pCode *pc2, reg_info *reg, int can_free)
{
	...
	mc35_printpCode (stderr, pc1);
	mc35_printpCode (stderr, pc2);
#endif

	//fprintf(stderr,"%s\n",__FUNCTION__);
	if (pc1)
		mc35_Remove1pcode(pc1, reg, debug_code++);

	if (pc2)
	{
		mc35_Remove1pcode(pc2, reg, debug_code++);
...
*
*-----------------------------------------------------------------*/
static int mc35_regUsedinRange(pCode *pc1, pCode *pc2, reg_info *reg)
{
	int i = 0;
	reg_info *testreg;
	...
	do
	{
		testreg = mc35_getRegFromInstruction(pc1);
		...
		if (i++ > 1000)
		{
			fprintf(stderr, "warning, mc35_regUsedinRange searched through too many pcodes\n");
			return 0;
		}

		pc1 = mc35_findNextInstruction(pc1->next);
...
static int mc35_regIsSpecial(reg_info *reg, int mayBeGlobal)
{
	if (!reg)
		return 0;
...
/*-----------------------------------------------------------------*
* void mc35_pCodeOptime2pCodes(pCode *pc1, pCode *pc2) 
*
...
*
*-----------------------------------------------------------------*/
static int mc35_pCodeOptime2pCodes(pCode *pc1, pCode *pc2, pCode *pcfl_used, reg_info *reg, int can_free, int optimize_level)
{
	pCode *pct1, *pct2;
	reg_info *reg1, *reg2;

	int t = mc35_total_registers_saved;
	...
	if (mc35_options.disable_df)
		return 0;

	//fprintf(stderr,"mc35_pCodeOptime2pCodes\n");
	...
	if ((PCI(pc1)->op == POC_CLRF) && (PCI(pc2)->op == POC_MOVFW))
	{
		...
		*/
		...
		pct2 = mc35_findNextInstruction(pc2->next);
		if (mc35_pCodeSearchCondition(pct2, PCC_Z, 0) == -1)
		{
			/* Z is definitely overwritten before use */
			newpc = mc35_newpCode(POC_MOVLW, mc35_newpCodeOpLit(0));

			mc35_pCodeInsertAfter(pc2, newpc);
			PCI(newpc)->pcflow = PCFL(pcfl_used);
			newpc->seq = pc2->seq;

			//fprintf (stderr, "%s:%d(%s): mc35_Remove2pcodes (CLRF reg, ..., MOVF reg,W)\n", __FILE__, __LINE__, __FUNCTION__);
			//mc35_Remove2pcodes(pcfl_used, pc2, NULL, reg, 0);
			pc2->destruct(pc2);
			//mc35_total_registers_saved++;  // debugging stats.
		}
	}
	else if ((PCI(pc1)->op == POC_CLRF) && (PCI(pc2)->op == POC_IORFW))
	{
		...
		if (mc35_pCodeSearchCondition(pct2, PCC_Z, 0) != -1)
		{
			pct2 = mc35_newpCode(POC_IORLW, mc35_newpCodeOpLit(0));
			pct2->seq = pc2->seq;
			PCI(pct2)->pcflow = PCFL(pcfl_used);
			mc35_pCodeInsertAfter(pc1, pct2);
		}
		//fprintf (stderr, "%s:%d(%s): mc35_Remove2pcodes (CLRF/IORFW)\n", __FILE__, __LINE__, __FUNCTION__);
		mc35_Remove2pcodes(pcfl_used, pc1, pc2, reg, can_free);
		mc35_total_registers_saved++; // debugging stats.
	}
	else if (PCI(pc1)->op == POC_MOVWF)
	{
		...
		if (PCI(pc2)->op == POC_MOVFW)
		{
			...
			Change:
			  ...
			  movwf   reg2   ; pct2
				...
				*/
				reg2 = mc35_getRegFromInstruction(pct2);
				/* Check reg2 is not used for something else before it is loaded with reg */
				if (reg2 && !mc35_regIsSpecial(reg2, 1) && !mc35_regUsedinRange(pc1, pc2, reg2))
				{
					pCode *pct3 = mc35_findNextInstruction(pct2->next);
					...
					if ((mc35_pCodeSearchCondition(pct3, PCC_Z, 0) == -1) && (mc35_pCodeSearchCondition(pct3, PCC_W, 0) == -1))
					{
						DFPRINTF((stderr, "   optimising MOVF reg ... MOVF reg,W MOVWF reg2 to MOVWF reg2 ...\n"));
						pct2->seq = pc1->seq;
						mc35_unlinkpCode(pct2);
						mc35_pCodeInsertBefore(pc1, pct2);
						if (mc35_regUsedinRange(pct2, 0, reg))
						{
							//fprintf (stderr, "%s:%d(%s): mc35_Remove2pcodes IF (MOVWF reg, ..., MOVW reg,W  MOVWF reg2)\n", __FILE__, __LINE__, __FUNCTION__);
							mc35_Remove2pcodes(pcfl_used, pc2, NULL, reg, can_free);
						}
						else
						{
							//fprintf (stderr, "%s:%d(%s): mc35_Remove2pcodes ELSE (MOVWF reg, ..., MOVW reg,W  MOVWF reg2)\n", __FILE__, __LINE__, __FUNCTION__);
							mc35_Remove2pcodes(pcfl_used, pc1, pc2, reg, can_free);
						}
						mc35_total_registers_saved++; // debugging stats.
						return 1;
		...
		pct1 = mc35_findPrevInstruction(pc1->prev);
		if (pct1 && (PCI(pct1)->pcflow == PCI(pc1)->pcflow))
		{
				...
				(PCI(pc2)->op == POC_MOVFW))
			{
				...
				reg1 = mc35_getRegFromInstruction(pct1);
				if (reg1 && !mc35_regIsSpecial(reg1, 0) && !mc35_regUsedinRange(pc1, pc2, reg1))
				{
					...
					Or, if we're not deleting the register then the "To" is:
						...
						movwf  reg
					*/
					pct2 = mc35_newpCode(PCI(pc2)->op, PCI(pct1)->pcop);
					mc35_pCodeInsertAfter(pc2, pct2);
					PCI(pct2)->pcflow = PCFL(pcfl_used);
					pct2->seq = pc2->seq;

					if (can_free)
					{
						//fprintf (stderr, "%s:%d(%s): mc35_Remove2pcodes CANFREE (MOVF reg1,W; MOVWF reg2; MOVF reg2,W)\n", __FILE__, __LINE__, __FUNCTION__);
						mc35_Remove2pcodes(pcfl_used, pc1, pc2, reg, can_free);
					}
					else
					{
						...
						* the register is needed somewhere else.*/
						mc35_unlinkpCode(pc1);
						mc35_pCodeInsertAfter(pct2, pc1);

						//fprintf (stderr, "%s:%d(%s): mc35_Remove2pcodes ELSE (MOVF reg1,W; MOVWF reg2; MOVF reg2,W)\n", __FILE__, __LINE__, __FUNCTION__);
						mc35_Remove2pcodes(pcfl_used, pc2, NULL, reg, can_free);
					}

					//fprintf (stderr, "%s:%d(%s): mc35_Remove2pcodes ALWAYS (MOVF reg1,W; MOVWF reg2; MOVF reg2,W)\n", __FILE__, __LINE__, __FUNCTION__);
					mc35_Remove2pcodes(pcfl_used, pct1, NULL, reg1, 0);
					mc35_total_registers_saved++; // debugging stats.
				}
			}
		}
	...
	return (mc35_total_registers_saved != t);
}

/*-----------------------------------------------------------------*
* void pCodeRegOptimeRegUsage(pBlock *pb) 
*-----------------------------------------------------------------*/
static void mc35_OptimizeRegUsage(set *fregs, int optimize_multi_uses, int optimize_level)
{
	reg_info *reg;
	int used;
		...
		*/
		...
		if (used == 2)
		{
			...
			*/
			...
			/*
			fprintf (stderr, "mc35_OptimizeRegUsage: %s  addr=0x%03x rIdx=0x%03x type=%d used=%d\n",
			reg->name,
			reg->address,
			...
			*/
			...
			if (pcfl_used && pcfl_assigned)
			{
				...
				*/
				...
				if (pcfl_used->seq == pcfl_assigned->seq)
				{
					...
					mc35_pCodeOptime2pCodes(pc1, pc2, pcfl_used, reg, 1, optimize_level);
				}
				else
				{
			...
			else
			{
				...
				mc35_Remove2pcodes(pcfl_assigned, pc1, pc2, reg, 1);
				mc35_total_registers_saved++; // debugging stats.
			}
		}
		else
						...
						if (rset2)
						{
							...
							if (pc2 && isPCI(pc2) && ((pcfl2 = PCI(pc2)->pcflow) != NULL))
							{
								if (pcfl2 == pcfl1)
								{
									...
									if (mc35_pCodeOptime2pCodes(pc1, pc2, pcfl_used, reg, 0, optimize_level))
										searching = 0;
								}
							}
...
#define OPT_PASSES 4
	...
	do
	{
		saved = mc35_total_registers_saved;

		/* Identify registers used in one flow sequence */
		mc35_OptimizeRegUsage(mc35_dynAllocRegs, level, (OPT_PASSES - passes));
		mc35_OptimizeRegUsage(mc35_dynStackRegs, level, (OPT_PASSES - passes));
		mc35_OptimizeRegUsage(mc35_dynDirectRegs, 0, (OPT_PASSES - passes));

		if (mc35_total_registers_saved != saved)
			DFPRINTF((stderr, " *** pass %d, Saved %d registers, total saved %d ***\n",
					  (1 + OPT_PASSES - passes), mc35_total_registers_saved - saved, mc35_total_registers_saved));

		passes--;

	} while (passes && ((mc35_total_registers_saved != saved) || (passes == OPT_PASSES - 1)));

	if (mc35_total_registers_saved == t)
		DFPRINTF((stderr, "No registers saved on this pass\n"));
}

/*-----------------------------------------------------------------*
* void mc35_RegsUnMapLiveRanges(set *regset)
*
*-----------------------------------------------------------------*/
static void mc35_RegsSetUnMapLiveRanges(set *regset)
{
	reg_info *reg;
...
void mc35_RegsUnMapLiveRanges(void)
{
	...
	mc35_RegsSetUnMapLiveRanges(mc35_dynAllocRegs);
	mc35_RegsSetUnMapLiveRanges(mc35_dynStackRegs);
	mc35_RegsSetUnMapLiveRanges(mc35_dynDirectRegs);
	mc35_RegsSetUnMapLiveRanges(mc35_dynProcessorRegs);
	mc35_RegsSetUnMapLiveRanges(mc35_dynDirectBitRegs);
	mc35_RegsSetUnMapLiveRanges(mc35_dynInternalRegs);
}
