...
/*------------------------------------------------------------------*/
void resolveIvalSym(initList *ilist, sym_link *type)
{
...
/*------------------------------------------------------------------*/
static void moveNestedInit(initList *deepParent, initList *src)
{
...
byteOfVal(value *val, int offset)
{
  ...
  if (SPEC_LONG(val->etype))
  {
    if (SPEC_USIGN(val->etype))
      ...
      return offset < 4 ? (SPEC_CVAL(val->etype).v_long >> shift) & 0xff : (SPEC_CVAL(val->etype).v_long < 0 ? 0xff : 0);
  }
  ...
  if (SPEC_NOUN(val->etype) == V_INT)
  {
    if (SPEC_USIGN(val->etype))
      ...
      return offset < 2 ? (SPEC_CVAL(val->etype).v_int >> shift) & 0xff : (SPEC_CVAL(val->etype).v_int < 0 ? 0xff : 0);
  }
  ...
  if (SPEC_NOUN(val->etype) == V_CHAR)
  {
    if (SPEC_USIGN(val->etype))
      ...
      return offset < 1 ? SPEC_CVAL(val->etype).v_int & 0xff : (SPEC_CVAL(val->etype).v_int < 0 ? 0xff : 0);
  }
...
valShift(value *lval, value *rval, int lr)
{
  ...
  if (SPEC_LONG(val->type))
  {
    if (SPEC_USIGN(val->type))
    {
      SPEC_CVAL(val->type).v_ulong = lr ? (TYPE_TARGET_ULONG)ulFromVal(lval) << (TYPE_TARGET_ULONG)ulFromVal(rval) : (TYPE_TARGET_ULONG)ulFromVal(lval) >> (TYPE_TARGET_ULONG)ulFromVal(rval);
    }
    else
    {
      SPEC_CVAL(val->type).v_long = lr ? (TYPE_TARGET_LONG)ulFromVal(lval) << (TYPE_TARGET_ULONG)ulFromVal(rval) : (TYPE_TARGET_LONG)ulFromVal(lval) >> (TYPE_TARGET_ULONG)ulFromVal(rval);
    }
  }
  else
  {
    if (SPEC_USIGN(val->type))
    {
      SPEC_CVAL(val->type).v_uint = lr ? (TYPE_TARGET_UINT)ulFromVal(lval) << (TYPE_TARGET_ULONG)ulFromVal(rval) : (TYPE_TARGET_UINT)ulFromVal(lval) >> (TYPE_TARGET_ULONG)ulFromVal(rval);
    }
    else
    {
      SPEC_CVAL(val->type).v_int = lr ? (TYPE_TARGET_INT)ulFromVal(lval) << (TYPE_TARGET_ULONG)ulFromVal(rval) : (TYPE_TARGET_INT)ulFromVal(lval) >> (TYPE_TARGET_ULONG)ulFromVal(rval);
    }
  }
  return cheapestVal(val);
}
