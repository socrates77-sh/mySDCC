...
#define SKIP_SPACE(x, y)                          \
  {                                               \
    while (*x && (ISCHARSPACE(*x) || *x == '\n')) \
      x++;                                        \
    if (!*x)                                      \
    {                                             \
      fprintf(stderr, y);                         \
...
#define EXPECT_STR(x, y, z)                \
  {                                        \
    while (*x && strncmp(x, y, strlen(y))) \
      x++;                                 \
    if (!*x)                               \
    {                                      \
      fprintf(stderr, z);                  \
...
replaceRule(lineNode **shead, lineNode *stail, peepRule *pr)
{
  ...
  for (cl = *shead; cl != stail; cl = cl->next)
  {
    ...
    {
      pl = (pl ? connectLine(pl, newLineNode(cl->line)) : (comment = newLineNode(cl->line)));
 ...
 */
bool isLabelDefinition(const char *line, const char **start, int *len, bool isPeepRule)
{
  const char *cp = line;
...
bool isLabelReference(const char *line, const char **start, int *len)
{
  const char *s, *e;
