...
int dbuf_printOperand(operand *op, struct dbuf_s *dbuf)
{
  sym_link *opetype;
        ...
        if (OP_SYMBOL(op)->isspilt)
        {
          if (!OP_SYMBOL(op)->remat)
            if (OP_SYMBOL(op)->usl.spillLoc)
              dbuf_printf(dbuf, "[%s]", (OP_SYMBOL(op)->usl.spillLoc->rname[0] ? OP_SYMBOL(op)->usl.spillLoc->rname : OP_SYMBOL(op)->usl.spillLoc->name));
            else
...
/*-----------------------------------------------------------------*/
int isOperandGlobal(const operand *op)
{
  if (!op)
    return 0;
...
/*-----------------------------------------------------------------*/
int isOperandVolatile(const operand *op, bool chkTemp)
{
  if (!op)
    return 0;
...
/*-----------------------------------------------------------------*/
int isOperandLiteral(const operand *const op)
{
  sym_link *opetype;
...
/*-----------------------------------------------------------------*/
bool isOperandInPagedSpace(operand *op)
{
  sym_link *etype;
...
/*------------------------------------------------------------------*/
bool isOperandInDirSpace(operand *op)
{
  sym_link *etype;
...
/*-----------------------------------------------------------------*/
bool isOperandInBitSpace(operand *op)
{
  sym_link *etype;
...
/*--------------------------------------------------------------------*/
bool isOperandInCodeSpace(operand *op)
{
  sym_link *etype;
...
/*-----------------------------------------------------------------*/
bool isOclsExpensive(struct memmap *oclass)
{
  if (port->oclsExpense)
...
/*-----------------------------------------------------------------*/
int isiCodeInFunctionCall(iCode *ic)
{
  iCode *lic = ic;
...
operandOperation(operand *left, operand *right, int op, sym_link *type)
{
  ...
  case LEFT_OP:
    ...
    if (IS_LONGLONG(type))
      ...
      retval = operandFromValue(valCastLiteral(type,
                                               ((TYPE_TARGET_ULONG)double2ul(operandLitValue(left)) << (TYPE_TARGET_ULONG)double2ul(operandLitValue(right))),
                                               ((TYPE_TARGET_ULONG)double2ul(operandLitValue(left)) << (TYPE_TARGET_ULONG)double2ul(operandLitValue(right)))));
    break;
  case RIGHT_OP:
...
/*-----------------------------------------------------------------*/
int isOperandEqual(const operand *left, const operand *right)
{
  ...
  case VALUE:
    return (compareType(left->svt.valOperand->type, right->svt.valOperand->type) &&
            (!IS_FLOAT(getSpec(left->svt.valOperand->type)) ? (operandLitValueUll(left) == operandLitValueUll(right)) : (operandLitValue(left) == operandLitValue(right))));
  case TYPE:
...
/*------------------------------------------------------------------*/
int aggrToPtrDclType(sym_link *type, bool force)
{
  if (IS_PTR(type) && !force)
...
geniCodeArray(operand *left, operand *right, int lvl)
{
  ...
  IC_RESULT(ic) = newiTempOperand(((IS_PTR(ltype) && !IS_AGGREGATE(ltype->next) && !IS_PTR(ltype->next)) ||
                                   (IS_ARRAY(ltype) && IS_FUNCPTR(ltype->next)))
                                      ? ltype
                                      : ltype->next,
                                  0);
...
/*-----------------------------------------------------------------*/
void geniCodeFunctionBody(ast *tree, int lvl)
{
  iCode *ic;
  operand *func;
  char *savefilename;
...
/*-----------------------------------------------------------------*/
void geniCodeIfx(ast *tree, int lvl)
{
  iCode *ic;
...
int geniCodeJumpTable(operand *cond, value *caseVals, ast *tree)
{
  ...
  sizeofJumpTable = (1 + max - min) * port->jumptableCost.sizeofElement + port->jumptableCost.sizeofDispatch + sizeofMinCost + sizeofMaxCost;
...
/*-----------------------------------------------------------------*/
void geniCodeSwitch(ast *tree, int lvl)
{
  iCode *ic;
  ...
  case ADD_ASSIGN:
  {
    ...
    return geniCodeAssign(left,
                          geniCodeAdd(geniCodeRValue(operandFromOperand(left),
                                                     FALSE),
                                      right, getResultTypeFromType(tree->ftype), lvl),
                          0, 1);
  }
  case SUB_ASSIGN:
  {
    ...
    return geniCodeAssign(left,
                          geniCodeSubtract(geniCodeRValue(operandFromOperand(left),
                                                          FALSE),
                                           right, getResultTypeFromType(tree->ftype)),
                          0, 1);
  }
  case LEFT_ASSIGN:
    return geniCodeAssign(left,
  ...
  case AND_ASSIGN:
    return geniCodeAssign(left,
                          geniCodeBitwise(geniCodeRValue(operandFromOperand(left),
                                                         FALSE),
                                          geniCodeRValue(right, FALSE), BITWISEAND, operandType(left)),
                          0, 1);
  case XOR_ASSIGN:
    return geniCodeAssign(left,
                          geniCodeBitwise(geniCodeRValue(operandFromOperand(left),
                                                         FALSE),
                                          geniCodeRValue(right, FALSE), '^', operandType(left)),
                          0, 1);
  case OR_ASSIGN:
    return geniCodeAssign(left,
