 ...
 */
...

    {
      ...
      dbuf_append (&dbuf, "-D__SDCC_MC", sizeof ("-D__SDCC_MC") - 1); //zwr
...
_pic14_do_link (void)
{
  ...
  sprintf (lfrm, "{linker} {incdirs} {sysincdirs} {lflags} -w -c -m -o \"{outfile}\" \"{user_ofile}\" {spec_ofiles} {ofiles} {libs}"); //zwr
  ...
  if (pic14_getPIC()->isEnhancedCore)
  ...
  else
    {
		//zwr
		/*
		if ( strcmp (port->target, "mc30") == 0 )
			addSet (&libFilesSet, Safe_strdup ("libsdcc-mc30.lib"));
		else if ( strcmp (port->target, "mc31") == 0 )
			addSet (&libFilesSet, Safe_strdup ("libsdcc-mc31.lib"));
		else
			addSet (&libFilesSet, Safe_strdup ("libsdcc-mc32.lib"));*/
    }
  SNPRINTF (temp, sizeof (temp), "%s.lib", procName); //zwr
...
/* Globals */
PORT mc3x_port =		//MC30 zwr
{
  TARGET_ID_PIC14,
  "mc3x",
  "SinoMCU MC3X",        /* Target name */
  "",               /* Processor */
  {
    picglue,
