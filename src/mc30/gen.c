...
call_libraryfunc (char *name)
{
  ...
  //emitpcode (POC_PAGESEL, popGetWithString (name, 1)); //zwr
  /* call the library function */
  emitpcode (POC_CALL, popGetExternal (name, 0));
  /* might return from different page... */
  //emitpcode (POC_PAGESEL, popGetWithString ("$", 0)); //zwr
  ...
  if (IFFUNC_ISCRITICAL (ftype))
    {
      ...
      if (IFFUNC_ARGS (sym->type))
        {
          fprintf (stderr, "MC3X: Functions with __critical (%s) must not have arguments for now.\n", sym->name);//zwr
          exit (1);
        }                       // if
    }                           // if
