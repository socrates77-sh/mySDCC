...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(insertIntoLoop)
{
  eBBlock *ebp = item;
  V_ARG(set **, regionSet);
...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(isNotInBlocks)
{
  eBBlock *ebp = item;
  V_ARG(set *, blocks);
...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(addToExitsMarkDepth)
{
  eBBlock *ebp = item;
...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(addDefInExprs)
{
  eBBlock *ebp = item;
  V_ARG(cseDef *, cdp);
  V_ARG(ebbIndex *, ebbi);
...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(pointerAssigned)
{
  eBBlock *ebp = item;
  V_ARG(operand *, op);
...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(hasNonPtrUse)
{
  eBBlock *ebp = item;
  V_ARG(operand *, op);
...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(findInduction)
{
  induction *ip = item;
  V_ARG(operand *, sym);
  V_ARG(induction **, ipp);
...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(mergeRegions)
{
  region *theLoop = item;
  V_ARG(set *, allRegion);
  region *lp;
...
/*-----------------------------------------------------------------*/
static DEFSETFUNC(mergeInnerLoops)
{
  region *theLoop = item;
  V_ARG(set *, allRegion);
  V_ARG(int *, maxDepth);
...
/*-----------------------------------------------------------------*/
int loopOptimizations(hTab *orderedLoops, ebbIndex *ebbi)
{
  region *lp;
  int change = 0;
  int k;
